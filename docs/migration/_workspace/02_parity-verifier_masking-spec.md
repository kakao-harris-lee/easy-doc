...
fixture: `parity/fixtures/masking/masking.json` — **83 케이스·단언 299개**, 생성기 `dump_parity_fixtures.py::build_masking`
...
### 2.4 판정을 보류한 자리 (`verdict_pending`) — 4차에서 0건, **6차에서 2건**(§10.3)
...
1. `parity/actual/masking/masking.json`이 `parityHarness` 실행으로 생성된다 (`runtime: kotlin`, **83 케이스**).
...
3. 아래가 종료 코드 **3**(부분 검증 통과)으로 끝난다:
   ...
   ```
   마지막 줄이 `부분 검증 통과(게이트 아님): … 성질 판정 83건(단언 299개) … 불충족 0건`이어야 한다.
