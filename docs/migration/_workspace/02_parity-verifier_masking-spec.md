...
### 2.4 판정하지 않는 자리 (`known_gap`) — 4차에서 0건, **6차에서 2건**(§10.3)
...
**그래서 `known_gap`으로 남겼다.** `absent`를 걸면 과잉을 요구사항으로 굳혀 나중에 카드 패턴을 좁히는 개선이 회귀로 잡히고, `present`를 걸면 판정되지 않은 방향을 이 레인이 대신 정하는 것이 된다.

| 케이스 | 관측 |
|---|---|
| `known-gap-four-groups-single-space` | `2021 2022 2023 2024` → 가려진다 |
| `known-gap-five-groups-single-space` | 다섯 열이면 **앞 네 개만** 가려진다 — 후행 그룹 경계를 보지 않는다 |

> ⚠ **이 도메인의 `known_gap`이 0건에서 2건으로 늘었다.** §2.4가 *"새 `known_gap`을 넣으려면 그 자리가 아직 어느 게이트에도 판정을 요청하지 않은 상태여야 한다"*고 적었는데, 이 자리는 판정을 **요청받고 명시적으로 판정하지 않기로 한** 상태다. §2.4의 기준을 그대로 읽으면 이것은 `known_gap`이 아니라 대기다. **그러나 대기 표기가 없다** — 그리고 `absent`/`present` 어느 쪽도 지금 걸 수 없다는 것이 판정문의 결론이다. 이 어긋남을 숨기지 않고 적어 둔다: 다음 회차에서 "판정 보류" 표기를 별도로 만들지, `known_gap`의 정의를 넓힐지 정해야 한다.
