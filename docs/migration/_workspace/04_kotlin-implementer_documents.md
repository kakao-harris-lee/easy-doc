...
# C4-R — 리더 지적 R-1~R-4 조치 (같은 변경 단위)

**일자:** 2026-08-21 / **기준:** 리더 수락 후 지적 R-1·R-2·R-3 + stop-time codex 게이트가 낸 R-4.
세 건 모두 「처방의 논리가 맞는가」가 아니라 **「그래도 무엇을 증명하지 못하는가」**를 물어 나왔다.

## C4-R1 — 새 탐지기의 도달 경계를 **실측했다** (전칭을 사실로 갈았다)

**무엇이 틀렸나.** C4 의 KDoc 이 *"이 축은 무엇이 앞단에서 거절했는지를 묻지 않는다 — 열거에 의존하지 않는 이유가 그것이다"* 라고 적었다. **재지 않은 전칭**이었다. 관측 지점이 `@WebMvcTest` 슬라이스이므로 참인 것은 「이 축은 **슬라이스 안에서 일어난 거절만** 본다」였다.

**형태별로 길이 가드를 심어 쟀다.** 가드는 `POST /workspaces` 를 422 + **배열** `detail` 로 거절한다(관측 대상은 「그 응답이 탐지기에 보이는가」이므로, 술어를 단순하게 두어 교란 요인을 없앴다). 형태 선택은 환경 변수 `EASYDOC_PROBE_FORM` 이다. 임시 측정 코드는 **커밋하지 않았고** 재현용 사본을 세션 스크래치에 남겼다.

| 앞단 장치 형태 | 슬라이스 축 | 컨테이너 축 | 판정 |
|---|---|---|---|
| (없음 — 기준선) | GREEN 0/5 | GREEN 0/2 | 기준선 |
| `@Component` 필터 | **RED 1/5** | **RED 1/2** | 두 축 모두 **본다** |
| 임포트 안 된 `@Configuration` 의 `@Bean` 필터(= `CorsConfig` 형태) | GREEN 0/5 | **RED 1/2** | 슬라이스가 **못 본다** ← 리더 가설 확인 |
| 톰캣 Engine 밸브 | GREEN 0/5 | **RED 1/2** | 슬라이스가 **못 본다** |
| `WebMvcConfigurer.addInterceptors` 로 등록한 인터셉터 | **RED 1/5** | **RED 1/2** | 두 축 모두 본다 |
| `@Component` `HandlerInterceptor`(등록 없음) | GREEN 0/5 | GREEN 0/2 | **가드로 성립하지 않는다**(아래) |
| `WebMvcConfigurer.addArgumentResolvers` 의 커스텀 리졸버 | GREEN 0/5 | GREEN 0/2 | **불리지 않는다**(아래) |

**마지막 두 줄을 「탐지기가 놓쳤다」로 읽으면 오독이다 — 심은 가드가 애초에 돌지 않았다.**
- `@Component HandlerInterceptor` 는 **인터셉터 체인에 등록되지 않는다.** 같은 인터셉터 코드를 `WebMvcConfigurer.addInterceptors` 로 등록하면 두 축이 모두 빨개진다(위 표 4행) — **등록만 다른 통제 비교**라 원인이 등록임이 확정된다.
- 커스텀 인자 리졸버는 `supportsParameter` 호출이 **0회**였다(`System.err` 추적으로 실측). 내장 `@RequestBody` 처리가 먼저 잡으므로 커스텀 목록에 닿지 않는다. 즉 이 형태는 **이 다섯 필드의 가드가 될 수 없다.**

**조치.** 컨테이너 축을 세웠다 — `RequestFieldRejectionReachTest`(`@SpringBootTest(RANDOM_PORT)` + 실 PostgreSQL). 판정과 프로브 조립은 새 `RequestFieldProbes` **한 벌**을 두 축이 공유한다(두 벌이면 한쪽만 고쳐지는 날 서로 다른 것을 재면서 둘 다 초록이 된다). 슬라이스 축은 남긴다 — 도커·DB 없이 돌고 DTO 가 생기는 즉시 도는 축이다. 두 KDoc 의 전칭을 위 표로 갈았다.

**그래도 증명하지 못하는 것** — 컨테이너 축도 **이 저장소가 실제로 띄우는 스택** 안의 장치만 본다. 리버스 프록시·API 게이트웨이·서블릿 컨테이너 교체처럼 **프로세스 밖**에서 길이를 재는 배치는 어느 축도 보지 못하며, 그것을 잡는 장치는 **없다**(측정된 경계다. 백로그 B-12).

## C4-R2 — `measured_on` 축을 **핀이 강제하는 자리**로 옮겼다

**리더가 좁힌 판정을 확인했다.** 축 자체는 살아 있다 — `WorkspaceContractTest` WC-7(원시 초과·정규화 후 이하 → 통과)·WC-8(정규화가 저장까지), `AuthContractTest` S-2·S-5(정규화 후 길이). 남은 문제는 **구조**였다: 그 케이스들은 서로 다른 클래스에 손으로 쓰였고, C7 이 `edited_text`(`measured_on`: 정규화 후)를 만들면 **길이 프로브는 핀이 강제하지만 정규화 프로브는 아무도 강제하지 않는다.** 게다가 C4 프로브 값은 `"가".repeat(n)` 이라 원시 길이 == 정규화 길이여서 원리적으로 축을 구별할 수 없었다.

**고른 답: 정규화 프로브를 탐지기 안에 둔다**(기존 케이스를 재구현하지 않고, 존재 검사도 하지 않는다).

- **존재 검사를 고르지 않은 이유**: 「어느 테스트에 그 케이스가 있다」를 소스에서 확인하는 장치는 **범위 선언형**이고 규칙 ⑶ 이 걸린다 — 이름·문면 매칭이라 케이스가 이름을 바꾸면 조용히 0건이 된다. F3 에서 이미 옳게 거부한 방향이다.
- **재구현이 아닌 이유**: 여기서 하는 단언은 **「갈리는 입력이 계약 축대로 갈린다」한 줄**이다. WC-7 은 그 위에 통과 상태를, WC-8 은 저장까지의 전파를, S-4 는 문구 동일성을 함께 재고 그것들은 그대로 남는다. 겹치는 부분은 **관측 지점이 다른 의도적 중복**이다(슬라이스 하나 + 컨테이너 하나).

**프로브 모양은 계약이 정하고 방향은 측정한다.** 원시 길이 `base+noise`, 정규화 후 `base` 인 값을 만들고 — 상한 필드는 `base = limit`, 하한 필드는 `base = limit-1` — 계약 `measured_on` 이 `NORMALIZED` 면 **통과**, `RAW` 면 **거절**을 요구한다. 상한/하한 방향은 `limit±1` 중 어느 쪽이 거절됐는지로 **측정**한다(코드에 적지 않는다). 잡음은 필드별로 다르다(`email` 은 앞 공백, 나머지는 제어문자) — 출처는 그 필드의 `measured_on` 산문이다.

**컨테이너 축이 프로브 결함 하나를 잡았다.** `name` 의 경계 프로브와 정규화 프로브가 정규화 후 같은 문자열이 되어 **409(이름 중복)** 이 났다 — 재려던 축 대신 다른 계약 조항을 재고 있었다. 슬라이스 축은 프로브마다 계정을 새로 만들어 **그 전제를 우회하고 있었다.** 값 조립을 「길이는 지키고 첫 글자만 회전」으로 고쳐 두 축에서 함께 닫았다.

**그래도 증명하지 못하는 것** — 잡음 선택이 계약 `measured_on` **산문에서 손으로 옮긴 것**이라, 계약이 정규화 규칙을 바꾸면(예: `email` 도 제어문자를 걷는다) 프로브가 낡은 잡음을 쓸 수 있다. 그 드리프트를 잡는 장치는 **없다**(백로그 B-13).

## C4-R3 — `name` 의 두 문구를 갈래별로 묶었다

종전 판정은 `detail !in declared` 뿐이라 **상한 위반에 빈 값 문구가 나가도 통과**했다. 계약이 문구를 **둘** 선언한 필드에 대해 「관측된 서로 다른 거절 문구의 집합이 선언 집합과 정확히 같다」를 요구하도록 바꿨다. 갈래 수를 계약에서 읽으므로 문구가 늘면 그만큼의 관측을 요구한다.

**빈 값 프로브를 여기 배선했다**(WC-10 에 의존하지 않는다). 근거: 이 축의 강제자는 계약 필드 집합에 걸린 **정확 열거 핀**이고, 두 번째 문구의 관측을 다른 클래스에 맡기면 C7 의 새 필드에서 같은 빈자리가 재발한다. 문구가 하나인 필드에는 빈 값 프로브를 돌리지 않는다 — `text` 의 빈 본문 문구는 `fields[].detail` 이 아니라 `POST /documents` 422 예시(`empty_body`)라 집합이 어긋난다.

**그래도 증명하지 못하는 것** — 「어느 문구가 어느 갈래에 대응하는가」는 계약에 **라벨이 없다.** 이 단언은 「두 갈래가 서로 다른 문구를 쓴다」까지이고, 둘이 **맞바뀌어** 나가도 통과한다. 계약에 갈래 라벨을 넣는 것은 조항 변경이라 `contract-keeper` 몫이다(백로그 B-14).

## C4-R4 — 열거를 **없앴다**: 이름 목록 → (전이적) `@Constraint` 보유

**stop-time codex 게이트가 막은 경로.** 바이트 축의 관측창은 경계 ±1 근처다. 그래서 **계약보다 느슨한** 경계를 가진 제약은 어느 프로브에서도 발화하지 않는다 — 계약 상한 50 인 `name` 에 `@CodePointLength(max = 100)` 을 달면 49·50·51·정규화·빈 값 전부 100 아래다. 그리고 `@CodePointLength` 는 종전 `FORBIDDEN_ANNOTATIONS` **밖**이다. **두 강제자가 각각 인정한 구멍이 합성되면 통과한다.**

**프로브를 멀리 쏘지 않았다.** 창이 넓어질 뿐 여전히 창이고, `text`(4,000)에서 크게 키우면 전송 상한에 걸려 413 이 나와 축이 오염된다 — 열거를 넓히는 것과 같은 종류의 조치다.

**대신 열거를 없앴다.** Bean Validation 제약에는 정의적 성질이 있다: 그 애너테이션 타입 자신이 `@jakarta.validation.Constraint` 로 (합성 제약이면 전이적으로) 메타 애너테이트돼 있고, 없으면 Bean Validation 이 제약으로 취급하지 않으므로 **예외가 원리적으로 없다.** 제약은 명세상 `RUNTIME` 유지라 리플렉션으로 보인다. `RequestFieldConstraintLayerTest` 의 판정을 **「이름이 목록에 있는가」에서 「(전이적) `@Constraint` 를 지녔는가」로** 갈았고 `FORBIDDEN_ANNOTATIONS` 를 지웠다. **범위 선언형이 탐지형이 된 자리다.**

**음성 대조 — 세 칸 (실측 2026-08-21).** 변이: `WorkspaceNameRequest.name` 에 `@field:CodePointLength(max = 100)` + `WorkspaceController.create` 의 `@RequestBody` 에 `@Valid`.

| # | 겨눈 장치 | 결과 | 판정 |
|---|---|---|---|
| ⑴ | **종전 열거 스캔**(같은 변이 + 판정 한 줄만 옛 아홉 이름으로 되돌림) | **GREEN 0/4** | 열거는 이 편집을 통과시킨다 |
| ⑵ | 바이트 축 — 슬라이스 | **GREEN 0/5** | 관측창 밖이라 발화하지 않는다 |
| ⑵ | 바이트 축 — 컨테이너 | **GREEN 0/2** | 같음 |
| ⑶ | **새 구조 단언**(전이적 `@Constraint`) | **RED 1/4** | *"이름이 아니라 (전이적) @Constraint 보유로 판정한다"* 로 그 자리를 지목 |

세 칸이 예측대로 채워졌다. **복원 sha256 전건 일치**(`WorkspaceDtos.kt` `bd2b6222…6566`, `WorkspaceController.kt` `afa60fef…9234`, `RequestFieldConstraintLayerTest.kt` `57102739…f5dd`). `cp`·`git stash` 미사용.

**부수 판정 하나 — `@NotNull` 도 이제 걸린다.** 종전 열거는 그것을 일부러 뺐다(계약이 「필드 누락」을 배열 갈래로 명시했으므로 스키마 층 실패가 맞다는 논리). 그러나 `@NotNull` 은 누락이 아니라 **명시적 `null`** 에 발화하고, 이 저장소는 그 갈래를 전역 `Nulls.FAIL`(`InvalidNullException` → `type: "missing"`)로 이미 처리한다. 다섯 필드에 `@NotNull` 을 다는 것은 **두 번째 판정 지점**을 만드는 일이라 함께 금지하는 편이 옳고, 오늘 붙어 있지 않다는 것을 이 케이스가 확인한다. **이것은 금지 범위의 확대**이므로 사실로 기록한다.

**그래도 증명하지 못하는 것 — 리더의 독해를 검토했다.** 리더가 본 잔여는 「Bean Validation 이 아닌 앞단 가드가 계약과 **똑같은 문자열 바이트**를 내는 경우」이고 「계약이 나간 바이트를 정본으로 삼으므로 위반이 아니다」였다. **그 독해가 맞다** — 계약 `x-request-field-constraints.x-what-this-does-not-say` 계열 조항과 `x-stored-text-domain.x-what-this-does-not-say`(*"어느 층에서 거르는지는 구현 판단이다. 계약이 정하는 것은 나간 바이트뿐이다"*)가 같은 규정이다. 다만 **한 가지 단서를 붙인다**: 그런 가드는 계약이 정한 **측정 축**(정규화 전/후)까지 같아야 위반이 아니고, 축이 갈리면 C4-R2 의 정규화 프로브가 잡는다. 즉 잔여는 「축까지 같은 대체 구현」이고 그것은 정의상 계약 준수다.
...
# C4-R8 — 개수를 「선언된 것」이 아니라 **「실제로 돈 것」**으로 센다

**일자:** 2026-08-21 / **계기:** stop-time codex 게이트 다섯째 — *"R-7의 새 개수 핀이 개별 강제자 비활성화를 여전히 통과시킵니다."* **내가 R-7 보고에 잔여로 신고했고 리더가 원장에 적고 넘어간 항목**이다. 잔여의 처분 기준이 정직성이 아니라 **악용 비용**이어야 했다.

## C4-R8-1. 구멍 실측 — 고치기 전 1·2행이 **초록**이었다

`MIN_TESTS_IN_FLOOR_CLASS` 는 소스의 `@Test` **애너테이션 수**를 센다. 그래서 메서드를 끄는 편집은 개수를 바꾸지 않는다. 바닥 클래스(`ValueSlotInvariantReachTest`)의 케이스 **하나**에 각각 심고 전체 `build` + 요구 모드 핀 게이트를 돌렸다:

| 변이 | `./gradlew build --rerun-tasks` | 핀 게이트(`REQUIRE_REPORT=1`) |
|---|---|---|
| `@Disabled` 한 줄 | **exit 0** | **141 passed — 초록** |
| `assumeTrue(...)` 한 줄 | **exit 0** | **141 passed — 초록** |

두 편집 모두 **한 줄**이고, 그 상태에서 값 자리 불변식은 돌지 않는다. 클래스도 개수도 그대로이므로 소스 축·바닥 목록·트리 스캔 어느 것도 울리지 않았다. 이것이 지적의 근거다.

## C4-R8-2. 처방 — 열거하지 않고 **리포트**에 묻는다

`@Disabled` 를 제외 목록에 넣는 것이 첫 충동이지만 그것은 열거이고, 게다가 **은폐형**이다 — `@Disabled`·`@DisabledIf*`·`@EnabledOnOs`·`assumeTrue`/`assumeFalse`·태그 제외·Gradle `--tests` 필터·JVM 인자가 전부 같은 일을 하고 그 목록은 닫히지 않는다(규칙 4 ⑵: 은폐형은 넓히지 말고 탐지형으로).

**결과는 「그 테스트가 안 돌았다」 하나로 수렴한다.** 그래서 Gradle JUnit XML 에서 그것만 본다. 축을 둘 더했다.

1. **`test_바닥_클래스가_리포트에서_하한만큼_실제로_돌았다`** — 바닥 클래스별 **실행된** 케이스 수가 `MIN_TESTS_IN_FLOOR_CLASS` 하한 이상. 요구 모드에서는 대상이 바닥 전건이고, 꺼져 있으면 리포트에 실재하는 것만 보되 **판정하지 못한 목록을 출력한다**(조용히 건너뛰면 그 플래그가 은폐형이 된다). 요구 모드에서 대상이 0 이면 실패다(공허 통과 금지).
2. **`test_리포트에_건너뛴_테스트가_없다`** — 리포트에 나온 **전부**에 대해 건너뜀 0. **하한 표가 필요 없다** — 어떤 기제로 껐든 건너뜀으로 나타난다. 범위를 바닥으로 좁히지 않은 근거는 실측이다: 신선한 전체 `build` 에서 실행 **1,062** · 건너뜀 **0**(`@Tag("llm")` 제외는 발견 단계에서 빠져 리포트에 아예 없다). **오늘 이 저장소에 정당한 건너뜀이 0 이므로 범위도 그만큼이다.**

**파서는 한 벌로 유지했다.** `_report_execution()` 을 `_report_counts()`(실행·건너뜀·전체)에 위임하도록 바꿨다 — 두 벌을 만들면 한쪽만 고쳐지는 날 서로 다른 것을 세면서 둘 다 초록이 된다.

## C4-R8-3. 소스 축은 **남겼다** — 두 축이 덮는 범위

| 축 | 세는 것 | 덮는 것 | 덮지 못하는 것 |
|---|---|---|---|
| **소스 축**(`…테스트_개수가_하한_아래로…`) | 소스의 `@Test` 애너테이션 수 | **선언이 사라지는 편집**(메서드 삭제). **Gradle 없이 돈다** — `quality` 잡에서도 잡는다 | 선언은 남고 **안 도는** 편집 |
| **리포트 축**(`…리포트에서_하한만큼_실제로_돌았다` · `…건너뛴_테스트가_없다`) | JUnit XML 의 **실행된**·**건너뛴** 케이스 수 | **안 도는 편집 전부** — 기제를 하나도 열거하지 않는다 | 리포트가 없는 실행 경로(`quality` 잡) |

마지막 칸이 소스 축을 남긴 이유다. `quality` 잡에는 Gradle 이 없어 리포트가 없고, 그 잡에서도 「메서드가 사라졌다」는 잡혀야 한다. L1/L2 를 함께 둔 R-5 와 같은 판단이고, **이번에도 범위가 실제로 달라서** 함께 둔다.

## C4-R8-4. `KOTLIN_GATE_REACH_REQUIRE_REPORT` 의 CI 도달 — **실측: 켜진다**

`.github/workflows/ci.yml` 을 읽었다. **`kotlin` 잡**(파일 230행)의 스텝 **「Kotlin 테스트 클래스 실행 대조 (리포트 XML — 삭제·미실행·skipped 시 실패)」**(297행)가

```yaml
working-directory: .
env:
  KOTLIN_GATE_REACH_REQUIRE_REPORT: "1"
run: uv run pytest tests/test_kotlin_gate_reach.py
```

로 배선돼 있고, 그 스텝은 같은 잡의 `build` **뒤**에 있다(그 순서를 `test_요구모드_대조_앞에_리포트를_덮는_스텝이_없다` 가 파일을 파싱해 지킨다). **그러므로 새 두 축은 CI 에서 요구 모드로 돌고 대상이 바닥 전건이다 — `ci.yml` 을 고칠 필요가 없었고 고치지 않았다.**

**플래그 부재 시 동작**(규칙 3 근거 6 재발 여부): 조용히 건너뛰지 **않는다**. ⑴ 하한 축은 리포트에 실재하는 바닥 클래스만 판정하고 **판정하지 못한 목록을 출력**한다. ⑵ 건너뜀 0 축은 플래그와 무관하게 돌고, 리포트가 아예 없을 때만 그 사실을 출력하고 넘어간다. 즉 플래그는 **대상 범위를 넓히는 스위치**이지 검사를 켜는 스위치가 아니다.

## C4-R8-5. 음성 대조 (4행 전건, 고친 뒤 재실측)

| 변이 | build | **고치기 전** | **고친 뒤** | 지목한 장치 |
|---|---|---|---|---|
| 1. `@Disabled` 한 줄 | exit 0 | **141 passed — 초록** | **RED** | `리포트에_건너뛴_테스트가_없다` · `바닥_클래스가_리포트에서_하한만큼_실제로_돌았다` |
| 2. `assumeTrue(비상수 거짓)` 한 줄 | exit 0 | **141 passed — 초록** | **RED** | 같은 둘 — **기제를 열거하지 않았음의 증거** |
| 3. 메서드 삭제(깔끔) | exit 0 | RED (R-7 이 닫음) | **RED** | `리포트에서_하한만큼…` · **`소스 축`** |
| 4. 무변이 | exit 0 | 초록 | **초록 143 passed** | 없음 |

2행이 핵심이다 — `@Disabled` 만 겨눈 처방이면 여기서 통과한다. 3행에서 「건너뜀 0」이 **울리지 않는** 것도 뜻이 있다: 삭제는 건너뜀이 아니므로 그 축이 오탐하지 않는다.

> **첫 시도의 2·3행은 판정으로 쓰지 않았다.** `assumeTrue(false)` 는 상수 조건이라 이후 코드가 도달 불가가 되어 `-Werror` 로 **빌드가 깨졌고**, 3행의 거친 삭제는 미사용 잔재를 남겨 같은 일이 났다. 둘 다 「탐지」가 아니라 내 변이가 컴파일을 깬 것이다. 비상수 조건과 깔끔한 삭제로 바꿔 `build exit 0` 을 확보한 뒤의 결과가 위 표다.
>
> 그 과정에서 **리포트가 낡을 수 있다**는 것도 관측했다 — 빌드가 실패한 회차에서는 앞 회차의 리포트가 남아 「건너뜀 1」이 그대로 읽혔다. CI 는 같은 잡에서 build 직후에 읽으므로 신선하지만, **로컬에서 빌드가 깨진 상태의 판정은 신뢰하지 않아야 한다.**

복원 sha256 일치 — `ValueSlotInvariantReachTest.kt` `4a8a74e8…0e85`(4행 전건), `tests/test_kotlin_gate_reach.py` `143b3c5b…9596`(HEAD 대조 시). `cp`·`git stash` 미사용.

## C4-R8-6. 범위 밖에서 발견한 것 — **HEAD 가 이미 ruff 를 어기고 있었다**

작업 중 `uv run ruff check .` 이 `tests/test_kotlin_gate_reach.py` 에서 **E501 1건**을 냈다. 내 편집이 아니었다 — HEAD(`8506e39`, 리더 `d816fb0`)의 파일을 그 자리에 되돌려 프로젝트 ruff 를 돌리니 **exit 1, Found 1 error** 였다(실측; 되돌린 뒤 sha256 대조로 복원 확인).

원인은 **표시 폭**이다. 그 줄은 66 **글자**지만 한글이 폭 2 로 세어져 **102**가 된다. `--isolated` 로는 E501 이 기본 규칙 집합에 없어 통과하므로, 부분 실행에서는 보이지 않는다.

**메시지 문자열을 두 줄로 재줄바꿈해 해소했다**(의미 변화 없음, 핀 상수 무변경). 리더 회차의 「ruff 0」 보고와 어긋나는 사실이므로 그대로 적는다.

## C4-R8-7. 그래도 무엇을 증명하지 못하는가 — **남는 것의 악용 비용**

| 남은 우회 | 악용 비용 | 오늘 이것을 잡는 것 |
|---|---|---|
| **단언만 비우기**(`assertThat(x).isNotNull()` 로 갈기, 프로브 값을 위반 아닌 값으로) | **한 줄이지만 diff 가 그 단언 자리에 남는다** — 리뷰가 보는 곳이다 | **없다.** pitest 뿐(B-19) |
| **케이스 하나 지우고 무해한 케이스 하나 더하기**(개수 유지) | 중간 — 두 편집이 한 커밋에 있어야 하고 새 메서드가 눈에 띈다 | **없다.** pitest 뿐(B-19) |
| `@Disabled`·`assumeTrue`·태그·필터로 끄기 | **한 줄** | **이 회차가 닫았다**(리포트 축) |
| 메서드 삭제 | 한 줄~ | R-7 소스 축 + 이 회차 리포트 축 |
| 클래스 삭제 | 파일 하나 | 바닥 목록 + 개수표 + 리포트 축 |
| **리포트 축 자체를 끄기**(`ci.yml` 의 그 스텝 삭제) | 한 줄 | `test_CI_가_이_대조를_경로_명시로_배선했다`(장치 밖 단언) |
| **`MIN_TESTS_IN_FLOOR_CLASS` 값을 낮추기** | 한 줄 | **없다** — 라쳇은 diff 로만 보인다. 리더 핀(`MIN_TEST_CLASSES`·`MIN_FLOOR_CLASSES`)과 같은 성질이다 |

**한 줄로 강제자를 끌 수 있는 경로 중 아직 열린 것은 마지막 줄 하나다** — 하한 값을 내리는 편집. 그것을 막는 것은 리뷰뿐이고, 같은 성질을 리더 핀 두 개가 이미 갖고 있다. 「단언 비우기」는 한 줄이지만 **그 줄이 단언 자리에 남아 리뷰가 보는 곳**이므로 pitest 를 기다릴 수 있다고 판단한다 — 그 판단의 근거가 위 표의 「악용 비용」 열이고, 처분은 리더 몫이다.
...
| `./gradlew ktlintCheck detekt build moduleBoundaryCheck parityHarness --continue --rerun-tasks` | **BUILD SUCCESSFUL** (경고 0) |
| 개인정보 스캐너 | exit 0 (**BLOCK 0**) |
| `KOTLIN_GATE_REACH_REQUIRE_REPORT=1 uv run pytest tests/test_kotlin_gate_reach.py` | **143 passed** (140 → 리포트 축 2 + 파서 위임 확인) |
| `uv run ruff check .` | All checks passed (**HEAD 의 E501 1건을 고친 뒤**) |
| `uv run mypy . .claude` | Success: 139 source files |
