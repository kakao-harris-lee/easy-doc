...
| # | 바꾼 노드 | 빨강이 된 케이스 | 판정 |
|---|---|---|---|
| **N-11a** | `fields[?name].limit` 50→**49** | WC-5 · WR-6 · P-19·P-20 | 상한이 코드에 복제돼 있지 않다 |
...
| **N-18** | `parameters[0].name` → `workspaceId` | WR-1·WR-3~WR-5·WR-8·WR-9 · WD-1·WD-2·WD-4·WD-5·WD-7·WD-8 | P-21 이 배선돼 URL 이 계약에서 온다 |
