...
**privacy-gate 판정 5건** (`07_privacy-gate_masking-verdicts.md`): 판정1 X-2 수정(종류 A·B — 구현·fixture 반영 완료, 해제조건 3·4 잔여) · 판정2 X-5 조건부 수용(규약+탐지기 — 완료) · 판정3 X-8 즉시 배선(완료, 리더 승인) · 판정4 X-11 Phase 5 유지+`baseUrl` 비바인딩(가드 테스트 존재) · 판정5 value class 3종 toString 수정(`bed5300` — 해제 확인은 privacy-gate 재확인 대기). **privacy-gate 레인은 세션 한도로 중단(2026-08-14 00시대, 3:20 재개 가능)** — 대기열 3건: ⓐ 스캐너 red 처분(`BODY_NAMES` 확장이 `collect_golden.py` 3줄을 잡음 — 오탐 추정, refine 훅 설계 판정. **CI quality 잡 스캔 단계가 이 판정까지 red**) ⓑ Y-2(구분자 무상한 반복의 과잉 마스킹 — U+3000 연속·U+2013 범위 표기) ⓒ `MaskingResult` toString 탐침.
...
**열린 판정 — 리더·사용자** (Phase 2 종료 전 처분 필요): ① **보정 채택 판정식이 본문·팩트 손실을 보지 않는다**(갈림 후보 ① — 축이 자리표시자·위반 건수뿐이라 축약을 개선으로 오인하는 방향으로 기울어 있음. "본문을 잃지 않았다" 기준 신설은 제품 정책 결정이라 **사용자 판단 필요**) ② Y-4(재시도를 가로지르는 누계 상한의 정본 부재) ③ Y-1 심각도(known_gap 전환은 `3934f06`으로 처리됨 — 재판정 여부) ④ 08 교차 종합의 조치 목록(진행 중).
