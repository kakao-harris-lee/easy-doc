...
| 품질 합격선 수치 확정·승인 (계획 §5 Phase 0 · §4.6 게이트2·5) | 아니오 | **[criteria-pivot 신설 2026-08-12]** 같은 커밋이 Phase 0 종료 조건에 "품질 합격선 승인"을 넣었으나 합격 수치가 저장소 어디에도 없다. tests/golden·judge 도구는 있으나 "필수 항목 충족률·규칙 위반 허용치·judge 절대 점수"의 수치 미확정 — 이 값 없이는 인벤토리 STY-03·STY-04의 미충족 판정 자체가 불가능하다 | 합격선 수치 미확정 (인벤토리 §9-A) | 리더 / Phase 0 재개 | criteria-pivot 재판정 (2026-08-12) |
...
- **`[호환]` Fernet·JWT·Argon2 spike + DB baseline(필수조치 D)** — 계획 변경 이력·CLAUDE.md가 지정한 **기준 전환 예외**다. 롤백 창에서 값이 같아야 하는 것 자체가 요구사항이므로 "Python과 같다"가 정당한 근거다. 예 유지. (단 spike는 **가능성** 판정이고 실제 구현·필수조치 A·B·C는 Phase 3·4 미결로 이미 원장에 있다.)
