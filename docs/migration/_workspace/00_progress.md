...
| DOCX/PDF/HWPX 추출 | 아니오 | `안 돎` | 미착수. Phase 0 spike(`00_kotlin-implementer_doc-spike.md`)만 있고 Kotlin 코드 0 | XXE/zip bomb/10MB/스캔 PDF 방어가 아직 어느 층에도 없다(I-11) · **추출기가 붙는 순간 X1(고아 서로게이트)이 도달을 얻는다** — privacy-gate 스스로 「손상된 docx/hwpx 가 고아 서로게이트를 만들 수 있다」고 적었다 | documents 단위 | leader (2026-08-19, 게이트 25) |
| 암호화 저장·복호화 조회 | 아니오 | `ci:kotlin` | **암호 어댑터는 섰고 저장 경로는 0이다.** `AesGcmContentCipher`(`fcf584b`)와 포트 `ContentCipher`(`74ec2b0`)가 CI `kotlin` 잡에서 돌지만(`AesGcmContentCipherTest` 17건 · `EncryptionSchemeSchemaTest` 3건), **`ContentCipher` 를 부르는 제품 코드 0 · INSERT repository 0**(3레인 일치 — 그래서 이 게이트의 ①사건이 0이다) | **게이트 26 으로 절반이 닫혔다 — 아래 「게이트 26 후속」 절이 정본이다.** **X9/F-6 는 성격이 바뀌었다**: 우회 스위치 삭제로 테스트가 자기점검을 *건너뛰는* 대신 *통과*하게 되어 자기점검 통과 로그가 **13 테스트 클래스**에서 나온다(직전 0) — 「모든 컨텍스트가 0세대로 조립된다」는 서술은 **더 이상 참이 아니다**. 남은 것은 **실제 INSERT 를 하는 통합 테스트**이고 그것은 이 단위의 몫이다 · **X10 은 닫혔다**(`7be37db`, 게이트 26 이 「이 회차 최고 형태」로 판정) · **X5/F-5 재암호화 4조건은 그대로 열려 있다**(포트 시그니처만 준비됨) | documents 단위 | leader (2026-08-19, 게이트 25) |
...
## 다음 세션 인수인계 (2026-08-20 세션 종료 시점, 리더)

> **여기부터 읽어라.** 이 절은 상태 요약과 포인터만 둔다 — 값은 각 산출물이 정본이다.
> 세션 커밋 범위 `0ce88b4..cb08e59` (24커밋). **워킹 트리 clean** (untracked 3건은 세션 이전부터
> 있던 것: `.playwright-mcp/` · `docs/*.doc` 2개). 미푸시 **40커밋**.

### §1 — 지금 어디인가

**Phase 4, `documents` 작업 단위 진행 중.** 착수는 리더가 허용했다(위 「게이트 26 후속」 §5).

| 단위 | 상태 |
|---|---|
| `crypto` | 구현 완료 · 게이트 25 · 조치 · **게이트 26** · 조치 — **닫힘** |
| `documents` | **C1(문서 추출기) 완료** (`df0766e`). 첫 게이트 덩어리 **G-α = C1 + C2 이므로 덩어리는 미완** |
| `export` | 미착수 (별 단위 — 계획 §0) |

**계약 레인**: C1(K5 해소) 완료 (`e0f102f`). **L-1 은 단독 집행 불가 판정**, L-2 미착수, C2~C9 남음.

### §2 — 다음 첫 동작

**`documents` C2** — 저장 경로 · 봉투 · **X9/F-6 실제 키 통합 테스트** · **X5/F-5 재암호화 4조건** ·
...
| **7** | 계획 §9 질문 ⑨(parity `ingest` 도메인 신설 — `parity-verifier` 합의) · ⑧(실문서 fixture — 지금 전부 합성물이라 「주요 fixture 불일치」를 절체 전에 검출할 방법이 없다) | 차분 비교 판정 |
| 8 | L-3(C4→C5 선결) · L-4·L-5·L-6 · 계획 §9 나머지 | 미도래 |
| — | **사용자 축 이월 2건**: ⑲ X24-3 넓은 강제(3회차 독립 재발) · ㉑ X1 처방 충돌(리더 기본값 「거부」가 서 있다) | 언제든 |

### §4 — 리뷰 게이트

**게이트 26 은 닫혔다.** 다음은 **게이트 27**이고 범위에 반드시 넣을 것:

- 이 세션의 조치·구현 커밋 (`385770e..cb08e59` 중 문서 전용을 뺀 것)
- **`e572476`**(L1 규칙 본문) — 게이트 26 기준선의 조상이라 **문면을 아직 아무도 안 봤다**(판정 ⑤)
- cross §10 **미교차 3건**

`{scope}` 는 리더가 지정해 내려보낸다. 이 세션은 `04_gate25-fixes` 를 썼다.

### §5 — 함정 (다음 사람이 밟기 쉬운 것)

1. **원시 제어문자가 이 게이트에서만 열두 번 재발했고 전부 「이스케이프를 쓰라」고 적는 문장 자신이었다.**
   그 글자를 타이핑하지 말고 **바이트 값에서 표기를 계산해** 넣어라. 커밋 메시지도 대상이다.
   전수 탐지기는 `tests/test_raw_control_chars.py`, 발생 시점 차단은 각 JSON 생성기의 `dump_json` 계열.
2. **`tests/test_kotlin_gate_reach.py` 의 핀은 손유지다.** Kotlin 레인에 열려 있으므로 테스트 클래스를
   더하면 함께 갱신한다. **이름을 빼는 것은 다르다** — 가드 소멸을 조용히 통과시키는 유일한 통로이고
   자동 탐지가 없다(위 「게이트 26 후속」 §6).
3. **`ParserNodeRegistryTest` 는 두 숫자를 각각 고정해야 한다**(명세 정의 36 · `ContractSpec.kt` 전용 1 ·
   합집합 37). 합집합만 재면 정의가 명세에서 코드로 새어도 초록이고, **그게 K5 가 태어난 경로다.**
4. **P-22 를 개명하지 마라** — `ContractSpec.kt:409` `deletionRefusalPrecedenceExample()` 로 확정됐다.
   documents 파서 범위는 **P-23~P-37**.
5. **게이트 명령에 파이프를 쓰지 마라**(종료 코드가 가려진다). 필요하면 `run_gate.sh` 경유.
   **`git stash` 를 공유 작업 트리에서 쓰지 마라** — 이 세션에 한 레인이 밟았고, 그 실행의 측정치를
   오염으로 폐기하고 재측정했다.
...
- 실제 GitHub Actions 관측: 미푸시 40커밋이라 이 세션 변경분은 **CI 에서 한 번도 안 돌았다**
