...
| **⑩ 게이트 19 ①·⑥ 수정 커밋** | **완료** (`e91ecdd` + `e600861`, 2 커밋) — **리뷰 미수령** | `ci:quality` | 개수 → **내용 결속**. stop-time 게이트가 같은 우회를 **두 번** 잡아 두 번째 커밋이 나왔다. **독립 리뷰를 받지 않았고**, 별도 게이트를 열지 않는 대신 **Phase 3 첫 리뷰 게이트 범위에 두 커밋을 포함**한다(리더 결정). 상세는 「게이트 19 후속」 §1 |
...
커밋 **`e91ecdd`·`e600861`** 은 **리뷰 미수령**이라 그 자체를 착수 근거로 쓰지 않고 **다음 게이트
대상으로 넘긴다**(⑩ 행 · 「게이트 19 후속」 §1).
...
| **게이트 19 잔여** | **해소 2** — ①·⑥ (`e91ecdd` + `e600861`, **리뷰 미수령**) / **한계 등재 3** — ②·③·⑧ (리더 결정 ⑶ — 수정하지 않는다) / **동결 대상** — ⑦⑨⑩⑪⑫⑬⑭⑯ + 게이트 18 이월(리더 결정 ⑴) / **판정 필요 1** — ⑤(§3 ⑪). **전부 하네스 자기 검사 장치**, 제품 코드 **0** | **러너 실사용(CI 배선) 전** · ①⑥ 리뷰는 **Phase 3 첫 게이트** | 「게이트 19 후속」 §1·§2 |
...
(**`e91ecdd` + `e600861`** 2 커밋), 그리고 **리더 결정 3건**을 원장 형식으로 옮긴 것이며, **수치·판정에는 커밋 해시를 함께 적는다**
(수치 인용 규약 — 기준 시점 없는 절대 수치는 다음 커밋에 거짓이 된다).
...
### §1 — 게이트 19 조치 배치 현황 (`e91ecdd` · `e600861`)

> 표의 번호(①~⑯)는 `reviews/19_gate18-fixes_cross.md` §3 교차 대조표의 항목 번호다.

| 게이트 19 | 커밋 | 실행 경로 | 근거·음성 대조 | 상태 |
|---|---|---|---|---|
| **① `_DYNAMIC_LOOKUP_NAMES` 빈 선언 통과** (Claude 단독 **차단②**, R19-1) | `e91ecdd` → `e600861` | `ci:quality` | **개수가 아니라 내용으로 묶었다.** `e91ecdd`: `_REQUIRED_DYNAMIC_LOOKUP_NAMES`(`getattr`·`__import__`·`eval`·`exec` **4종**)를 신설해 `_DYNAMIC_LOOKUP_NAMES` 가 그 넷을 **포함**함을 단언한다 — 비우거나 같은 개수의 다른 이름으로 치환하면 실패한다. 형제 상수(`_MAINLINE_ROOTS`·`_HELPER_SUFFIXES`)와 달리 이 집합은 비어도 "동적 조회 없음" 단언이 **공허하게 참**이었다(cross §6.1 B·V1 = **36 passed**). `e600861`: **핵심 4종 밖의 junk 치환**이 남아 있었다 — 나머지 이름도 `hasattr(builtins, name)` 로 **실재하는 builtin** 이어야 한다(없는 이름은 어떤 호출과도 일치하지 않아 자리만 채운다) | **해소** — **리뷰 미수령** |
...
`e91ecdd` +41 −2 · `e600861` +15 −1)라 리더가 직접 썼다. ⑶ **stop-time 게이트가 같은 우회를 두 번
잡았다** — **1차**: 초판이 `_DYNAMIC_LOOKUP_NAMES` 를 **개수**로 고정했는데 그것은 **같은 개수의 다른
...
**음성 대조 6변이** (일회용 worktree · `write_bytes` · `cp` 미사용): `e91ecdd` 기준 **36 passed** /
빈 집합 · 7개 junk 치환 · 문구 치환 2종 → **각 1 failed**. `e600861` 2변이(문구 복사본 · 비-builtin
이름) → **각 1 failed**.

**같은 지적이 두 번 난 것을 사실로 남긴다.** 1차 정정이 「개수 → 내용」이라는 **옳은 방향**이었는데도
그 방향을 **적용하지 않은 통로가 두 개 남았다** — 이것은 cross §9.4 가 기록한 「매 회차 같은 처방을
새 상수에는 적용하지 않아 다시 난다」는 형태가 **한 배치 안에서 되풀이된 것**이다.

**리뷰 미수령 — Phase 3 첫 리뷰 게이트 범위에 포함한다 (리더 결정).** **`e91ecdd`·`e600861` 두 커밋
모두 독립 리뷰를 받지 않았다** — **첫 리뷰 게이트 범위 = `e91ecdd` + `e600861`** 이다.
별도 게이트(20회차)를 열지 않는 이유는 **1줄~수십 줄 정확 일치 누락에 회차 하나를 여는 순환을
끊기 위해서**이고, 「하네스 변경도 리뷰 대상이다」라는 규칙은 **다음 게이트로 지킨다** — 즉 Phase 3 첫 리뷰
게이트의 대상 범위에 **`e91ecdd` + `e600861`** 을 **명시적으로 포함**시킨다. cross §7 이 이 방향에 대한 **반대 근거 3건과
...
| ③ | **API 과부하로 레인 3개가 중단·재개됐다.** 게이트 19 조치(`e91ecdd`·`e600861`)는 그 결과 **리더 직접 편집**으로 처리됐고, 그래서 **저작자와 심판자가 같아졌다** — 이 배치가 **리뷰 미수령**인 이유다. 그 상태에서 **stop-time 게이트가 같은 우회를 두 번 잡아** 두 번째 커밋이 나왔다 | `e91ecdd`·`e600861` 커밋 본문 · §1 성립 조건 ⑵⑶ | **하네스 규율의 예외가 환경 사정으로 발생했다는 사실을 지우지 않는다.** 예외를 정상화하지 않기 위해 **다음 게이트 범위에 두 커밋을 명시 포함**(§1 말미)시켰고, 그 사실을 아래 「Phase 3 착수 판정」 절이 착수 조건 행으로 다시 든다. **stop-time 게이트가 두 번 다 잡았다는 것이 리뷰 대체가 되지는 않는다** — 그것은 같은 세션 안의 검사이고, 독립 리뷰가 겨누는 것은 바로 그 자기 승인이다 |
...
**로컬 게이트 전건 통과** (HEAD `e600861`) — `ruff check` **exit 0** · `ruff format --check` **exit 0** ·
...
`e91ecdd`·`e600861` 은 **기존 테스트 안에 단언을 더한 것**이라 건수를 늘리지 않는다.
**중간 커밋 `e91ecdd` 도 일회용 worktree 에서 따로 쟀다** — `ruff`·`format`·`mypy`(137 파일) exit 0 ·
pytest **1242 passed exit 0**. 두 시점의 건수가 같다는 것이 「단언만 더했다」의 근거다.
worktree 는 `git worktree remove --force` 로 제거했다.
...
| **⑧ 게이트 19 ①·⑥ 수정 커밋** | **충족 — 단 리뷰 미수령** | `ci:quality` | **`e91ecdd` + `e600861`** (차단② ①과 합의 ⑥. stop-time 게이트가 같은 우회를 두 번 잡아 2 커밋이 됐다). **독립 리뷰를 받지 않았으므로 두 커밋을 착수의 적극 근거로 쓰지 않는다** — 착수를 막지 않는 이유는 둘 다 **하네스 자기 검사 테스트 1파일**(`tests/test_parity_ci_gate.py`)이고 제품 코드에 닿지 않기 때문이다. 리뷰는 **Phase 3 첫 리뷰 게이트 범위에 포함**한다(「게이트 19 후속」 §1) |
...
| **9** | **첫 리뷰 게이트 범위에 `e91ecdd` + `e600861` 을 포함**한다 — 리뷰 미수령 **2 커밋**을 다음 게이트가 반드시 본다 | 「게이트 19 후속」 §1 말미 |
