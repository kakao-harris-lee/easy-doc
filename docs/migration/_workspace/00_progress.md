...
|---|---| --- |---|---|---|---|---|
| 개인정보 마스킹 포팅 (`app/privacy/masking.py`) | 아니오 | `안 돎` | `masking` | - | 패턴 우선순위·자리표시자 번호·구간 겹침이 판정 대상 | - | - |
...
| 보정 채택 판정 포팅 | 아니오 | `안 돎` | `repair-adoption` | - | 자리표시자 유실·위반 건수 악화 가드 | - | - |
| placeholder 보존 검사 포팅 | 아니오 | `안 돎` | `repair-adoption` · `export` | - | **확인 필요**: 변환 결과 전체의 `missing_placeholders` 산출(`app/services/conversion.py:115`)은 어느 정본 도메인에도 배정돼 있지 않다. 도메인을 늘릴지 기존 도메인에 케이스를 넣을지 착수 시 판정 | - | - |
