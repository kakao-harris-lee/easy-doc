...
**그런데 그 잔여는 카나리의 핵심 주장을 흔들지 않는다.** 근거:

- 이 장치가 증명해야 하는 것은 **「본문·제목이 저장 경로(암호화·JDBC·트랜잭션)를 지난다」**이고,
  그것을 지는 것은 **⑵·⑶ 두 팔**이다. 둘은 **`202/NONE`** 으로 핀돼 있고 **성공 상태는 이른
  거절로 위조할 수 없다** — 앞단에서 끊기면 202 가 나오지 않는다.
- 잔여가 약화시키는 것은 **거절 경로 팔(⑷⑸⑹)의 층 귀속**이고, 그 팔들은 **보조 커버리지**다.

**그래서 「부분」 표기로 열어 두고 마감을 붙인다 — 마감 `export` 단위 착수 전.** 그때
