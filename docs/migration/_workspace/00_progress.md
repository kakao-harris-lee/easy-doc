...
| 텍스트 정규화·제어문자 제거 포팅 | 아니오 | `안 돎` | `text` | - | XML 1.0 비허용 문자만 제거하고 탭·개행·복귀는 유지 | - | - |
| 프롬프트 렌더링과 동적 어려운 말 목록 포팅 | 아니오 | `안 돎` | `prompts` | - | 시스템·사용자·보정 프롬프트 **전문**이 대조 대상 | - | - |
| 스타일 규칙 포팅 (`app/easyread/style_rules.py`) | 아니오 | `안 돎` | `style` · `style-tables` | - | 규칙 상수 표 전체 덤프까지 일치해야 한다. CLAUDE.md 규약대로 프롬프트 생성과 골든셋 평가가 같은 정의를 써야 한다 | - | - |
