...
> 있던 것: `.playwright-mcp/` · `docs/*.doc` 2개). **미푸시 323커밋 — 이 세션 변경분은 CI 에서
> 한 번도 돌지 않았다.**
...
| 순위 | 항목 | 어디서 걸리는가 |
|---|---|---|
| **1** | **M-3 — `lockSourceText`/`lockEnvelope` 가 `ownerId` 를 받지 않는다.** 소유권 조건 없는 **유일한 읽기 경로**이고 호출자 제한이 KDoc 한 문장뿐이다. C6 이 이것을 쓰면 타 사용자 노출이 성립한다. `privacy-gate` 판정 요청이 올라가 있다 | **C6 이전** |
| **2** | **L-1 잔여 두 갈래** (§2) | C3 |
...
- **실제 GitHub Actions 관측** — 미푸시 323커밋. 이 세션 변경분의 CI 도달은 **0**이다
