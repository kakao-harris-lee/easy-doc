...
**게이트 09 이후 수정 파동 (2026-08-14)**: 스캐너 재설계 `3570bdc`(M-01·M-03·M-09 — 사슬 파싱·상태 유지 lexer·균형 인자) · `75bfb40`(M-02·M-04 — 탐지기를 끄는 두 방법) · `3727905`(M-10·M-11 — 누락 6종 거절/실려 온 0 수용, usage 보존 절반은 M-12로 Phase 5 이월) · stop-gate 지적 `c2255dc`(주석 안 괄호가 인자 구간을 조기 종결 — "안전 접근이 방패" 기제, 재현이 회부 문구를 정정) · §4-septies 집행 `ac0307e`(TAB 제거·합집합 근거 교체·경계축 3+3) · fixture 동결 `56a70c1`(TAB 방향 전환 — **게이트 exit 3 복귀**, 108건·374단언, 경계축 12·감수 표면 known_gap 2·M-08 중복 검출·Z-7 분할: 상시 7종/한계 선언). **게이트 10(`10_detector-redesign`) 1단계 진행 중** — focus: 차단 3건 해소 검증·"Claude 단독 근거 해제" 6건 재확인·§4-septies 정합·fixture 방향.

**하네스 미결 (게이트 10 이후 처분)**: parity-verifier가 스스로 신고한 `known_gap` 정의 어긋남 — "판정을 요청받고 명시적으로 보류한" 상태(연도 배열 2건)를 "아직 판정을 요청하지 않은 자리" 정의로 표기 중. "판정 보류" 표기 신설 vs 정의 확장.

**열린 판정 — 리더·사용자** (Phase 2 종료 전 처분 필요): ① **보정 채택 판정식이 본문·팩트 손실을 보지 않는다**(갈림 후보 ① — 축이 자리표시자·위반 건수뿐이라 축약을 개선으로 오인하는 방향으로 기울어 있음. "본문을 잃지 않았다" 기준 신설은 제품 정책 결정이라 **사용자 판단 필요**) ② Y-4/C-07(재시도를 가로지르는 누계 상한 — "변환 1건"의 요구 해석, Phase 5) ③ 판정문 §4-ter.4 조건 5의 absent/present 문언 정정(privacy-gate 다음 회차 — 리더 확인은 완료, fixture는 정본 방향) ④ 감수한 과잉 표면(연도 배열)의 방향 판정(현재 known_gap 무단언 — 교차 검증 없는 단독 관측).
