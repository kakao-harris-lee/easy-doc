...
| **게이트 18 잔여 8건** | **해소 4** — X3(+R18-1·R18-2)·X4·T2 (`e2282b3`) · R3(이 갱신) / **진행 중 3** — X1·X2a·T1 (parity 레인) / **열림 1** — X5. **전부 하네스 자기 검사 장치**, 제품 코드 0 | **수정 배치 → 게이트 19** | 「게이트 17·18 후속」 §2 |
| **사용자 판단 대기 10건** | 게이트 15·14·16 이월 6 + 게이트 18 충돌 4. **전부 심각도·처방 라벨이고 차단 판정은 없다** — 어느 쪽으로 나든 마감이 바뀌지 않는 것이 다수다 | 각 행 명시 | 「게이트 17·18 후속」 §3 |
