...
| D-13 | 지원 확장자 집합 | **L2** | 구현은 상수를 갖고 **테스트가 계약 `x-input-limits.supported_upload_formats` 에서 읽어 케이스를 유도**(P-26) | 집합이 늘어도 검사가 안 늘면 새 형식이 **검사 자체를 받지 않는다** | N-26 형태 — 계약 원소를 빼면 DC-14 가 줄어야 한다 **← 상태는 §9.2-quater D-t 참조(2026-08-20 에 세웠다. N-26 은 미실행)** |
...
| D-17 | ~~파일명을 저장·로깅하지 않는다~~ | **L1** | ~~확장자 판별에만 쓰고 버린다. 제목은 사용자 제공 또는 본문 첫 줄 유도~~ | ~~파일명 자체가 개인정보일 수 있다~~ | ~~제목에 파일명이 들어가면 빨개지는 케이스~~ **← 이 행은 2026-08-20 사용자 결정으로 뒤집혔다. §9.2-quater D-o 가 정본이다.** 로깅 금지는 그대로이고, **저장 금지와 본문 유도 두 가지가 바뀌었다** |
...
- **D-6 (개정)** — 층 **L3** / 장치 **`XMLInputFactory` 3속성 명시(`SUPPORT_DTD=false` ·
  ...
  **← 이 문장은 거짓이다. 아래 D-6′ 로 대체한다.**
  **잃는 것**: DTD 폭탄과 손상 파일의 사용자 문구가 같아진다(§1.5 지점 1 이 이미 적은 갈림).

- **D-6′ (게이트 27 M-2 — 위 음성 대조 문면의 정정, 2026-08-20)** —
  음성 대조: **`SUPPORT_DTD` 를 빼면** billion laughs(UTF-8·UTF-16) fixture 가 통과한다.
  **나머지 두 속성은 구조 단언뿐이고 행동 음성 대조가 성립하지 않는다** —
  `SUPPORT_DTD=false` 가 파서 수준에서 DOCTYPE 을 먼저 끊으므로 뒤쪽 둘을 빼도 fixture 는
  여전히 거부된다.
  **실측(2026-08-20, 이 레인이 직접)**: `SecureXml.newInputFactory()` 에서
  `IS_SUPPORTING_EXTERNAL_ENTITIES` 와 `ACCESS_EXTERNAL_DTD` 두 줄을 **빼고**
  `HwpxExtractorTest` 전건을 돌렸다 → `tests 8 · failures 0 · errors 0`(UTF-8 폭탄 · UTF-16
  폭탄 · XXE 케이스 모두 그대로 초록). 즉 옛 문면이 약속한 "하나라도 빼면 통과한다"는
  **재지 않은 초록**을 약속하고 있었다. `SecureXml.kt` 는 sha256 대조로 원상 복구했다.
  **코드와 테스트는 이미 정직했다** — `IngestDefensesTest.StAX 속성이 명시돼 있다` 는 세
  속성을 **구조로만** 단언하고 행동을 주장하지 않는다. 거짓은 이 계획 문면 하나였다.
  **D-d 와 같은 형태다**(앞쪽 방어가 뒤쪽 방어의 행동 음성 대조를 가린다). 같은 구조가
  문서에서 재발한 것이므로 원문을 지우지 않고 무엇이 왜 틀렸는지 남긴다.
...
## 9.2-quater 게이트 27 조치에서 이 계획을 고친 지점 (2026-08-20)

> 같은 규율이다 — **계획을 먼저 고치고 사유를 적은 뒤** 코드가 들어갔다.
> 정본은 `reviews/04_documents_cross.md`(교차 종합)이고, 판정은 리더가 내렸다.

| # | 계획이 적었던 것 | 실제로 한 것 | 사유 |
|---|---|---|---|
| **D-o** | **D-17** — *"파일명을 저장하지 않는다. 제목은 사용자 제공 또는 **본문 첫 줄** 유도"* | **본문 유도를 없앴다.** 제목의 바탕은 ⑴ 사용자가 준 제목 ⑵ ~~**업로드 파일 이름**~~ ⑶ `제목 없음` 뿐이다. **(⑵ 는 2026-08-20 재판정으로 취소됐다 — §9.2-quinquies D-o′)** `AUTO_TITLE_TARGET_LENGTH`·`TITLE_ELLIPSIS`·`shortenDerivedTitle` 은 **삭제**했다 | **codex C-1(Critical ①) + 2026-08-20 사용자 결정.** `documents.title` 은 평문 컬럼이고 업로드 시점에는 마스킹이 돌지 않는다(마스킹은 워커의 일). 본문 첫 줄을 30자로 잘라 넣는 갈래는 **암호화와 마스킹 두 방어를 동시에 우회**했다. 안 쓰게 된 상수·함수를 남기면 다음 사람이 다시 부르므로 지웠다 |
| ~~**D-o-1**~~ **(취소 — §9.2-quinquies)** | 같은 D-17 — *"파일명 자체가 개인정보일 수 있다"* | **파일 이름을 평문 `title` 로 저장한다** | 사용자 결정이다. 근거는 **본문과 이름의 성격 차이** — 본문은 맡긴 내용이고 파일 이름은 사용자가 붙여 건넨 이름이라 「적어 준 제목」과 같은 부류다. **⚠ 이 결정은 두 문서와 충돌한다** — 계약 `DocumentTextRequest.title`(`contracts/easy-doc-v1.yaml:1848-1850`)의 *"생략하면 본문 첫 줄에서 유도한다 … **파일명은 제목으로 쓰지 않는다**"*, `DocumentFileRequest.title`(`:1872`)의 *"생략하면 추출한 본문 첫 줄에서 유도한다"*, 그리고 `migration-safety-gate` SKILL.md `:63` 의 **보장** 문장 *"파일명은 아예 저장하지 않는다"*. **이 레인은 그 두 파일을 고치지 않았다** — 계약은 `contract-keeper`, 게이트 문장은 `privacy-gate` 몫이다. 남는 위험(`홍길동_주민등록등본.docx` 가 평문으로 남는다)은 `TitleRules.kt` 머리 주석에 적었다 |
| ~~**D-o-2**~~ **(취소 — §9.2-quinquies)** | — | 파일 이름의 **정의역을 정했다**: 마지막 경로 조각만(`/`·`\` 둘 다) · 마지막 확장자 제거(맨 앞 점은 제외) · 제어문자 제거 · 255 코드 포인트에서 자르기 · 남는 것이 없으면 `제목 없음` | 파일 이름도 사용자 입력이라 정의역 판정이 필요하다(리더 지시). 경로를 그대로 저장하면 **로컬 디렉터리 구조**(보통 계정 이름을 포함)가 평문으로 남는다. 확장자는 `source_format` 이 이미 든다 |
| ~~**D-o-3**~~ **(취소 — §9.2-quinquies)** | — | **컨트롤러 배선은 열지 않았다.** `DocumentService.store` 가 `filename` 파라미터를 받고 `createFromFile` 이 그것을 넘긴다. multipart 파트의 실제 `filename` 을 `createFromFile` 에 넣는 일은 **C3** 몫이다 | 리더 지시. 파라미터를 지금 열어 두지 않으면 C3 이 유스케이스를 다시 열어야 한다 |
| **D-p** | §5 **D-4** — 추출 길이 상한을 「누적 중에」 | **한 층 위에도 걸었다.** `BlockSink.ensureRoomFor` 를 신설하고 `HwpxExtractor.SectionBlocks`·`DocxExtractor.elementBlocks` 가 문자를 덧붙이기 **전에** 예산을 묻는다. 두 파서 모두 블록 목록을 먼저 만들지 않고 sink 로 흘려보낸다 | **codex C-7.** `ExtractedTextBuilder` 는 줄마다 검사해 정직했지만, 파서가 구역/문서 전체를 모은 **뒤에야** 그것을 불렀다 — `ExtractedTextBuilder` KDoc 이 **원본의 결함으로 기술한 바로 그 형태**가 한 층 위에 남아 있었다. **갈리는 자리**: 확정 결과는 다듬은 값이고 조립 중 조각은 다듬기 전이라, **공백만 잔뜩 든 거대한 문단**은 예전 판이 받던 것을 지금 거절한다(fail-closed 방향) |
| **D-p-1** | — | 대조용 `DocxExtractor.blocks()` 도 **같은 예산**을 지는 `BlockList` 로 모은다 | 예산 없는 sink 를 하나라도 두면 그것이 제품 코드 안의 무제한 통로가 되고, 그런 통로는 언제나 「대조 전용」이라는 이름으로 시작한다 |
| **D-q** | — | `HwpxExtractor` 의 `createXMLStreamReader` 를 **`try` 안으로** 옮겼다 | **codex C-4/C-9.** 리더 생성은 잘못된 인코딩 선언·잘린 BOM 에서 `XMLStreamException` 을 던지는데, `try` 밖이라 그 예외가 그대로 올라갔다 — 계약이 못박은 422 대신 500 이 나가고 라이브러리 메시지가 로그 규약을 우회한다. **이 회차 지시 목록에는 없었으나** 같은 메서드를 재작성하는 중이었고 교차 종합 §11 이 이 레인으로 보낸 항목(행 4)이라 함께 고쳤다 |
| **D-r** | §9.2 **D-d** — POI 방어는 「구조 단언 + 기록된 한계」 | **음성 대조가 성립하는 축을 하나 만들었다.** `IngestDefensesTest` 가 전역 값을 일부러 어긋뜨린 뒤 `IngestConfiguration().documentTextExtractor()` **만** 부르고 값이 돌아오는지 본다. `@BeforeEach`/`@AfterEach` 로 원값을 복원한다 | **codex C-10.** 이전 판은 테스트 첫 줄에서 `PoiZipDefenses.apply()` 를 **스스로** 불러 제품 배선을 재지 않았고, JVM 전역 static 을 복원하지도 않았다. D-d 가 적은 한계(「앞쪽 방어가 가린다」)는 실재하지만 **다른 것**을 덮고 있었다 — 그 한계는 「폭탄이 막히는가」축의 것이고, 「조립이 값을 설치하는가」축은 잴 수 있었다 |
| **D-s** | — | `StatementCountingPremiseTest` 에 **하한**(`MIN_PORT_ADAPTERS = 12`)과 **바닥 목록**(`KNOWN_PORT_ADAPTERS`)을 두고, 포트 판정을 **재귀 인터페이스 폐쇄**로, 금지 손잡이를 **할당 가능성**으로 바꿨다 | **M-4 + codex C-6.** `isNotEmpty()` 만 요구해 **크롤이 1로 줄어도 통과**했다. 같은 저장소의 `SensitiveToStringReachTest` 는 하한과 바닥 목록을 둘 다 갖는데 이 자리만 빠져 있었다. 재귀 폐쇄·할당 가능성은 codex 가 함께 지목한 같은 행(행 6)이고 오늘 분모를 바꾸지 않는다(합성 probe 로 판정 함수만 실행으로 확인) |
| **D-t** | §5 **D-13** — P-26(계약에서 읽어 케이스 유도)은 **C3 몫**, 그때까지는 개선 백로그 **B-9** | **지금 세웠다.** `api` 모듈에 `UploadFormatContractTest` 를 두고 `ContractSpec` 으로 `x-input-limits.supported_upload_formats` 를 **읽어** `SourceFormat.UPLOAD_FORMATS` 와 대조한다 | **M-1.** 미구축인데 `SourceFormat` KDoc 과 `DocumentExtractorsTest` 의 `@DisplayName` 이 **있다고 적고 있었다**(전수 grep: `supported_upload_formats` 를 읽는 코드·테스트 0건). 그리고 계획이 요구한 장치가 「선택적 개선」으로 재분류돼 있었다 — 그렇게 옮기면 그 미구축은 영영 닫히지 않는다. `ContractSpec` 이 `api` 테스트에만 있어 `infrastructure` 가 아니라 `api` 에 두었다(파서 배선을 둘로 만들지 않는다). **N-26(계약 원소 제거 → 검사 축소)은 실행하지 않았다** — 계약 파일을 고쳐야 하고 이 레인은 그것을 하지 않는다. 대신 「없는 경로면 실패한다」를 실행으로 확인해 기대값의 출처를 고정했다 |
| **D-u** | — | `tests/test_kotlin_gate_reach.py` 에 **하한**(`MIN_TEST_CLASSES = 85`)과 **바닥 목록**(`FLOOR_TEST_CLASSES` 18건)을 두고, 실패 안내문에서 **축소 경로 안내를 걷어냈다** | **codex C-5.** `TEST_CLASS_COUNT` 는 목록과 **서로** 맞는지만 보는 상수라 탐지기 파일·목록·개수를 **함께** 줄이면 전부 통과했다. 게다가 안내문이 그 경로를 그대로 안내했다(*"파일을 지웠다면 … 도 함께 고쳐야 하고"*). 하한 85 의 근거는 실측이다 — 게이트 27 대상 리비전 `6515548` 이 가졌던 수 |

### 실행한 음성 대조 (전건 실측, 2026-08-20)

| 대조 대상 | 되살린 결함 | 결과 |
|---|---|---|
| **D-o** (평문 제목) | `resolveTitle` 을 본문 첫 줄 30자 유도로 되돌림 | **빨강 4건** — `JdbcDocumentStoreTest.본문 표식이 평문 열에 남지 않는다` 가 `[title]` 을 지목했고, `DocumentServiceTest` 3건이 함께 빨강 |
| **D-p** (추출 예산) | `SectionBlocks` 를 목록 누적 + 사후 넘김으로 되돌림 | **빨강** — 기대 `문서가 너무 깁니다` ↔ 실제 `파일이 손상되었습니다`(파서가 깨진 꼬리까지 읽었다는 관측) |
| **D-q** (리더 생성) | `createXMLStreamReader` 를 `try` 밖으로 | **빨강** — `리더 생성 예외가 도메인 예외로 바뀌지 않았다` |
| **D-r** (POI) | `IngestConfiguration` 의 `PoiZipDefenses.apply()` 삭제 | **빨강** — `조립이 maxEntrySize 를 설치하지 않았다` |
| **D-u** (게이트 핀) | 선언·개수를 **함께** 축소(88→86, 바닥 2건 제거) | **빨강** — 바닥 목록 축이 `SensitiveToStringReachTest` 누락을 지목. 이어 80 까지 줄이자 하한 축도 빨강(`80 >= 85` 실패) |
| **D-6′** (StAX) | `IS_SUPPORTING_EXTERNAL_ENTITIES`·`ACCESS_EXTERNAL_DTD` 제거 | **초록 유지**(`tests 8 · failures 0`) — 옛 문면이 거짓임을 확인한 대조다 |

복원은 전부 **sha256 대조**로 확인했다. 다만 한 자리에서 절차 사고가 있었다 —
`tests/test_kotlin_gate_reach.py` 를 `git checkout --` 로 되돌렸는데 그 파일의 수정이
**아직 커밋되지 않은 상태**라 이번 회차 작업분이 함께 사라졌다(재작성해 복구). **교훈**:
「복원은 git 경유」 규칙은 **커밋된 기준선**을 전제한다. 미커밋 작업 위에서 음성 대조를 할
때는 **먼저 커밋하고** 나서 되살리거나, 역패치 + sha256 대조로 되돌려야 한다.

### 이 회차가 하지 **않은** 것

- `contracts/easy-doc-v1.yaml` 수정 — 하지 않았다. 필요한 개정은 §11 에 적어 `contract-keeper` 로 올린다.
- `migration-safety-gate` SKILL.md `:63` 수정 — 하지 않았다. `privacy-gate` 판정 대상이다.
- 교차 종합 **M-3**(`lockSourceText`/`lockEnvelope` 가 `ownerId` 를 받지 않는다) — 마감이 **C6** 이고 `privacy-gate` 판정 대기다.
- **CR-1**(원장 `00_progress.md` 문언) — 리더가 고친다. 원장을 건드리지 않았다.
- 표 18 TRACE(J-1) · §9 질문 ⑦ — 리더 판정 대기.
- 교차 종합 Minor 8건(행 16~23) 중 이번 지시에 없는 것 — 손대지 않았다.

### 계약 레인(`contract-keeper`)에 올릴 조항

1. **`DocumentTextRequest.title`**(`:1848-1850`) — *"생략하면 본문 첫 줄에서 유도한다(30자 목표, 어절 경계에서 자르고 `…` 부착)"* 를 **"생략하면 `제목 없음`"** 으로. 같은 절의 *"파일명은 제목으로 쓰지 않는다 — 파일명 자체가 개인정보일 수 있다"* 는 **파일 모드에 한해 뒤집혔다**(붙여넣기 모드에는 파일 이름이 없으므로 이 절에서는 문장을 지우는 것이 아니라 근거를 옮겨야 한다).
2. **`DocumentFileRequest.title`**(`:1872`) — *"생략하면 추출한 본문 첫 줄에서 유도한다"* 를 **"생략하면 파일 이름에서 경로와 확장자를 뗀 값. 남는 것이 없으면 `제목 없음`"** 으로.
3. **`POST /documents` description** — 제목 유도 규칙을 적는 자리가 필요하면 여기다. 30자·어절 경계·`…` 는 **더 이상 구현에 없다.**
4. (기존 대기) 큐 등록의 **순서 문장**·`'502'` 응답 선언·`EnqueueFailed` 세 자리 — 교차 종합 행 2.
5. (기존 대기) HWPX 리더 생성 실패의 응답 코드 — 이제 **422**로 정화된다(행 4). 계약이 500 을 허용하는 문장을 갖고 있지 않은지 확인이 필요하다.

---

## 9.2-quinquies 파일 이름 갈래를 **되돌린다** (2026-08-20 사용자 재판정)

> 같은 규율이다 — 계획을 먼저 고치고 사유를 적은 뒤 코드가 들어갔다.
> 판정 주체는 사용자이고, 리더가 계약 `:1848`·`:1850`·`:1872` 와
> `migration-safety-gate` I-4 를 직접 확인한 뒤 §9.2-quater 의 D-o-1~D-o-3 을 취소했다.

### 무엇이 뒤집혔나

§9.2-quater 는 리더 지시의 전제 — *"계약은 `title` 을 선택으로만 적고 생략 시 동작을
규정하지 않는다"* — 를 받고 그것이 **거짓임을 실측으로 지적**했다(§9.2-quater D-o-1 의
⚠ 문단, 그리고 `04_kotlin-implementer_gate27-fixes.md` §2). 그 지적이 받아들여졌다.
계약은 생략 시 동작을 **명시하고**, 파일 이름 사용을 **명시적으로 금지**한다.
...
| `migration-safety-gate` SKILL.md `:63` | **보장** — *"파일명은 아예 저장하지 않는다(파일명 자체가 개인정보일 수 있다)."* |

「본문과 파일 이름은 성격이 다르다」는 §9.2-quater D-o-1 의 근거는 **규정을 이기지 못한다.**
실질 위험도 같은 종류였다 — `홍길동_주민등록등본.docx` 는 이름만으로 *누가 어떤 서류를
올렸는지*를 평문 열에 남기고, 경로·확장자·제어문자를 떼도 남는 것이 바로 그 부분이다.

### 고친 것

| # | §9.2-quater 가 적었던 것 | 이번에 한 것 | 사유 |
|---|---|---|---|
| **D-o′** | 제목의 바탕 ⑵ = 업로드 파일 이름 | **⑵ 를 없앴다.** 바탕은 「사용자가 제목 칸에 적어 준 값」 하나뿐이고, 없으면 `제목 없음` 이다. 두 모드(붙여넣기·파일)가 같다 | 계약 `:1850` · 게이트 I-4. 본문 유도 제거(C-1)는 **그대로 유지**한다 |
| **D-o-1′** | ~~파일 이름을 평문 `title` 로 저장한다~~ | **취소.** `resolveTitle` 의 시그니처를 `resolveTitle(given: String?)` 로 되돌렸다 | 같은 위 |
| **D-o-2′** | ~~파일 이름의 정의역(경로·확장자·제어문자·255자)~~ | **취소.** `baseNameOf` 를 삭제했다 | 판정할 값이 서비스까지 오지 않으므로 정의역 자체가 불필요하다 |
| **D-o-3′** | ~~`store` 가 `filename` 을 받는 통로를 C3 을 위해 미리 열었다~~ | **취소.** `TitleSources` 를 지우고 `store(…, givenTitle: String?, …)` 로 되돌렸다. `createFromFile` 의 `filename` 은 `extractor.extract` 까지만 흐른다 | **통로가 없으면 되살리는 편집이 시그니처에 드러난다.** C3 이 열 것도 없다 — 계약이 금지한 값이다 |
| **D-o-4′** | — | **파일 이름 표식 탐지기를 신설했다.** `JdbcDocumentStoreTest.파일 이름 표식이 평문 열에 남지 않는다` — 표식을 파일 이름 맨 앞에 두고 업로드해 `documents` 행의 **어느 평문 열에도** 없음을 확인한다. 분모·가드는 본문 표식과 공유한다(`assertNoMarkerInDocumentRow`) | I-4 의 *"파일명은 아예 저장하지 않는다"* 를 **문장이 아니라 실행**으로 세운다. 본문 표식만으로는 부족하다는 것을 음성 대조 ⒝ 가 실측으로 보였다(아래) |
| **D-o-5′** | — | `TitleRulesTest` 에 **시그니처 핀**(`private val signaturePin: (String?) -> String = ::resolveTitle`)을 두었다 | 순수 함수 층에서 잴 수 있는 유일한 축이 「받을 자리가 몇 개인가」다. 본문이든 파일 이름이든 되살리려면 인자가 늘고, 그 순간 컴파일이 깨진다. `core` 는 Spring 없이 도는 모듈이라 `kotlin-reflect` 를 전제하지 않으므로 반사 대신 타입으로 잰다 |

### 실행한 음성 대조 (2026-08-20, 전건 실측)
...
**두 대조가 함께 보인 것**(이것이 탐지기를 하나 더 세운 이유다): ⒝ 에서 **본문 표식
탐지기는 초록으로 남았다.** 즉 §9.2-quater 가 세운 탐지기 하나만으로는 파일 이름 유입을
잡지 못한다 — 새어 나간 것이 본문이 아니기 때문이다. 반대로 ⒜ 에서 파일 이름 탐지기도
초록이었다. **두 축은 서로를 대신하지 못한다.**

복원은 **바이트 백업 → `Path.write_bytes` → sha256 대조**로 했다(둘 다
`a3982ac9…b514029` 일치). §9.2-quater 회차에서 난 절차 사고(미커밋 작업 위에서
`git checkout --`)를 되풀이하지 않았다 — 규칙 5 가 이미 이 절차를 갖고 있었다.

### 계약 레인에 올릴 조항 — **위 §「계약 레인(`contract-keeper`)에 올릴 조항」의 1·2 는 폐기한다**

리더가 `contract-keeper` 에 **직접** 내려보낸 개정이 정본이다(`:1848`·`:1872` → 생략 시
`제목 없음`, `:1850` 의 파일명 금지 문장은 **유지**). 이 레인은 계약 파일을 고치지 않는다.

### D-o-6′ 계약 대조 테스트를 **지금 세웠다** — 빈자리를 백로그로 미루지 않는다

착수 시점 실측: 계약 파일을 읽는 테스트는 `ContractSpec` 경유 **17개 파일**이었고, 그중
**`title` 관련 조항을 읽는 것은 0건**이었다(전수 grep — `max_title_length` 는
`DocumentLimits.kt`·`TitleRules.kt`·`TitleRulesTest.kt` 에 **산문으로만** 나오고 값은 손으로
적혀 있었다). 즉 코드와 계약이 서로 다른 것을 정본이라 주장해도 **아무것도 빨개지지
않는 상태**였고, 이번에 실제로 그 상태가 며칠 있었다.

작업 중 계약 레인이 `5e751f8` 로 개정을 커밋했고, 그 개정은 `x-title-policy` 절을 신설해
값을 **기계가 읽을 수 있게** 두었다 — 그리고 주석으로 자리를 지정했다:
*"고정 문구. 계약 테스트는 이 값을 코드에 복제하지 말고 여기서 읽는다."*

그래서 `api` 모듈에 **`TitlePolicyContractTest`** 를 세웠다(`UploadFormatContractTest`(D-t)와
같은 형태·같은 모듈 — YAML 파서 배선을 둘로 만들지 않는다). 다섯 케이스가 잰다.
...
| `x-title-policy.sources` == `[given_title]` | 계약이 허용 출처를 늘리면 드러난다 |
| `x-title-policy.forbidden_sources` == 탐지기 표의 키 집합 | **금지마다 실행 축이 있는가.** 계약에 금지가 하나 늘면 "그것을 재는 탐지기가 없다"가 빨개진다 |
| 없는 경로면 **실패한다** | 기대값의 출처 고정(0건·기본값으로 조용히 통과하는 형태 배제) |

넷째가 이 파일의 중심이다. 금지를 계약에만 적으면 그것은 **문장이지 게이트가 아니다** —
표가 `body_text` → 「본문 표식」 탐지기, `upload_filename` → 「파일 이름 표식」 탐지기로
짝지어 두어, 계약과 실행 축이 갈리는 순간을 드러낸다.

**N-26 형태(계약에서 원소를 빼면 검사가 줄어드는가)는 이번에도 실행하지 않았다** — 계약
파일을 고쳐야 하고 이 레인은 그것을 하지 않는다(게다가 계약 레인이 같은 시각 그 파일을
쓰고 있었다). 대신 다섯째 케이스가 기대값의 출처를 실행으로 고정한다.
