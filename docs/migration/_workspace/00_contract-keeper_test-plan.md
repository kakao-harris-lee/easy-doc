...
|---|---|---|---|---|---|---|---|
| 1 | `POST /auth/signup` | 201 + no-store/nosniff + `{id,email}` | — (인증 불필요) | — | 이메일 형식 · **이메일 상한 초과(X-F13)** · **비밀번호 하한 미만(X-F11)** → 전부 **문자열 detail** / 필드 누락 → **배열 detail** | **409** 이메일 중복 | `paths./auth/signup`, `x-private-response-headers`, `x-request-field-constraints.fields[0]`·`[1]` |
...
| **X-A2** | **자격증명이 제시된 갈래들** — 없는 이메일 · 비밀번호 불일치 · 위조 토큰(서명 불일치·필수 클레임 누락·`typ` 용도 불일치 포함) · 만료 토큰 · 삭제된 계정 → **모두 401**, 응답 본문이 실패 사유를 구분하지 않는다. **`Authorization` 헤더가 아예 없는 갈래는 이 균일성의 대상이 아니다** — 무헤더를 함께 넣은 비교(4갈래 응답 시간 비 등)는 이 조항의 측정이 아니다 | C | `x-auth.failure_uniformity` (**정본** — 범위 절 포함) | 구분하면 계정 열거 공격의 단서가 된다. 무헤더는 요청자가 스스로 만든 상태라 응답에서 얻는 정보 이득이 0이고, `components/responses/Unauthorized`가 적는 **두 문구 구조의 첫째 갈래**로 남는다 |
...
| **X-C9** | 컨테이너 생성 응답 **6종**(X-D2c)의 **본문 모양** — ⚠️ **5종 미측정.** 2026-08-12 실측은 헤더만 봤다. **2026-08-19 현행화(M-405)**: 대상이 7 → 6이 됐고(405는 ④가 아니라 ①이다) 그중 **1종**(콜론 없는 헤더 줄 400)의 본문이 최상위 키 `detail` 하나로 관측됐다 | C | `x-error-body-universality.x-unmeasured` (E-4) | **1종 측정으로 E-4를 닫지 않는다.** 남은 5종을 "충족"으로도 "위반"으로도 적지 않는다. 만족시킬 수 없는 자리가 남으면 계약을 좁히지 말고 리더 재심 — 헤더 쪽에서 밟은 순서와 같다 |
...
| **X-D2c** | **컨테이너 레벨 응답에도 두 헤더가 있다** — ✅ **2026-08-12 실측 완료.** 필터에 닿는 것: 핸들러 없는 404·415·413·프리플라이트 OPTIONS 200·`sendError`→`/error` 503·**알 수 없는 메서드 405**(2026-08-19 M-405 재분류). **필터에 못 닿는 것 6종**(요청 대상 금지 문자 400 · 콜론 없는 헤더 줄 400 · 요청 줄 파손 400 · 헤더 상한 초과 400 · `Host` 없음 400 · 알 수 없는 버전 505)은 **Tomcat Engine 밸브**가 덮는다. 회귀는 `@SpringBootTest(RANDOM_PORT)` + **원시 소켓**으로 유지 — **MockMvc로 옮기면 근거가 사라진다**(측정한 것처럼 보이는 통과가 나온다) | C | `x-global-response-headers.x-phase3-measurement`, `.enforcement`, `.x-405-reclassification` | 실패 양상이 "누가 빠뜨렸는가"에서 **"강제 수단이 닿는가"**로 옮겨간 자리. **재 보니 실제로 비어 있었다** — 계약을 좁히지 않고 강제 수단을 넓혀 해결(리더 재심 불필요). **2026-08-19**: 미도달 목록의 한 항목이 사실과 달라 정정됐다(G1). 이제 **열거자 6 = 계약 6**이라 열거자↔계약 대조를 빌드에 넣을 수 있다 |
...
| **X-F11** | **`SignupRequest.password` 하한 — 하한 미만 → 422이고 `detail`이 문자열 타입이다**(배열이 아님을 명시 단언) / 하한과 정확히 같은 길이 → 통과 (**경계값 양쪽**). 하한 값은 `fields[1].limit`에서 읽는다 | C | `x-request-field-constraints.fields[1]`(`:392-396`) · `SignupRequest.password.x-service-constraint`(`:1674`) · `components/responses/ValidationFailed`(`:1516-1518`) | **게이트 15 X5 신설.** F3 판정 뒤에도 `password`를 덮는 행이 이 표에 **하나도 없었다** — 하한을 아예 구현하지 않아도, `@Size(min=…)`로 구현해 배열 `detail`을 내도 전건 통과한다. 계약은 그 구현을 **계약 위반**이라 부르고 "contract test가 `detail`의 **타입까지** 단언한다"고 스스로 적어 두었는데(`:1516-1518`) 그 단언의 자리가 없었다. `password`는 X-F9(정규화 후 3필드)에도 X-F1(`text`)에도 들어가지 않아 **다른 어느 항목도 대신 덮지 못한다** |
| **X-F12** | **원시 측정의 반대 축** — 앞을 공백으로 채워 **원시 길이는 하한과 같고 trim 후에는 하한 미만**인 비밀번호가 **통과한다**. 하한 값은 `fields[1].limit`에서 읽고, 공백 개수는 그 값에서 유도한다 | C | `fields[1].measured_on`(`:395`) · `x-service-constraint.measured_on: raw`(`:1674`) | **게이트 15 X5 신설.** X-F9는 *정규화 후*를 재는 3필드를 고정하고 이 항목은 *원시*를 재는 필드를 고정한다. **한쪽만 있으면 구현이 다섯 필드에 같은 측정 기준을 일괄 적용해도 아무 데서도 안 걸린다** — 자리마다 실제 강제 지점을 확인해 갈라 놓은 F3의 판정(`x-contrast-case` `:377-384`)이 통째로 무너지는 경로다. ⚠️ 이 행의 기대값은 **게이트 15 X13(리더 판정 대기)에 종속**된다 — `measured_on`이 바뀌면 이 행도 함께 바뀐다. **바뀌기 전까지는 현행 조항이 기준이다** |
| **X-F13** | **`SignupRequest.email` 상한의 거절 쪽** — 정규화 후 상한 초과 → 422 + **문자열** `detail` / 정규화 후 정확히 상한 → 통과. 길이 위반의 `detail`이 **형식 오류의 `detail`과 같은 문자열**임도 함께 단언한다. 상한 값은 `fields[0].limit`에서 읽는다 | C | `fields[0]`(`:387-391`) — 특히 `detail`의 인라인 주석(`:391`)이 두 위반에 같은 문구를 쓰는 이유를 적는다 · `SignupRequest.email`(`:1663-1670`) | **게이트 15 X5 · 1회차 C-B.** 현재 `email` 상한은 **통과 쪽만**(X-F9의 원시→정규화 케이스) 덮여 있어 **상한을 아예 구현하지 않아도 전건 통과한다.** 길이 전용 문구를 따로 신설하는 구현도 계약 위반이다 — 계약이 두 위반에 같은 문구를 지정한 것 자체가 조항이므로 **문구의 동일성까지** 본다 |
...
> **X-A2 행은 2026-08-19에 정정됐다 (게이트 24 R-1의 문서 드리프트 잔재).** 종전 문면은
> 「헤더 누락」을 균일화 열거의 첫 원소로 두고 있었고, 그것은 `x-auth.failure_uniformity`가
> 범위 절을 갖기 **전**의 넓은 문면에서 온 잔재다. 계약 쪽은 `dec3124`에서 정합됐으나 이
> 행은 그 커밋 범위 밖이라 남아 있었다(같은 커밋의 `affected_tests`가 "별도 문서 커밋에서
> 정정한다"로 예고한 1건이 이것이다).
>
> **강제되는 동작은 이 정정 전후로 달라지지 않는다** — 실제 케이스(`03_contract-keeper_auth-test-spec.md`
> L-3·M-3)는 이미 두 갈래로 나눠 재고 있었고 `AuthenticationWorkUniformityTest`도 토큰 제시
> 갈래만 묶는다. 바뀐 것은 **이 표가 정본을 어떻게 인용하는가**뿐이다. 그래도 고치는 이유는,
> 인용이 정본보다 넓으면 다음 사람이 이 행을 근거로 무헤더를 균일성 측정에 넣기 때문이다 —
> 실제로 4갈래 시간 비가 그렇게 만들어진 적이 있다.
...
> 비교하는 형태로 쓴다.
>
> **이 규약이 다스리는 범위 (2026-08-18 명시)** — **X-F11·X-F12·X-F13 세 행의 모든 열**과
> 위 커버리지 표, 그리고 §2 엔드포인트 표의 #1 행 ④ 칸. 「고정하는 계약 조항」 열만이 아니다.
>
> **초판(`526bfeb`)은 이 범위에 도달하지 못했다** — 게이트 16 N(1회차 C-C). 세 행의 서술
> 열과 #1 행이 상한 값(`8`·`255`)과 `detail` 인라인 주석 전문을 그대로 옮겨 적고 있었다.
> 규약을 만든 계기가 「별건 1 — 손으로 옮긴 계약 요약이 원본과 갈렸다」인데 **그 규약이 자기
> 세 행에서 빈 채로 통과했다**(CLAUDE.md 규칙 4의 범위 선언형). 값이 틀려서가 아니라 장치가
> 빈 선언이었기 때문에 고친다. 지금은 경계를 「하한/상한」이라는 **역할 이름**으로 적고 값은
> `fields[n].limit`에서 읽게 한다.
>
> **넓히지 않는다 — 이 규약은 위 범위 밖의 기존 행에 적용되지 않는다.** X-F1·X-F2·X-F9·X-F10과
> §2 표의 다른 행은 이 규약보다 먼저 쓰였고 여전히 상한 값·`detail` 문구를 전사한다. 그것을
> 여기서 함께 고치면 규약이 근거보다 넓어진다(규칙 4). **사실만 등재한다** — 기존 행의 전사는
> 미해소이며, 어디까지가 규약 안인지 몰라 전체가 준수 중이라고 읽는 것이 이 등재가 막으려는
> 오독이다.
...
> | **X-F9** | **부분** — `email`·`name` | `AuthContractTest` S-5 · `WorkspaceContractTest` WC-7(+WC-8). `edited_text` 는 Phase 4 |
...
> | **X-A3 · X-B1 · X-B2** | **실행 소스 실재** (2026-08-19 작업 공간 단위에서 마감) | `WorkspaceEndpointReachTest` — X-A3 는 **두 축**(WC-12 본문 · WR-8 경로 변수), X-B1 은 WR-3·WD-2(`403 아님`을 명시 단언), X-B2 는 WR-4·WD-3 |
> | #10~#13(작업 공간) · X-F10 | **실행 소스 실재** | `WorkspaceContractTest`(C-M) · `WorkspaceEndpointReachTest`(C-R·C-I) · `AuthUnavailableContractTest`(WL-7). 명세 36케이스 전건 대응 — X-F10 은 WC-4 |
>
