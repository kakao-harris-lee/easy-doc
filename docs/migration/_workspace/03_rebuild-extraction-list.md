...
2. **폐기 게이트(delete-gate)** — Python 코드 삭제(계획 §5 Phase 8)에 붙는 선행 조건. 이것이 재개발 계획에서 Phase 8이 "결과"가 아니라 **선행 조건이 붙은 단계**인 이유다.

> **2026-08-12 개정 — 차단 조건을 P1+P2로 교체(리더 승인).** 1차본의 게이트("A~M 전 항목이 명세·fixture로 옮겨져야 삭제 가능")는 **과했다.** 사용자 결정이 *"개발 요구사항과 스펙이 존재한다면 굳이 파이썬 코드를 보고 확인할 필요가 없다"*이므로, **이미 명세된 것**과 **데이터 자산**에까지 전사 의무를 걸 이유가 없다. 전사가 아니라 **검증된 재구축**이 판단을 체화한다. 새 차단 조건은 아래 **P1(데이터 반출) + P2(검증된 재구축 + 알려진 명세 누락 0)** 둘뿐이며, 전문(全文) 목록 §A~§M은 **차단 조건이 아니라 재구축 시 참고 자료**로 강등된다. 새 게이트는 이 문서 끝 「폐기 게이트 (Phase 8 선행 조건)」에 있다.
...
- **실행 명세로서의 테스트.** 아래 여러 항목은 `tests/easyread/*`·`tests/golden/*`가 큐레이션 규칙을 **기계 검증**한다. 가장 값싼 충실한 추출은 종종 "같은 fixture로 Kotlin 테스트를 다시 세우는 것"이다. **단 이 전사는 차단 조건이 아니다**(2026-08-12 개정) — Kotlin 재구축이 §6 게이트를 실제로 통과하면 같은 불변식이 다시 선다. 차단되는 것은 테스트 파일 안에 **데이터**가 사는 경우뿐이며(`converted_samples.py`·`tests/golden/documents/`·`tests/ingest/fixtures/`), 그것은 전사가 아니라 **반출(copy-out)**로 닫는다 — P1.
...
| `DIFFICULT_WORD_REPLACEMENTS`(246키→뜻풀이) | 사전 자산 | **재도출 대상 아님 + 삭제 전 반출(copy-out) 대상 — P1.** "이전 대상 아님"으로만 적으면 Phase 8이 같이 지운다 |
...
## 폐기 게이트 (Phase 8 선행 조건) — P1 + P2

> **2026-08-12 개정(리더 승인).** 1차본의 "A~M 전 항목 전사" 게이트를 **P1+P2로 교체**했다. 옛 게이트는 **이미 명세된 것**(마스킹·인증·추출 보안·채점 게이트)과 **데이터 자산**에까지 전사 의무를 걸어 과했다. 차단하는 축은 **P1(데이터 반출)** 과 **P2(검증된 재구축)** 둘이고, P2에는 같은 날 codex stop-time 게이트 지적으로 **선행조건 하나**(알려진 명세 누락 0)가 붙었다. 그 밖에는 아무것도 차단하지 않는다.

### P1 — 데이터 반출 (유일한 '영구 손실' 위험) — **차단**

`app/**`·`tests/**` 삭제 전, 그 안에 사는 **큐레이션 데이터**를 생존 위치(`parity/fixtures/`·Kotlin test resources·`data/` 등 삭제 구역 밖)로 복사한다. 유한 목록이고, 이 목록이 닫히면 P1은 닫힌다.

**왜 이것만 영구 손실인가.** 치환 비문 워드리스트(`COMPOUND_HEAD_NOUNS` ≈53 + `LEXICALIZED_GLOSSES` 26 + `COMPOUND_TAIL_KEYS` 3)는 미션 GG 실측 튜닝 3커밋(`eae75c7`·`0894854`·`a4c9fd9`)의 산출물이고 **각 낱말이 오탐 경계**다. 보존 회귀 코퍼스 6건이 전부 **단일 문서 020 파생**이라 **대다수 낱말을 코퍼스가 건드리지 않아** 재튜닝으로 복원되지 않는다. 보존 비용 ≈ 0(이미 기계가독 리스트), 재도출 비용 = 고. 나머지 데이터도 성질이 같다 — 사람이 큐레이션했고 코드에서 유도되지 않는다.

**형식 규칙: 프로즈로 전사하지 마라.** 데이터는 **기계가독 파일**(JSON/YAML/원본 이진)로 복사하고, 이 문서와 인벤토리는 **참조만** 한다. 문서 안에 표로 옮겨 적는 것은 반출로 치지 않는다.

| # | 자산 | 확인된 물리 경로 | 규모(실측) | 삭제 구역? |
|---|---|---|---|---|
| P1-1 | 골든 코퍼스 | `tests/golden/documents/*.json` | 56 파일, `required_facts` 합 **253** | **예** (`tests/**`) |
| P1-2 | 골든 코퍼스 미아 파일 | `tests/golden/041-2026년_국민기초생활보장_사업안내.jso` | 10,199 B, 확장자가 `.json`이 아닌 **`.jso`**(오타), untracked. `documents/041-*.json`과 별개로 존재 | **예** — 반출 전 `documents/`판과 **중복인지 판정**한 뒤 반출 또는 폐기 |
| P1-3 | 치환 비문 회귀 코퍼스 | `tests/easyread/converted_samples.py` (301줄) | `CONVERTED_SAMPLES: tuple[tuple[str, str, tuple[str, ...]], ...]` — (이름, 변환 결과 본문, 그 본문에서 검출돼야 하는 뜻풀이 전부) **6샘플** + 기대집합 + 3개 제외구 근거 | **예** |
...
| 심볼 | 행 | 성격 |
|---|---|---|
| `DIFFICULT_WORD_REPLACEMENTS` (246키→뜻풀이) | 52 | 사전 자산 |
| `COMPOUND_HEAD_NOUNS` (≈53) | 503 | **오탐 경계** — "사전 키에서 유도 불가"로 코드가 명시 |
| `LEXICALIZED_GLOSSES` (26) | 454 | **오탐 경계** |
| `COMPOUND_TAIL_KEYS` {기한·기일·정액} | 484 | **오탐 경계** |
| `PROMPT_ONLY_WORDS` (상기·하기·게시·반려·하자) | 332 | 자동 채점 제외 집합 |
| `DOUBLE_PASSIVE_PATTERNS` (5) | 22 | 데이터 |
| `_COMMA_CHARS` (반각·전각·모점) | 19 | 데이터 |
| `STYLE_PRINCIPLES` (6) | 344 | 문안 = 프롬프트 소스 |
| 사전 **오탐 제외 낱말 목록**(인지·경우·미만·이내·이상·세대·초기·수급·당해·상이·연기·상한·휴대·노후·전원·검진·이행) + 값 감사 사유("적음"=동음이의, "줌"=한 글자) | 주석 27–51 | **주의 2건**: ① 이것만 주석(비-심볼)이라 반출 시 **낱말 리스트 + 사유 필드**를 갖는 기계가독 레코드로 만든다 ② 지시서가 "PII 제외 15낱말"로 불렀으나 실제로는 **PII와 무관한 '사전 오탐 제외' 낱말**이고, 주석에 열거된 것은 **17개**다(15는 부정확) |
| `NOMINAL_GLOSSES` (576) · `MODIFIER_CHECKED_GLOSSES` (585) | 576·585 | **파생값 — 반출 대상 아님.** 사전 + `LEXICALIZED_GLOSSES`에서 계산된다. 반출하면 원본과 갈릴 수 있으니 **파생 규칙만 재구축이 다시 세운다** |
| `GLOSS_COLLISION_PATTERNS` (598) | 598 | 정규식 = **알고리즘**이지 데이터가 아니다 — P1 아님, P2가 다룬다 |

> **P1-7 주 — spike fixture는 삭제 구역 안이 아니라 저장소 밖에 있다.** 지시서는 "삭제 구역이면 목록에 넣어라"였으나, 실측 결과 `app/**`·`tests/**` 어디에도 없다. `00_kotlin-implementer_doc-spike.md` §9가 *"합성 fixture·oracle JSON은 전부 스크래치패드에 만들었다"*고 기록한 그대로이며, 물리적으로는 위 `/private/tmp/...` 경로에 **아직 살아 있다**(확인함). 이는 삭제 구역보다 **불리한 위치**다 — Phase 8을 기다리지 않고 tmp 정리로 사라진다. 셋을 함께 기록한다: ① 이 fixture들이 덮는 것은 **저장소 fixture 6개에 없는 케이스**다 — SDT·`a:t`(도형)·`m:t`(수식)·다단 PDF·**선언 크기 위조 zip**, 즉 DOC-01 정확성과 I-10 zip 방어의 유일한 실증물이다 ② 생성기 `extra_fixtures.py`의 소스는 **doc-spike 문서에 실려 있지 않다**(문서에는 실행 명령 한 줄뿐) — 스크래치패드가 지워지면 재작성해야 한다 ③ 생성기가 `app.ingest.extractors`를 import해 oracle을 만들므로, **Python 삭제 후에는 oracle 재생성이 불가능**하다. 넷째로, 생성기가 쓰는 `forged_size.zip`은 현재 `extra/`에 **없다**(2파일만 잔존) — 위조 zip은 생성기 재실행으로만 되살아난다. **P1은 이 셋(이진 2 + 생성기)을 저장소 안 생존 위치로 옮기면 닫힌다.**

### P2 — 검증된 재구축 — **차단**

**도메인별로** 판정한다. Kotlin 재구축이 계획 §6 검증 매트릭스의 **Requirements 게이트 + 해당 도메인 게이트**를 **실제로 통과하면**, 그 도메인의 Python을 지운다. 재구축이 판단을 체화하고 게이트가 검증한다.

**"게이트가 존재한다"로는 부족하고 "실제로 통과"여야 한다.** 골든 게이트는 아직 돌지 않는다 — `tests/golden/baseline.json`이 **미기록**이고(§4.6 열린 항목 ⓐ) 실제 변환문 적용은 `-m llm` 레인 전용이다. 그래서 **프롬프트 도메인의 Python 삭제는 첫 baseline 기록 이후**로 순서를 못박는다. 이 조건이 없으면 Python을 지우고 새 프롬프트를 쓰는데 **어느 쪽도 품질을 제약하지 못하는 창**이 생긴다.

#### P2 선행조건 — 알려진 명세 누락 0 (2026-08-12 추가, codex stop-time 게이트 지적 · 리더 인정)

> **지적:** *"P1/P2가 알려진 명세 누락을 둔 채 Python 삭제를 허용합니다."* — **성립한다.**

**해당 도메인의 알려진 명세 누락이 0이어야 그 도메인의 Python을 지운다.**

**왜 게이트 통과만으로는 못 막는가.** P2는 §6 매트릭스 게이트에 판정을 맡기는데, **명세에 없는 요구는 게이트가 검사할 수 없다.** 아래 마이크로 갭 3건이 정확히 그 상태였다 — 처음에는 "기록만, 다른 소유자"로 분류돼 **삭제와 아무 차단 관계가 없었다.** 그러면 인증 도메인에서 이런 일이 난다: 계정 존재 timing oracle 방지가 **Kotlin에 없고**(요구가 없으니 구현할 이유가 없다) **명세에 없고**(그게 갭이다) **Python에도 없다**(지웠다). 지식이 증발한다.

**근거.** 이 세 갭은 **Python을 읽어서 발견됐다**(codex 계획 심사 Q2 → deep-reasoner 정독). 그것이 곧 *"Python에는 아직 명세가 못 담은 요구가 남아 있다"*는 증거다. 발견되는 족족 명세로 올리지 않으면 **삭제가 그 증거를 없앤다.**

**일반 규칙.** 재구축 중 Python에서 **명세에 없는 요구를 발견하면, 그 요구가 명세에 오르기 전에는 해당 도메인의 Python을 지우지 않는다.**

**이것은 "전 항목 전사"로의 회귀가 아니다.** 차이는 **범위**다 — 전사는 *모든 것*을 옮기라는 것이었고, 이 규칙은 *발견된 누락만* 올리라는 것이다. 발견되지 않은 것에는 의무가 없고, 발견된 것에는 예외가 없다.

| 도메인 | 통과해야 할 게이트(§6) | 추가 순서 조건 |
|---|---|---|
| 마스킹 (§E) | Requirements + Security (+ `masking` fixture) | — |
| 텍스트 정규화·후처리 (§F·§H) | Requirements + Unit | — |
| 스타일 규칙·치환 비문 검출 (§B·§C) | Requirements + Unit | **P1-3·P1-4 반출 완료 후** (재구축 입력이 그 데이터다) |
| **프롬프트·변환 오케스트레이션 (§A·§G·§J)** | Requirements + **Quality** | **`tests/golden/baseline.json` 첫 기록 이후에만 삭제** |
| 문서 추출·내보내기 (§I·§L) | Requirements + Document (+ Security) | **P1-5·P1-6·P1-7 반출 완료 후** + **갭 μ-3 닫힘 후**(DOC-01·DOC-03) |
| 인증 (§M) | Requirements + Crypto + Security | **갭 μ-1·μ-2 닫힘 후**(I-8·I-9) |
| 저장 암호화 | Requirements + Crypto | — |
| 작업 큐 | Requirements + Worker | — |
| API 표면 | Requirements + Contract | — |
| 오프라인 도구 (§K) | — | **Phase 8 대상 아님.** Phase 9까지 존치 — 건별 이식/폐기는 Phase 9 작업이며 Phase 8 선행 조건이 **아니다**(순환 의존 제거) |

### 명시적으로 삭제한 선행조건 (더 이상 차단하지 않음)

교체 전 게이트에 있었으나 **이제 차단 조건이 아니다.** 남기려면 별건으로 소유자를 정한다.

- **갈래 1 — 이미 명세된 것의 전사**: 마스킹(`02_parity-verifier_masking-spec.md` + `masking` fixture 존재), 인증(safety-gate I-7·I-8·I-9), 내보내기, 문서 추출 보안(I-10), 채점 게이트(`02_quality-baseline.md`). 스펙이 있으면 Python을 볼 필요가 없다 — 사용자 결정.
- **프롬프트 문안 전사** (§A 표): 재구축이 §4.6대로 문안을 **고쳐도 되므로**, 옛 문안 보존은 차단 사유가 아니다. Quality 게이트가 결과를 잡는다.
- **검출 알고리즘 전사** (§C의 `GLOSS_COLLISION_PATTERNS`·`_is_nominalized`·`find_gloss_collisions`, §B의 `_appears_at_word_start`·`split_sentences`): P2의 Unit 게이트가 재구축 결과를 검증한다. 단 **입력 데이터**(워드리스트)는 P1로 남는다.
- **설계 의도 프로즈**: 이미 생존 문서(이 문서 §A~§M, `02_*`, `00_requirements-inventory.md`)에 있다. 중복 전사 불필요.

### 알려진 명세 누락 3건 — **삭제 차단 항목** (닫는 주체는 다른 소유자)

> **2026-08-12 재분류.** 처음에는 "기록만, Phase 8 차단 아님"으로 적었으나 codex stop-time 게이트 지적을 받아 **차단 항목으로 격상**했다. Python 추출이 아니라 **한 줄 스펙 추가**로 닫히는 것은 그대로다 — 가벼움이 차단하지 않을 이유는 되지 못한다. **닫는 것은 소유 레인**(`.claude/skills/migration-safety-gate/`·`00_requirements-inventory.md`)**이 하지만, 닫히기 전에는 해당 도메인의 Python 삭제가 막힌다.**

| # | 닫을 곳(소유자) | 추가할 내용 | 현재 상태 | 막는 것 |
|---|---|---|---|---|
| μ-1 | `migration-safety-gate` **I-8** | **계정 존재 timing oracle 방지** — 없는 사용자에도 더미 해시로 동일 검증 비용을 치른다(조기 반환 금지). 조기 반환하면 응답 시간이 가입 여부를 누설 | I-8에 이 항목이 **없다** (codex 계획 심사 Q2가 지목) | **인증 도메인 삭제** |
| μ-2 | `migration-safety-gate` **I-9** | `typ` 클레임의 **다중 토큰 용도 네임스페이스 설계 의도** — 같은 시크릿으로 서명될 이메일 인증·비번 재설정 토큰이 액세스 토큰으로 오용되지 않게 용도를 새긴다(JOSE 헤더 `typ`와 별개 네임스페이스) | I-9에 "불일치 거부"만 있고 **왜 그 클레임이 있는지**가 없다 | **인증 도메인 삭제** |
| μ-3 | 인벤토리 **DOC-01** / **DOC-03** | DOC-01에 **"중복·유령 내용 금지"**(`mc:Fallback` 이중 계수·linked header 중복·`w:delText` 제외) / DOC-03에 **"owner-password 열람 가능 PDF는 수용, user-password 불가만 거절"** | 어느 항목에도 없다 | **문서 추출 도메인 삭제** |

**이 표는 닫히면 비지 않는다.** 재구축 중 Python에서 명세에 없는 요구를 새로 발견하면 μ-4, μ-5로 **이 표에 추가**하고, 그 도메인 삭제는 다시 막힌다(위 일반 규칙).

---

**요약.** 차단은 **P1(반출)** 과 **P2(검증된 재구축 + 알려진 명세 누락 0)** 둘. P1이 안 닫히면 **되찾을 수 없는 것**이 사라지고, P2가 안 닫히면 **아직 안 선 것**을 지우거나 **게이트가 검사할 수 없는 요구**를 증거째 지우게 된다. 그 밖의 전사는 차단하지 않는다.
