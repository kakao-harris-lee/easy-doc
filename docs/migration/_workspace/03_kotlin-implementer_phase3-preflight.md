...
~~**지금은 아무 데서도 걸리지 않는다.** … 전부 계획이다 — 실행 소스가 0건이다.~~
**2026-08-19 갱신 (contract-keeper 실측) — 여섯 중 셋이 강제자가 됐다.** X-F11·X-F12·X-F13 은
`f9ee3e6` 에서 Kotlin 계약 테스트로 쓰였고(`AuthContractTest` S-4~S-8), 계약 파일을 직접 파싱해
기대값을 읽는다. 결속은 음성 대조로 확인했다 — 계약 값을 바꾸면 해당 케이스가 빨강이다
(재현 실측 `03_contract-keeper_auth-verification.md` §3). F3 애너테이션 금지 자체도
`RequestFieldConstraintLayerTest` 가 **계약에서 대상 목록을 읽어** 강제한다.

**나머지 셋은 여전히 계획이다** — X-F1·X-F2 와 X-F10, 그리고 X-F9 의 `name`·`edited_text` 케이스.
그 필드의 DTO 가 아직 없다. **이 표를 통째로 「이미 있는 강제 장치」로 읽지 마라** — 행마다 갈린다.

> **함께 등재 — 강제자가 생겼다고 매번 도는 것은 아니다.** 계약 파일이 Gradle 테스트 태스크의
> **선언된 입력이 아니어서**, 계약만 바뀐 변경은 `UP-TO-DATE`/`FROM-CACHE` 로 초록이 된다(실측).
> 위 셋의 강제는 **테스트가 돌 때** 성립한다. 상세와 처분은 위 검증 문서 §5.

| 검증 | 무엇을 잡나 | 출처 | **실행 상태** |
|---|---|---|---|
| **X-F11** | `password` 하한 미만 → 422 이고 **`detail` 이 문자열 타입**(배열이 아님) · 하한과 같은 길이는 통과 | **2026-08-15 신설** (게이트 15 X5) | **실행 소스 실재 (`f9ee3e6`)** — `AuthContractTest` S-6·S-7 · 음성 대조 N-1·N-1b 확인 |
| **X-F12** | `password` 원시 길이는 하한과 같고 trim 후 미달인 입력이 **통과** — 원시 측정 축 | **2026-08-15 신설** | **실행 소스 실재 (`f9ee3e6`)** — 같은 파일 S-8 · N-1b 확인. **X13 판정에 종속**(계약을 먼저 단언해 뒤집히면 실패 메시지로 알린다) |
| **X-F13** | `email` 정규화 후 상한 초과 **거절** + 문구가 형식 오류와 동일 | **2026-08-15 신설** | **실행 소스 실재 (`f9ee3e6`)** — 같은 파일 S-4(거절)·S-5(통과) · 음성 대조 N-2·N-5 확인 |
| X-F1·X-F2 | `text`·`edited_text` 길이 경계 + **`detail` 타입 문자열** | 기존 (F3 때 타입 단언 추가) | **계획 — 실행 소스 0건** · 마감 Phase 4 (해당 DTO 없음) |
| X-F9 | 원시 초과·정규화 후 이하인 입력이 **통과**(정규화 후를 재는 3필드) | 기존 | **부분 — `email`·`name` 실행 소스 실재** (`f9ee3e6` S-5 · `951b1fd` `WorkspaceContractTest` WC-7, 저장까지 WC-8). `edited_text` 는 Phase 4 |
| X-F10 | `name` 이 공백만인 입력을 문자열 `detail` 로 거절 | 기존 | **실행 소스 실재 (`951b1fd`)** — `WorkspaceContractTest` WC-4 · 음성 대조 N-12 확인(2026-08-19 contract-keeper 독립 재현) |
