...
`00_contract-keeper_test-plan.md` §3 의 아래 항목이 이 금지의 **강제자**다. 계약 문서가 금지하고
테스트가 허용하면 다음 사람이 읽는 것은 테스트이므로, 금지는 이 목록으로만 실제 효력을 갖는다.

| 검증 | 무엇을 잡나 | 상태 |
|---|---|---|
| **X-F11** | `password` 8자 미만 → 422 이고 **`detail` 이 문자열 타입**(배열이 아님) · 8자 정확히 통과 | **2026-08-15 신설** (게이트 15 X5) |
| **X-F12** | `password` 원시 8자·trim 후 1자 입력이 **통과** — 원시 측정 축 | **2026-08-15 신설** |
| **X-F13** | `email` 정규화 후 255 초과 **거절** + 문구가 형식 오류와 동일 | **2026-08-15 신설** |
| X-F1·X-F2 | `text`·`edited_text` 4,000자 경계 + **`detail` 타입 문자열** | 기존 (F3 때 타입 단언 추가) |
| X-F9 | 원시 초과·정규화 후 이하인 입력이 **통과**(3필드) | 기존 |
| X-F10 | `name` 이 `"   "` 를 문자열 detail 로 거절 | 기존 |
