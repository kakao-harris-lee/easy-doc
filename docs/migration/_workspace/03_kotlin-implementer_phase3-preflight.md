...
| X-F9 | 원시 초과·정규화 후 이하인 입력이 **통과**(정규화 후를 재는 3필드) | 기존 | **부분 — `email` 케이스만 실행 소스 실재 (`f9ee3e6`, S-5)**. `name` 은 작업 공간 단위 · `edited_text` 는 Phase 4 |
| X-F10 | `name` 이 공백만인 입력을 문자열 `detail` 로 거절 | 기존 | **계획 — 실행 소스 0건** · 마감 작업 공간 단위 (`WorkspaceNameRequest` 없음) |
