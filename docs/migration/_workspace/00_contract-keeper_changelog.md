...
## 2026-08-19 · C-4 · 내보내기 파일명 금지 문자 집합 신설 + 인라인 헤더 2건 처분 판정

| 항목 | 값 |
|---|---|
| 계기 | **Phase 4 착수**(사용자 승인 2026-08-19). 리더 지시 1(인라인 헤더 2건 계약 처분)과 3(Phase 4 계약 테스트 명세). 전제는 `03_kotlin-implementer_phase4-preconditions.md` §2.1의 **전제 정정** — 오늘 계약의 인라인 헤더는 0건이 아니라 **2건**이다 |
| 근거 | **G2** (요구사항이 요구한다) — 계획 §5 Phase 2 종료 조건 「내보내기 파일명·`Content-Disposition` 생성 포팅」(원장 **L348**). 그 요구의 일부가 무단언이라는 것이 게이트 14 **F-3·F-4**와 게이트 16 후속 **R-1**이고, 마감이 **「Phase 4 착수 전」**이었다 |
| `info.version` | **올리지 않는다** (1.1.0 유지) — 상태 코드·본문 스키마·헤더 구조·예시·미디어 타입 무변경. 조항은 새것이지만 **오늘 적합한 어떤 구현에서도 나가는 바이트가 달라지지 않는다**(구현이 이미 만족한다) |
| React 영향 | **없음**(실측). `parseFilename`은 확장 파라미터를 **읽기만** 하고 문자 집합을 검사하지 않는다. `Location`을 읽는 코드는 프런트 전체에 **0건** |
| 통보 대상 | `kotlin-implementer`, `parity-verifier`, `privacy-gate`, `migration-reviewer`, 리더 |
...
| 조항 | 종전 | 지금 |
|---|---|---|
| `paths..../export...headers.Content-Disposition.x-filename-charset` | 없음 | **신설.** `forbidden`(기계가독 문자 집합) · `measured_on`(확장 파라미터를 퍼센트 디코딩한 값) · `check` · `rationale` · `x-openapi-expressibility` |
| 같은 헤더의 `description` | 구조·React 의존만 | **한 문단 추가** — 컴포넌트화하지 않는 판정과 사유 |
| `x-changelog` | — | 이 항목(`revision: C-4`) |

**문면은 위 두 곳뿐이다.** 스펙 전문을 여기 옮겨 적지 않는다.

### 없던 것은 성질이 아니라 **오라클**이었다

구현(`backend-kotlin/core/src/main/kotlin/kr/easydoc/core/easyread/Export.kt`)은 이미 이
성질을 만족한다. F-4가 지적한 것은 「구현이 틀렸다」가 아니라 **「그 요구가 어느 단언에도
걸리지 않는다」**였다.

계약에 기계가독 집합이 없으면 경계 테스트가 집합을 **코드에 복제**한다. 그러면 계약과 구현이
갈려도 테스트는 **자기 사본과 대조해 초록**이고, F-4는 「요구가 무단언」에서 「요구가 자기
사본에만 걸림」으로 **모양만 바꿔** 남는다. X-J2(계약 파일 직접 파싱)가 금지하는 형태다.

**계약이 값을 두 벌 갖는 것 아닌가 — 아니다.** `forbidden`은 **계약이 오라클**이고
`Export.kt`의 상수는 **구현**이다. 둘이 갈리면 경계 테스트가 빨강으로 알린다. 그것이 이
조항을 계약에 두는 이유이며, 계약 **안에서** 같은 값이 여러 벌인 문제(§4-3의 삼중 선언)와는
다른 축이다. 신설 시점에 두 문자열이 **바이트 동일**임을 실측으로 확인했다.

### 인라인 헤더 2건 — **컴포넌트화하지 않는다** (계약 동작 무변경)

**바꾸지 않기로 하는 것도 판정이므로 기록한다.**

| 근거 후보 | 성립하는가 |
|---|---|
| G1 계약이 사실과 다르다 | 아니다 — 두 선언은 실제로 나가는 것을 정확히 적는다 |
| G2 요구사항이 요구한다 | 아니다 — `$ref`를 요구하는 요구사항이 없다. OpenAPI도 인라인 헤더를 정상 문법으로 허용한다 |
| G3 정책 결정 | 아니다 |
| G4 지킬 수 없거나 지켜지지 않는 형태 | **아니다 — 가장 그럴듯했던 후보다.** X24-5가 드러낸 것은 조항의 결함이 아니라 **강제 수단의 도달 0**(파서가 인라인 갈래를 조용히 무시)이었고 `44eec3f`가 파서를 fail-closed로 닫았다. **탐지를 넓혀 해결된 자리를 구조 변경으로 되돌리지 않는다** |

**바꾸면 잃는 것**: `components/headers`의 세 항목은 전부 `schema.const`를 갖고 그 성질이
`ContractHeaderDeclarationTest`로 강제된다. 계산되는 값은 `const`를 가질 수 없으므로
컴포넌트화하려면 **그 탐지 조항을 약화시켜야 한다.**
**얻는 것**: 없다 — 두 헤더는 각각 한 응답에서만 쓰여 줄일 중복이 없다.

`inlineHeaderNames()`를 두 이름으로 고정한 강제자는 **그대로 둔다.** 셋째가 들어오는 커밋을
실패시키고, 그때 정할 것도 이미 적혀 있다(값이 고정이면 컴포넌트 + `const`, 계산되면 형식
단언 동반). **이 판정이 그 규칙의 첫 적용 사례**이고 두 헤더는 둘째 갈래다.

### 형식 검증 — 하나는 조항 없이 서고, 하나는 조항이 필요했다

| 헤더 | 형식을 계약에서 읽을 수 있는가 | 처분 |
|---|---|---|
| `Location` | **있다** — `paths`의 경로 템플릿(P-15가 이미 읽는다) + `DocumentCreatedResponse.properties.conversion_id.description`의 대응 선언 | **변경 없음.** 테스트가 두 노드를 읽어 기대값을 **조립한다** |
| `Content-Disposition` 구조 | **있다** — `schema.examples` 첫 원소가 두 파라미터를 모두 담은 완전한 값이다 | **변경 없음.** 예시를 **파싱해** 구조를 얻는다(P-17이 이미 선 갈래) |
| `Content-Disposition` 파일명 문자 집합 | **없었다** | **`x-filename-charset` 신설** |

### 영향받는 검증

| 항목 | 처리 |
|---|---|
| **CE-5·CE-6** (신규) | `04_contract-keeper_documents-test-spec.md` §2-6. 금지 집합을 계약에서 **읽어 케이스를 유도**한다 — 손열거 금지 |
| **P-23** (신규 파서 노드) | `forbidden`·`measured_on` |
| **N-20** (신규 음성 대조) | C1 구간만 지웠을 때 **C1 케이스만** 빨강(다른 블록 케이스는 깨지지 않아야 한다) |
| **같은 변경 단위** | CE-5·CE-6은 **내보내기 엔드포인트를 구현하는 그 커밋**에 들어간다 |
| **기존 테스트** | 무영향 실측 — 개정 후에도 인라인 헤더 **2건**, `:api:test --tests '*Contract*'` **exit 0** |

### 함께 처리한 문서 드리프트 1건 (계약 무변경)

`00_contract-keeper_test-plan.md` §2 **X-A2** 행이 `x-auth.failure_uniformity`를 인용하면서
「헤더 누락」을 균일화 열거에 넣고 있었다 — `dec3124`(게이트 24 R-1)가 계약을 정합하면서
`affected_tests`에 *"별도 문서 커밋에서 정정한다"*로 예고한 **바로 그 1건**이다. 정정했고,
정정 사유를 같은 절에 blockquote로 남겼다.

**강제되는 동작은 달라지지 않는다** — 실제 케이스(L-3·M-3)는 이미 두 갈래로 나눠 재고 있었다.
바뀐 것은 **표가 정본을 어떻게 인용하는가**뿐이며, 인용이 정본보다 넓으면 다음 사람이 그 행을
근거로 무헤더를 균일성 측정에 넣는다(실제로 4갈래 비가 그렇게 만들어졌다).

### 리더에게 올리는 것

| # | 항목 | 내용 |
|---|---|---|
| ⑴ | **`x-open-asymmetry` 판정 요청** | `04_contract-keeper_documents-test-spec.md` §7-2. 선택지 셋 + 각 안의 와이어 변화·React 영향·걸리는 검증. **게이트 15 X13(`password` 측정 축)과 같은 갈래이므로 함께 판정하기를 요청**한다. 계약 소유자는 **현행 조항을 유지한 채 검증만 채웠다** — 권고는 있으나 G 근거를 단독으로 세울 수 없다(남는 것은 G3이고 소유자는 정책을 만들지 않는다) |
| ⑵ | **O-17 — X-J1·X-J3의 조항 미확정** | 확정 Phase가 **4**인데 조항이 없어 케이스를 쓸 수 없다. 둘은 함께 판정해야 하고, X-J1은 사용자 가시 동작 변경이라 소유자 단독 범위 밖이다 |
| ⑶ | **이 개정(C-4) 보고** | 되돌리려면 YAML 한 절 삭제 + CE-5·CE-6·N-20 철회다. **L348 판정 자체는 그대로 리더의 것이다** |
| ⑷ | **O-14 — `/health` `description`의 자기모순** | 「캐시 금지 헤더도 붙지 않는다」가 남아 있는데 전역 부착 판정과 `x-superseded`가 이미 무효화했고 구현·테스트는 반대다. **G1이 명백한 한 문장이나 #14는 이 단위 밖**이라 손대지 않았다 — 무관한 엔드포인트를 Phase 4 개정에 끼우면 blast radius를 읽을 수 없다. 처리 단위를 지정해 주기를 요청한다 |
| ⑸ | **O-20** | CORS 노출 헤더 두 원소가 이 배치에서 처음 실물이 되지만 **브라우저 축(X-H5)은 Phase 6**이다. 이 배치를 근거로 X-H4/X-H5를 닫지 않기를 |

### 통보

| 대상 | 내용 |
|---|---|
| `kotlin-implementer` | 계약에 조항 하나가 늘었다(`x-filename-charset`). 내보내기 파일명 정제는 그 집합을 **계약에서 읽어** 재고, 구현 상수와 갈리면 CE-5가 빨강이다. 나머지 Phase 4 지시는 명세 §8 |
| `parity-verifier` | **새로 범위에 드는 것**: 내보내기 파일명의 금지 문자 집합 — 종전에는 계약에 없어 판정 오라클이 없던 자리다. **오라클이 없는 자리**: `withheld`·`ambiguous`의 처분(O-17) |
| `privacy-gate` | 파일명은 개인정보가 아니지만 **제목이 사용자 입력**이라 같은 입력 경로를 공유한다. 이 배치가 가려졌던 원문을 처음 응답에 싣고(CR-5) 복원 산출물을 처음 내보낸다(CE-12) |
| `migration-reviewer` | 계약 준수 축 — 이 개정의 G 근거(G2)와 비파괴 판정, 그리고 **인라인 헤더를 바꾸지 않은 판정**의 근거가 검토 대상이다 |

---

## 2026-08-20 · G27-C1 · 제목 유도 조항 폐기 — 제목은 문서 내용에서 만들지 않는다

| 항목 | 값 |
|---|---|
| 계기 | **게이트 27 codex Critical ①(C-1)** 의 처방 + **2026-08-20 사용자 결정** |
| 근거 | **G3** (정책 결정 변경). 계약이 유도 동작을 **명시적으로 요구**하고 있었으므로 코드만 고치면 구현이 계약 밖이 된다 |
| `info.version` | **1.1.0 → 1.2.0** — 앞선 두 개정(R-1·C-4)과 달리 **나가는 바이트가 바뀐다**(제목 없이 올린 문서의 `title`). 비파괴다: 타입·`maxLength`·필드 유무·상태 코드·헤더 무변경 |
| React 영향 | **런타임 코드 변경 0건**(실측). `types.ts`·`client.ts`·`HistoryPage.tsx` 어느 쪽도 고칠 것이 없다. 화면 문구만 달라진다 |
| 통보 대상 | `kotlin-implementer`, `parity-verifier`, `privacy-gate`, `migration-reviewer`, 리더 |

### 무엇이 왜 바뀌었나

유도된 제목은 **방어 둘을 동시에 우회**한다 — 봉인된 원문의 조각을 평문 열
`documents.title`에 복제해 **암호화를 우회**하고, 마스킹은 워커의 일이라 업로드 시점에는
돌지 않으므로 **마스킹도 우회**한다. 얻는 것은 목록 화면의 편의뿐이다.
`migration-safety-gate` **I-4** 의 보장 *"파일명은 아예 저장하지 않는다"* 와 같은 계열이며,
이 결정은 그 조항을 **강화한다**(파일 이름도 본문도 제목이 되지 않는다).

**사유를 조항에 남긴 이유**: 계약은 현재 동작의 스냅샷이 아니라 요구사항이 요구하는
인터페이스다. 왜 유도를 버렸는지가 없으면 다음 사람이 **편의**를 이유로 되살린다.

### 고친 자리 (전수)

| 자리 | 성격 |
|---|---|
| **`x-title-policy`** (신설, `x-input-limits` 뒤) | **정본.** `sources`·`forbidden_sources`·`fallback_title`(`제목 없음`)을 **기계가 읽을 수 있게** 두었다. `x-superseded` 에 폐기한 옛 문면을 남겼다 |
| `DocumentTextRequest.title` | 「본문 첫 줄 유도(30자·어절 경계·`…`)」 → 「고정 문구 `제목 없음`」. **파일명 금지 문장은 그대로 두고** 강화 사실을 덧붙였다 |
| `DocumentFileRequest.title` | 같은 규칙 + **파일명 금지를 이 절에도 명시**(종전에는 텍스트 모드에만 있어 파일 모드만 읽는 사람에게 도달하지 않았다) |
| `DocumentListItem.title` | 값의 출처와 `maxLength`의 의미(사용자 입력을 자른 결과의 사후 보장) |
| `x-input-limits.max_title_length` 주석 | 상한이 **사용자 입력에만** 걸린다는 사실 |
| `x-global-response-headers.applies_to` 의 `GET /documents` 주석 | 낡은 사유("본문 첫 줄에서 유도한 콘텐츠") 정정. **목록 10곳은 건드리지 않았다** |
| `x-filename-charset.check` | 파일명의 바탕 서술 정정 |
| `POST /workspaces` description | 대비 문구에서 「자동 유도」 제거 |

### 세우지 못한 것 — contract test

이 개정은 **Kotlin 코드를 쓰지 않았다**(같은 시각 구현 레인이 작업 중). 필요한 단언:
`title` 없이 JSON·multipart 로 올린 뒤 `GET /documents` 의 `title` 이 계약에서 읽은
`x-title-policy.fallback_title` 과 같음을 **HTTP 경계에서** 단언한다(값 손열거 금지 — X-J2).
음성 대조는 「본문 유도를 되살리면 빨강」이다.

### 구현 레인과의 정렬 — 한때 갈렸고 실측으로는 수렴 중이다

착지한 커밋 `13b0828` 의 `TitleRules.kt::resolveTitle(given, uploadFilename)` 은 제목이 없을
때 **업로드 파일 이름**을 썼다. 그 커밋은 같은 날 사용자 결정을 *"파일 이름은 써도 된다"* 로,
이 개정의 지시는 *"본문에서도 파일명에서도 만들지 않는다"* 로 적었다 — **둘은 동시에 참일 수
없다.**

**실측(같은 시각)**: 구현 레인의 작업 트리는 이미 `resolveTitle(given)` 한 인자로 줄어
**계약과 같은 규칙**이다. 나중 결정이 앞 결정을 덮었고 두 레인이 같은 곳으로 가고 있다.
**아직 커밋되지 않아 이력에는 갈린 상태만 남아 있다** — 구현 레인의 다음 커밋이 착지하면
닫힌다. 그때까지 `13b0828` 기준으로는 파일 모드가 계약 밖이다.

---

## 2026-08-20 · G27-C3 · 502 폐기 · 503 큐 줄 폐기 · 저장 정의역 신설

| 항목 | 값 |
|---|---|
| 계기 | **리더 판정 L-1 의 잔여 ⑴⑵**(게이트 26 `04_contract-keeper_gate26-contract-actions.md` §2-3·§7) + **교차 종합 행 2**(합의 Major, 수신자 `contract-keeper`) + **구현 계획 §9 질문 ④**(X1 거부 문구가 계약에 없다) |
| 근거 | **G1**(502·503 큐 줄 — 계약이 사실과 다르다: 어떤 구현에서도 나오지 않는다) · **G2**(저장 정의역·제목 서로게이트 — `migration-safety-gate` I-7 의 round-trip 요구) |
| `info.version` | **1.2.0 → 1.3.0** — 나가는 바이트가 바뀐다(`POST /documents` 가 502 를 낼 수 없고, 422 갈래가 하나 는다). **비파괴**: 상태 집합 축소는 소비자가 받는 값의 부분집합화이고, 422 갈래 추가는 이미 선언된 상태 안이다 |
| 소유자 단독인가 | **아니다 — 리더 지시로 집행했다.** `x-change-policy.escalate_to_leader` ④(배포·운영 동작)에 걸리는 변경이고, 게이트 26 이 그 이유로 단독 집행을 거부했다. 이 회차는 리더가 「계약·구현·테스트를 한 변경 단위로 C3 에서」를 지시해 그 조건이 충족됐다 |
| React 영향 | **런타임 파손 0건**(실측). 회수 대상 2건은 계약 `x-retired-responses.consumer_debt` 가 정본 |
| 통보 대상 | `kotlin-implementer`, `parity-verifier`, `privacy-gate`, 리더 |

### 무엇이 왜 바뀌었나

**⑴⑵ 는 같은 뿌리다 — 폐기된 아키텍처(Redis/ARQ)를 전제한 두 조항.**
큐가 PostgreSQL 의 테이블이 되면서 ⓐ 「커밋은 됐는데 등록만 실패」라는 상태와
ⓑ 「큐만 미배선」이라는 구성이 **둘 다 존재할 수 없게** 됐다.
실측(2026-08-20): `QueueUnavailableException` 을 던지는 제품 코드 **0곳**,
큐 배선에 대한 `ConfigurationException` **0곳**, 동기로 LLM 을 부르는 오퍼레이션 **0개**.

**그리고 큐 줄을 재다가 형제 줄도 죽어 있음을 발견했다.** 「저장 암호화 키 미설정 → 문서 API
전체 503」은 **2026-08-19 리더 판정(게이트 25 F-2·F-3·X8)** 이 기동 fail-fast 로 바꾼 자리다
(`CryptoConfiguration.verify()` — *"하나라도 어긋나면 앱을 띄우지 않는다"*). 키가 없으면
**서비스가 없으므로** 그 503 은 도달 0이었고, 계약이 그 판정을 반영하지 못한 채 남아 있었다.
**처음에는 DC-19 의 무대를 그 줄로 옮기려 했다** — 실측하지 않았으면 무대 없는 케이스를
다시 만들었을 것이다.

**한 판정에서 함께 처분한 이유**: 두 조항은 명세 X-C6 축(「502 ≠ 503」)의 두 팔이었다.
한 팔만 내리면 다른 팔이 무대 없는 케이스로 남는다(게이트 26 §2-3 이 미리 지목했다).
축은 **「503 ≠ 500」**으로 다시 세웠고 **두 팔 모두 도달이 실재한다**.

**⑶ 은 성질이 아니라 오라클이 없던 자리다.** `PlainBody` 가 짝 없는 서로게이트를 거부하는
성질은 이미 구현에 있고, 없던 것은 **계약의 기계가독 문구**다. 계약에 없으면 경계 테스트가
문구를 코드에 복제하고, 그러면 계약과 구현이 갈려도 자기 사본과 대조해 초록이다
(`x-filename-charset` 신설과 같은 갈래).

**⑷ 제목은 거절이 아니라 정제다.** 제목의 처분은 이미 정제이고(제어문자 제거 + 상한에서
자르기 + 대체 문구), 같은 필드에서 「길이는 자르고 문자는 거절」로 갈리면 **사용자가 라벨
하나 때문에 문서 접수를 거절당한다.** 본문은 반대다 — 거기서 잃는 것은 라벨이 아니라 문서다.

### 고친 자리 (전수)

| 자리 | 성격 |
|---|---|
| `POST /documents` description — 검사 순서 문장 + `EnqueueFailed`/502 문단 | 재서술. 한 트랜잭션·전량 롤백·**500**·멱등 |
| `POST /documents` `responses.'502'` | **삭제** (계약 전체에서 502 를 선언한 유일한 자리였다) |
| `components/responses/BadGateway` | **삭제** (참조가 그 하나였으므로 남기면 고아다). 자리에 안내 주석을 남겼다 |
| **`x-retired-responses`** (신설, `x-change-policy` 뒤) | **폐기 기록의 정본.** `status`·`ground`·`was`·`why`·`if_it_should_return`·`consumer_debt`. **어느 오퍼레이션에도 다시 나타나지 않는다**를 기계가 잴 수 있게 둔다 |
| `components/responses/ServiceUnavailable` | **도달 0인 두 줄 삭제** — 「큐(Redis) 미배선 → 업로드」(+`no_queue`)와 **「저장 암호화 키 미설정 → 문서 API 전체」**(+`no_cipher`). 남은 유일한 줄(인증 서명 키)의 범위를 「인증 API 전체」 → **「인증이 필요한 모든 요청」**으로 정정. 넷째 줄(DB)은 **재지 않아 그대로** |
| `ConversionResponse.failure_code.description` | `"EnqueueFailed"` 요구 삭제 + 「접수 뒤의 실패만 실린다」 |
| **`x-stored-text-domain`** (신설, `x-title-policy` 뒤) | **저장 본문 정의역의 정본.** `detail`·`detail_shape`·`status`·`layer`·`applies_to`(팔마다 **측정 상태 표식**) |
| `POST /documents` 422 description + `examples.undecodable_text` | 갈래 추가. **`examples` 가 전칭이 아님**을 명시 |
| `DocumentTextRequest.text` · `ConversionReviewRequest.edited_text` description | 정의역 포인터(길이와 **다른 축**) |
| `x-title-policy.rule` + **`x-surrogate-note`**(신설) | 제목의 서로게이트 **정제** + 본문과 처분이 다른 사유 |
| 파일 머리 주석의 `502` 언급 | 초판 작성 경위임을 명시 + 폐기 사실 |

### 세운 검증과 세우지 못한 것

**명세에 세웠다**(`04_contract-keeper_documents-test-spec.md`): DC-18 재정의(500 + 전량 롤백) ·
DC-19 재정의(503 의 무대를 **인증 서명 키**로 교체 — 첫 후보였던 저장 암호화 키도 무대가 아니었다) · X-C6 재정의(「503 ≠ 500」) ·
X-K1 축 신설 · DC-24·DC-25 신설 · 파서 P-38·P-39·P-40 · 음성 대조 N-31~N-34 ·
관찰 O-21·O-22.

**Kotlin 코드는 쓰지 않았다** — 이 레인의 경계다. 필요한 변경은
`04_contract-keeper_l1-residual-verdict.md` §4 에 파일·심볼 단위로 있고,
**계약과 같은 커밋(C3)에 들어가야 한다.** 그때까지 `GlobalExceptionHandler` 의
502 갈래와 `ErrorContractTest` 의 502 두 행이 **계약 밖**이다.

### 재지 않아 손대지 않은 것

- `ServiceUnavailable` 의 **넷째 줄** 「DB 세션 팩토리 미배선 → 전체」(명세 O-21) — 앞 두 줄과
  같은 계열로 의심되나 **기동 동작을 실제로 돌려 보지 않았다.** 앞 두 줄은 코드로 확인했고
  이 줄은 확인하지 못했다 — **그 차이가 지운 것과 남긴 것을 가른다.**
- `/health` description 의 Redis 문면(게이트 26 ⓖ · 명세 O-22) — #14 는 이 단위 밖이다.
- 둘 다 **O-14 와 한 단위로 묶기를 리더에게 제안**했다. 셋 다 「폐기된 아키텍처를 가리키는
  진단 서술」이라는 같은 종류다.

> **→ 위 「재지 않아 손대지 않은 것」 둘은 다음 절(G28-P5)에서 닫혔다.**
> **DB 줄은 유보를 지운 것이 아니라 재서 내렸다.** 「넷째 줄」이라는 서수 호칭도 그 절에서
> 안정 id 로 바뀌었다 — **이 절의 문면은 그때의 기재이므로 고치지 않는다.**

---

## 2026-08-20 · G28-P5 · 503 다섯 자리 정합(서수 → 안정 id) · DB 줄 실측 폐기 · 415 신설 · 검사 순서

| 항목 | 값 |
|---|---|
| 계기 | **게이트 28 리더 판정 P-5(마감 「즉시」)** — codex F-9 가 *"정본 자체로 무엇이 참인지 결정할 수 없다"* 를 짚고 교차 종합이 **다섯 자리**의 불일치를 고정 리비전에서 실측했다. 함께 지정된 단위: **O-14·O-21·O-22** · 합류 지시: **L-⑧ 판정 1·판정 3** · 선결 답: **P-4** · 채택: **P-12** |
| 근거 | **G1**(503 다섯 자리 · DB 줄 · `/health` 두 문면) · **G2**(415 · 검사 순서 · 저장 정의역 어휘) |
| `info.version` | 1.3.0 → **1.4.0**. 비파괴 — **415 선언 추가**(응답 **추가**는 소비자 쪽 비파괴)와 문면 정합이다. 삭제한 것은 **도달 0으로 실측된 503 구성 하나**이고 그 상태는 어느 구현에서도 나오지 않는다 |
| React 영향 | **없음.** `types.ts`·`client.ts`·컴포넌트 어느 쪽도 503 사유 목록·`/health` 진단 대상·415 를 읽지 않는다(`/health` 는 호출부 자체가 없고, `readErrorMessage` 는 상태 코드를 가리지 않고 `detail` union 만 본다). **415 는 `client.ts` 의 일반 오류 경로로 그대로 흐른다** |
| 통보 대상 | `kotlin-implementer`(지시 12건 — 판정문 §2~§6 의 ④), `parity-verifier`(415 의 `detail` **출처 규칙** 신규 · 「DB 미배선 503」 범위 이탈), `privacy-gate`(무인증 415 의 API 표면 누설), 리더(**판정 대기 3건**) |
| 전문 | `04_contract-keeper_p5-verdict.md` — 항목마다 ①고른 답 ②명령과 출력 ③고르지 않은 것의 대가 ④구현 지시 ⑤음성 대조 |

### 무엇이 바뀌었나

| 자리 | 변경 |
|---|---|
| **`x-retired-503-configurations`** (신설, `x-retired-responses` 뒤) | **폐기된 503 구성의 정본.** 항목마다 **안정 `id`**(`queue`·`cipher`·`datasource`)·`revision`·`why`·`if_it_should_return`. `x-retired-responses` 와 다른 축이다 — 저쪽은 **상태 코드**의 폐기이고 이쪽은 그 코드가 살아 있는 채로 **그것을 만드는 구성**의 폐기다 |
| `components/responses/ServiceUnavailable` | 폐기 목록 산문 **전체를 내리고 포인터로 교체**(13개 오퍼레이션이 선언하는 컴포넌트라 사유를 여기 두면 13곳이 인용한다). **「기동 자체는 막지 않는다」를 「이 줄뿐이다」로 좁혔다** — 저장 암호화와 DataSource 는 기동을 막는다(둘 다 실측·판정 근거) |
| **`datasource` 구성 폐기** | **실측 후.** bootJar 를 **다섯 구성**으로 띄웠다(대조군 포함). 미배선·도달 불가면 **컨텍스트 조립이 취소되어 앱이 뜨지 않고**, 강제로 띄운 구성(제품 구성 아님)에서조차 DB 를 만지는 요청은 **500** 이었다 |
| `paths./health.get.description` | ⑴ **「캐시 금지 헤더도 붙지 않는다」 삭제**(재실측: 붙는다) → 전역 부착 정본 포인터 ⑵ **Redis 를 진단 대상에서 삭제**(제품 코드 적중 0) ⑶ **`checks.database: false` 팔의 도달 0 등재** — **조항은 지우지 않았다** |
| `HealthResponse.properties.checks.description` | 두 키가 **같은 배선을 공유**함을 명시 + 도달 0 포인터 + **예시 둘이 「조항의 모양」이지 관측된 응답이 아님**을 명시 |
| **`x-unsupported-media-type`** (신설) + **`components/responses/UnsupportedMediaType`** (신설) | **415 의 정본.** 규칙을 **집합으로** 적는다(「요청 본문을 받는 모든 오퍼레이션」) + `detail_source`(상태 코드의 표준 사유 문구 — **한국어로 바꾸면 세 경로 균일성이 깨진다**) + `response_header_accept` + **미측정 팔 1건 명시** + **판정 대기 1건** |
| `paths` 6개 오퍼레이션의 `responses` | **`'415'` 선언 추가** — `/auth/signup` · `/auth/login` · `POST /documents` · `PUT /conversions/{id}` · `POST /workspaces` · `PATCH /workspaces/{id}` |
| `paths./documents.post.description` 검사 순서 | **복합 결함 조항 신설**(관측되는 응답은 목록에서 가장 앞선 검사의 것) + **작업 공간 식별자 형식 검증의 자리를 메웠다**(종전 목록에는 소유권만 있었다) |
| `x-stored-text-domain` | **`x-status-vocabulary` 신설**(닫힌 세 값 + `x-closed-enum`) · 파일 모드 팔을 **「도달하지 않음」 표식 + `measured_absent`**(어떻게 쟀나 · 무엇이 바뀌면 다시 도달하나)로 교체. **선언은 지우지 않았다** |
| `x-changelog` 1.3.0 항목 | **본문 무변경 + `superseded_by` 만 추가.** 그때 내린 것은 실제로 두 줄이었고 그 기재는 참이었다 — **고치면 역사 위조다** |

### 다섯 자리가 갈려 보인 진짜 이유 — 세는 방식이었다

「세 줄」은 **폐기 블록의 항목 수**, 「두 줄」은 **실제로 내린 수**, 「넷째 줄」은 **줄어들기
전 목록의 서수**였다. **셋 다 각자 참이었다.** 모순은 서수가 가리키는 목록이 그사이 줄어든
데서 나왔다. 그러므로 처방은 문면을 다시 세는 것이 아니라 **서수를 없애는 것**이다 —
안정 `id` 를 주고 다른 자리는 그 id 로만 가리킨다. 문면 수만 맞추면 **다음 폐기에서 또 갈린다.**

### 세운 검증과 세우지 못한 것

**명세에 세웠다**(`04_contract-keeper_documents-test-spec.md` §7-5):
DC-26·DC-27(**복합 우선순위 — 오늘 0건이던 자리**) · DC-28·CU-11(415) ·
X-L1·X-L2 축 신설 · 파서 **P-41~P-44** · 음성 대조 **N-35~N-45** · O-14·O-21·O-22 종결.

**Gradle 을 한 번도 돌리지 않았다.** 고친 것이 YAML·마크다운이고 **Kotlin 레인이 같은
트리에서 병렬 작업 중**이라 빌드 산출물을 공유하지 않았다 — **이 개정에 테스트 근거는 없다.**
**계약 값 축의 음성 대조 11건은 전건 미실행**이며, 그중 여섯은 P-41·P-44 가 서기 전에는
**실행 자체가 불가능**하다(그 파서가 오늘 0개라 「바꿔도 안 깨진다」가 예상 결과다 —
그것을 통과로 적으면 안 된다).

**Kotlin 코드는 쓰지 않았다** — 이 레인의 경계다. 지시는 판정문 §2~§6 의 ④ 칸에
`K-P5-1`~`K-P5-12` 로 파일·심볼 단위로 있다. 그때까지 **`/health` 가 계약을 위반한 상태**로
남는다(계약 `required` 에 `checks` 가 있고 구현에 그 필드가 없다 — **와이어에서 확인했다**).

### 재지 못해 손대지 않은 것

- **`PUT /conversions/{conversion_id}` 의 415** — 구현이 없다. 6개 중 **다섯만 실측**이고
  여섯째는 선언만이다. 그 사실을 계약 절(`x-measured.not_reached`)에 적었다.
- **복합 결함 입력을 소켓으로 재지 않았다** — 기제는 교차 종합의 고정 리비전 코드 실측을
  채택했고, 실행 재현은 DC-26·DC-27 의 몫이다.
- **DB 는 살아 있고 작업 테이블만 없는 구성** — 그 상태를 만들려면 마이그레이션을 우회해야
  하고 그것은 제품 구성이 아니다. **「충족」으로도 「위반」으로도 적지 않았다.**

### 리더 판정 대기 3건

⑴ **DB 기동 정책**(`escalate_to_leader` 4번) — 계약이 「기동을 막지 않고 `/health` 로
진단한다」와 「DB 를 진단한다」를 동시에 원하는데 **실측이 그 둘의 비양립을 보였다.**
선택지 셋(현행 유지 / 기동을 막지 않게 / DB 진단을 내린다)의 대가는 판정문 §7-2 ⑴ 에 있다.

⑵ **무인증 415 가 401 보다 앞선다 — 어느 리뷰도 짚지 않은 새 발견**(3번). 415 응답의
`Accept` 헤더가 **무인증 호출자에게 그 오퍼레이션이 받는 미디어 타입 집합**을 준다.
계약의 「인증이 먼저다」가 든 근거가 **API 표면 탐색**이라 정면으로 걸린다.
**조항을 좁히지 않았고 케이스도 쓰지 않았다** — 판정 전에 기대값을 못박으면 틀릴 수 있는
동작을 계약으로 굳힌다.

⑶ **원장 두 줄** — `00_progress.md` 는 계약 레인 범위 밖이라 대체 문면을 값으로 넘겼다.

---

## 2026-08-23 · G29-X8 · 두 팔의 `detail` 모양 정합 — JSON 팔 형식 오류 처분 복기 + 「타입 불일치」 한정

| 항목 | 값 |
|---|---|
| 계기 | 게이트 회차 `04_documents-c6r2` **X-8**. `POST /documents`의 JSON 팔이 형식 오류 `workspace_id`에 **배열** `detail`을, multipart 팔이 **문자열**을 낸다 — 같은 오퍼레이션·같은 결함에서 모양이 갈렸다. 리더가 contract-keeper 판정으로 내렸고(`04_contract-keeper_c7-rulings.md` §2), 판정 결과를 받아 리더가 이 개정 2건을 승인했다 |
| 근거 | **G4** (⑴⑵ 모두 — 조항이 지켜지지 않는/서로 모순인 형태다) + **G1** (⑴ 보조 — 두 팔을 대칭으로 진술하지 않아 사실과 어긋나 보였다) |
| `info.version` | **올리지 않는다** (1.4.0 유지) — 계약에 적합한 구현에서 **나가는 바이트가 하나도 달라지지 않는다**. 오늘 Kotlin이 다른 바이트를 내는 것은 이 개정이 만든 차이가 아니라 **개정 전부터 있던 위반**이다 |
| React 영향 | **런타임 영향 없음**(실측). `client.ts`의 `readErrorMessage`가 문자열·배열 **양쪽 분기를 이미 갖는다** — 이 결함은 화면을 깨뜨리지 않고 **문구를 한국어에서 영문(`Input should be a valid UUID`)으로 바꾼다.** F3 판정이 「상태 코드는 그대로라 테스트로도 잡히지 않는 품질 저하」라 부른 형태 |
| 통보 대상 | `kotlin-implementer`(차단 사유 — 구현 결함), `parity-verifier`(**의도된 차이는 늘지 않았다**), `migration-reviewer`, 리더 |

### 바뀐 조항 (`contracts/easy-doc-v1.yaml`)

| 자리 | 변경 |
|---|---|
| `components/schemas/DocumentTextRequest.properties.workspace_id` | **형식 오류 처분을 복기했다** — 422 · 문자열 `detail` · `"작업 공간 식별자 형식이 올바르지 않습니다"` · 값 비반향. 빈 문자열 처분과 404 소유권도 함께. **`format: uuid`가 강제 지점이 아님**과 검사 **순서**의 정본이 어디인지도 적었다 |
| `components/responses/ValidationFailed.description` | 배열 갈래의 「타입 불일치」를 **JSON 원시 타입 불일치**로 한정 + 문자열로 실려 온 값의 **타입 변환 실패는 문자열 갈래**임을 명시 + **경계가 전송 팔을 가리지 않음**을 신설 |
| `x-changelog` | `G29-X8` 항목 신설(맨 앞) |

### 값은 하나도 바꾸지 않았다 — 바꾼 것은 **어디에 적혀 있는가**다

`paths./documents.post.responses.422.description`이 *"`workspace_id` 형식 오류 … 앞의 것들은
문자열 detail"*이라고 **오퍼레이션 수준**으로 이미 적고 있었다. 계약에는 「JSON 팔의 422」와
「multipart 팔의 422」가 따로 없고 `POST /documents`의 422 **하나뿐**이다.

**그래서 이 개정이 없어도 X-8은 오늘 성립하는 구현 결함이다.** 개정의 목적은 판정의 근거를
만드는 것이 아니라 **재발을 막는 것**이다 — 같은 규칙을 한 팔에만 적어 두면 다른 팔에서
지켜지지 않는다는 것이 가설이 아니라 **이 결함 자체가 실측**이다(G4).

⑵는 F3(`x-request-field-constraints.x-decision`, 2026-08-13 · G1+G4)이 이미 한 번 판정한 것과
**같은 종류의 자기모순**이다 — 계약의 두 조항이 한 입력에서 서로 다른 답을 낸다. 그때와 같은
처방을 쓴다: 값을 바꾸지 않고 **어느 조항이 그 자리를 관할하는지**를 적는다.

### 영향받는 계약 테스트 (`04_contract-keeper_documents-test-spec.md`)

**신설 3건 · 축 1개.**

- **DC-29** — JSON 팔 단독 형식 오류 → 422 · 문자열 · 계약 문구와 일치. **오늘 이 입력의 테스트가 0건이다**
- **DC-30** — JSON 팔 복합(본문 길이 초과 + 형식 오류) → 본문 길이 단계의 `detail`. DC-27 ⑵의 JSON 팔 판
- **DC-31** — **두 팔 교차**: 같은 결함에서 두 응답의 `detail` **타입과 값이 같다**를 한 단언에.
  **이 쌍의 본체다** — 두 팔을 각자 계약과 대조하기만 하면, 문면이 한쪽 팔에만 있던 상태로
  되돌아갔을 때 **양쪽 다 초록**이다
- **X-L3 축 신설** — 「`detail`의 모양을 가르는 축은 걸린 규칙의 종류이지 전송 팔이 아니다」

**같은 편집에서 함께 고친 것(계약 무변경)**: `CU-11` **ID 충돌 해소**. 415 케이스가
2026-08-20에 신설되며 이미 쓰이던 `CU-11`(무인증 401)과 같은 ID를 받았고 표의 `CU-10` 앞에
삽입돼 순서도 깨져 있었다. 추적 표 셋이 이 ID를 **서로 다른 의미로** 인용해(§4 X-A1은 401,
§5 P-42와 §6 N-40은 415) 「CU-11 통과」가 무엇을 뜻하는지 정해지지 않았다. 먼저 있던 401이
`CU-11`을 유지하고 415가 **`CU-12`**를 받았다. 인용 3곳을 함께 고쳤다.

**그리고 `PUT /conversions/{id}` 블록의 계약 지목을 노드 경로 정본 + 행 번호 참고로 바꿨다.**
종전 판은 행 번호로만 지목했고 **그 값이 전부 낡아 있었다** — `fields[?edited_text] :420-424`는
오늘 계약에서 **`x-cors`**를 가리킨다. **빈 자리가 아니라 실재하는 다른 절**이라 지목을 따라간
사람이 「없다」가 아니라 **틀린 조항**을 읽는다. 어긋남이 일정 오프셋도 아니라(+420/+480/+378)
한 번에 되밀 수 없다. **이 드리프트는 이미 전파됐다** — 이 작업의 지시문도 낡은 값을 그대로
인용했다. **범위는 C7이 읽는 블록으로 한정했다**(리더 제약: 스펙 전체 재정비 금지).

### 하지 않은 것

- **Kotlin 코드를 고치지 않았다** — X-8의 조치 소재는 `kotlin-implementer`이고 배치는 C7이다.
  이 개정은 계약 문면과 명세만 닿는다. **그때까지 JSON 팔이 계약을 위반한 상태로 남는다.**
- **`x-open-asymmetry`를 해소하지 않았다** — `text`(원시 길이)와 `edited_text`(정규화 후 길이)의
  비대칭은 여전히 미결이고 소유가 parity 레인이다. C7에서 두 필드가 **같은 저장 경로**로
  값을 쌓게 되어 성격이 바뀐다는 것만 판정문 §4-2 ㉠에 적었다.
- **`x-stored-text-domain`의 `edited_text` 팔(`pending`)에 케이스를 세우지 않았다** — 계약이
  「#8을 구현하는 단위가 닫는다」고 적은 자리이고, 리더가 이 편집의 범위를 지목 규격화로
  한정했다. **C7 종료 판정에서 이 팔의 상태 갱신이 빠지면 종료 조건 미충족이다.**
- **음성 대조를 돌리지 않았다.** DC-29~DC-31은 명세일 뿐 오늘 단언이 0건이며,
  「빨개지는지」는 그 케이스를 쓰는 커밋에서만 잴 수 있다.
