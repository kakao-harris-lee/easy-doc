...
**두 갈래 모두 실행으로 관측했다.** 첫 실행은 이 캐시 키의 첫 실행이라 미적중이었으므로
`if: steps.pw-cache.outputs.cache-hit != 'true'` 의 **참** 갈래만 돌았다 — 선언만 있고
도달이 0인 갈래를 남기지 않기 위해 다음 실행에서 **거짓** 갈래를 확인했다.

| 실행 | 캐시 | 브라우저 설치 스텝 | apt 스텝 | e2e |
|---|---|---|---|---|
| [32229496368](https://github.com/kakao-harris-lee/easy-doc/actions/runs/32229496368) (`f3de501`) | 미적중 | 실행 — 14초 | 39초 | ✅ success 3분 53초 |
| [32230037832](https://github.com/kakao-harris-lee/easy-doc/actions/runs/32230037832) (`3ea1983`) | **적중** | **skipped** | **1분 56초** | ✅ success 4분 21초 |

**apt 가 6분 안에 39초 → 1분 56초로 3배 흔들렸다.** 같은 명령·같은 러너 이미지인데
그렇다 — 이것이 "미러 장애에 상한이 없다"는 진단의 직접 증거다. 28분 매달림은 이 분산의
꼬리였을 뿐 특별한 사건이 아니었고, 따라서 **상한이 없으면 재발한다.** 두 실측 모두
6분 상한 안에 들었고 `continue-on-error` 는 아직 한 번도 발동하지 않았다.
