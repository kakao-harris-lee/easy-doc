...
- **[2026-08-20 추가 — 게이트 27 M-5] 이 실행은 `269fe28`(C2 **이전**)에서 돌았다.**
  그래서 §2.3 의 「Flyway 이력 최종 1~4」는 **이미 낡았다** — C2(`6515548`)가 `V5__conversion_jobs.sql`
  을 더했고 `DocumentConfiguration` 이라는 새 조립을 넣었다. **그 조립이 compose 층에서 뜬 적은
  없다.** 이 문서가 닫는 것은 「게이트 26 의 `@Profile("!migrate")` 이후 기동 조건」이고,
  **C2·`d19175b` 이후의 기동은 다시 재야 한다.** 리뷰어가 이 빠진 한계를 짚었고 지적이 옳다.
