...
- **후속 확장(2026-09-09) — 이 문서가 구현한 범위 밖이다.** 사용자 확인으로 이 기능의 **주된 수요**가 밝혀졌다: 담당자가 **개인카드로 결제한 뒤 소속 기관 명의의 증빙**이 필요한 경우다. 개인카드 매출전표로는 기관 예산 정산이 되지 않기 때문이다. 그런데 한 거래에 매출전표와 세금계산서를 겹쳐 발행할 수 없으므로, 그 건은 **카드 결제 취소 → 계좌이체 재수납 → 발행** 순서로 전환해야 한다(크레딧은 회수하지 않는다 — 결제 수단만 바뀐다). 그래서 다음 세 가지가 더 필요하다: ① 요청 화면이 해당 기간의 **결제 수단을 표시**하고 카드 건이면 전환 절차를 안내 ② 어드민 화면에 전환 처리 흐름 ③ 결제 화면이 **결제 전에** 세금계산서 필요 여부를 물어 전환 자체를 줄이는 분기. 상세는 `docs/plans/2026-09-09-signup-to-invoice-process.md` §7.1·§10.5이고, 착수 순서는 master-plan §9 단계 2의 5-2·5-5다.
...
3. 계약 2.22.0, 마이그레이션 V15. 이 묶음은 **2.24.0 · V16**.
...
V16, core `BusinessNumber` 값 객체(정규화·체크섬), application `InvoiceRequestService`(+포트), Jdbc 어댑터, 계약 2.24.0(두 작업, 스키마, 409/422 예시, 변경 기록), 컨트롤러, `AuthenticatedEndpoints`, 메일 두 통, `invoice-handle` 프로필, 프런트 폼·목록, 러너북, backlog. 테스트: 체크섬(유효/무효/하이픈), 409 중복, 422 기간, 404 타 소유자, 메일 발송(FakeMailSender) 및 미설정 운영자 주소, 프로필 3 케이스(발급/거절/이미 처리), 실 DB 삽입·목록, Vitest 폼·목록·오류. → **구현(2026-09-07).**
