...
| REQ-08 | 문맥·원문 근거에 따라 그림이 도움이 되는 부분 제안, 사용자 요청 시 이미지 생성, 검토 후 선택 적용 | R7, ER-17~20 |
| REQ-09 | 소유권·만료·암호화·과금·중복 실행 방지·관측 | 전 단계 |

비범위: 자격 자동 판정, 법령 실시간 검색, 기관명·날짜의 자동 사실 수정, 개인별 장애 진단, OCR, PDF 생성, 협업 권한·공유 링크, 기존 문서 일괄 재생성. R7은 2026-09-24 수정안의 이미지 개별 내려받기를 포함하되, 새 문서 출력 형식이나 DOCX/HWPX 그림 삽입은 별도 범위 결정 전까지 포함하지 않는다.
...
**2026-09-26 설계 정정·미구현:** 아래 §5.1~5.5는 현행 v1의 구현 이력이다. 신규 설계는 [R2 진단·개선 계획](2026-09-26-action-guide-correction.md) §4~5를 우선한다. 안내문 적합성 판단과 근거 있는 행동 추출을 먼저 두고, 단계별 추가 안내와 전체 문서 보완을 구분한다. v1의 6섹션·4,000자·항목별 인용 검증을 전체 보완 계약으로 재사용하지 않는다. 전체 보완은 원문 전체 대응과 의미 보존·유효 편집 내용·변경 대조를 확인하며, 미해결 누락/충돌 상태에서는 전체 반영하지 않는다. 현행 멱등·정산·CAS·암호화·보존·확인 출력 경계는 유지한다. 계약 변경은 ER-28에서 OpenAPI·Kotlin·React API를 함께 반영한다.
...
ER-05~ER-07에는 D03의 크레딧 예약·정산과 D04의 문서당 provider 시작 작업 3회 상한이 구현됐다. 2026-09-21 사용자가 D04와 D03의 정산 경계를 확정했고, D03 산식은 100자 단위 올림 × 0.1크레딧으로 변경했다. R2 제한 노출 전 [검증·출시 계획 §6.1](2026-09-18-easy-read-validation-release.md#61-r2-정책-결정-기록--제한-노출-선행)에 결정과 근거를 기록한다. 확정 산식은 공백 포함 원문 100자 단위 올림 × 0.1크레딧(100자 0.1, 101자 0.2, 1,001자 1.1)이며 그 양을 예약하고, 검증된 안내문 후보를 현재 본문 버전으로 저장해 제공했을 때 소비한다. 제공 전 실패·stale·만료는 예약 반환, 실제 LLM 비용은 실패를 포함해 원장에 남긴다. 제공된 후보를 사용자가 적용하지 않는 경우나 제공 이후 본문 수정으로 stale이 되는 경우에는 자동 환불하지 않는다. 최초 변환·문단 재변환에도 같은 소수 산식을 적용한다. 기존 잔액·제공량·과거 거래는 수치를 보존하고, 진행 중 작업은 저장한 예약량으로 정산한다.
...
R7은 원문·쉬운 글·주변 조건을 함께 분석해 그림으로 설명하면 도움이 되는 문맥만 제안한다. 제안은 원문 근거·대상 문맥·도움이 되는 이유·그림 구성·보존할 조건과 본문 버전을 가진다. 적절한 문맥이 없으면 제안하지 않는다. 고정 카탈로그와 낱말 매핑은 이 요구사항을 충족하지 않는다.

이미지는 사용자의 명시적 생성 요청 후에만 만든다. 요청 키·본문/제안 버전으로 중복과 stale을 제어하고, 작업 조회·암호화된 비공개 이미지 저장·만료/삭제·이용량 정산을 구현한다. 생성 완료와 문서 적용은 분리하며 미리보기·대체텍스트 확인 후 사용자가 적용한다. 문서 소유자만 이미지 조회/개별 내려받기가 가능하다. 공개 카탈로그 이미지 API를 사용자 생성 이미지에 재사용하지 않는다.
...
R5 스냅샷은 문서당 최근 20개, 각 파생 payload는 암호화 전 UTF-8 64KiB 이하로 제한한다. 초기의 ‘20버전’ 제안을 2026-09-21 구현에서 스냅샷 20개로 구체화했다. 본문 버전만 제한하면 같은 본문의 반복 검수에서 저장량이 무한히 증가하기 때문이다. 초과 기록은 오래된 본문 스냅샷부터 제거하되 남은 이벤트는 ‘본문 스냅샷 없음’으로 표시한다. 전체 문서 보존 만료 시 이벤트도 함께 삭제한다. 법적 장기 감사 보존을 제공한다고 주장하지 않는다.

R7의 이미지 바이너리는 UTF-8 텍스트 payload 상한과 별도로 제한한다. 해상도·파일 크기·문서당 자산 수와 총 저장량은 D10에서 확정하고 접수/저장 시 모두 집행한다. 미정 상태에서 무제한 저장이나 실제 provider 활성화를 허용하지 않는다.
