...
1. `/usage`의 실제 가격표에서 Start·Basic·Pro를 선택해 비교한다. 테스트 카드 등록은 Start(월 99,000원/50크레딧)만 가능하며 Basic·Pro는 결제 준비 중으로 표시한다.
...
V25는 가격 확정 전의 활성 `starter`/`pro` 테스트 구독을 `start`로 옮기고 다음 테스트 갱신 금액·제공량을 99,000원·50크레딧으로 맞춘다. 과거 결제 내역은 당시 테스트 기록으로 보존한다.
...
아래 1,000원 기록은 가격 확정 전 실행한 역사적 검증 결과다. 현재 Start 테스트 결제 금액 99,000원의 재검증 결과는 아래 2026-09-14 절에 따로 기록한다.
...
## Start 99,000원 재검증 (2026-09-14)

본인 상점 테스트 키, 합성 카드, 일회용 PostgreSQL, fake LLM·메일로 외부 Toss E2E를 다시 실행했다. Start 99,000원 최초 테스트 승인과 월 갱신 승인, 영수증 조회, 각 승인 건의 40,000원 부분 환불과 59,000원 잔액 환불, 동일 환불 요청의 멱등 재전송, 구독 갱신 중단과 빌링키 삭제까지 1.1분에 통과했다. 최종 실행의 테스트 승인 2건은 모두 전액 환불했으며 실제 출금은 없다.

같은 날 집행 전용 E23도 추가했다. stub Start 구독으로 월 50크레딧을 설정하고 공백 포함 10,000자 요청 5건을 처리해 `40 → 30 → 20 → 10 → 0` 차감, worker 소비 확정, 다음 10크레딧 요청의 402를 실 React·Kotlin API·PostgreSQL·worker 흐름으로 검증한다.
