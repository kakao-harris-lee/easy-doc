...
- **A1 백엔드·계약 (L).** V17(세 변경 + 공지 표), `admin-grant` 프로필, `AdminGuard`(+403 매핑), `AdminQueryService`(목록·상세·오류·사용량 — 기존 서비스 조합, 새 조회는 Jdbc 어댑터), 크레딧 조정·세금계산서 처리에 actor 기록, 공지 서비스, 계약 2.25.0(`x-admin-only`, 11개 작업, `UserResponse.is_admin`), `AuthenticationCoverageContractTest` 확장, 인구조사. 테스트: 비관리자 403(미검증 관리자 포함), 목록 검색·페이지, 상세 본문 부재, 크레딧 조정 actor 기록, 처리 handled_by, 오류 집계, 공지 CRUD, `admin-grant` 3 케이스.
