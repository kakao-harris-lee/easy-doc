...
- 기준: `docs/master-plan.md`(크레딧 = 공백 포함 1,000자 = 1크레딧, Start 50 / Basic 200 / Pro 1,000 월 구독). 가격과 결제의 후속 정본은 `docs/plans/2026-09-13-service-pricing.md`다.
...
1. **계정 단위는 워크스페이스다.** 플랜(Start/Basic/Pro)은 워크스페이스에 붙는 개념이고, 팀원은 같은 잔액을 쓴다. 사용자 단위 잔액은 만들지 않는다.
2. **표 두 개(V15).**
...
9. **만료·플랜 자동화·PG·어드민 화면은 범위 밖.** 월 크레딧 만료(이월 없음)는 플랜 결정이 필요하므로 backlog에 남긴다. → **2026-09-08 확정: 만료 없음·수동 월초 부여(§7). → 2026-09-09 만료 부분 번복: 크레딧은 만료된다(§7 결정 1의 경고 참조).**
...
3. **가입 부여 50.** `EASYDOC_CREDITS_SIGNUP_GRANT=50`(Start 한 달치). 기본 워크스페이스 생성 시 1회 `grant/signup`. e2e 스택은 제 값(E21 1000, E22 1)을 그대로 쓴다.
