...
> **2026-09-12 변경:** 카드 전용 정책과 증빙 기준은 `2026-09-10-legal-tax-policy-final.md` §3.7을 따른다. 아래의 사용자 세금계산서 요청·계좌이체·카드→이체 전환 제안은 폐기되었고 현재 기능/출시 요건으로 사용하지 않는다. 기존 구현·보존 이력 설명은 남긴다.

- 작성 2026-09-07. 사용자 결정(2026-09-07): 「1·2·3 묶음으로 진행, 권고대로 결정」.
- 기준: `docs/master-plan.md` §3.3(호출 상한)·§4.1 P0-11(어드민)·크레딧 정책(1,000자 = 1크레딧, Start 50 / Basic 200 / Pro 1,000). 가격과 PG 판단은 `docs/plans/2026-09-13-service-pricing.md`가 후속 정본이다.
...
6. 계약은 2.19.0, 마이그레이션은 V11까지. 이 묶음은 **2.20.0 · V14**.

## 2. 결정

1. **원장 표 `llm_calls`(V14)** — 호출 1건 = 행 1건. 열: `id uuid`, `conversion_id uuid NULL`·`document_id uuid NULL`(**FK 없음** — 2026-09-07 U2 리뷰 정정: 참조 행이 지워져도 값이 남아 원장이 청구 근거로 자족한다), `workspace_id uuid NULL` (`FK workspaces ON DELETE SET NULL` — 2026-09-07 리뷰 정정: 워크스페이스 삭제가 청구 근거를 지우면 안 된다), `user_id uuid NOT NULL` (`FK users ON DELETE CASCADE`), `purpose varchar(16)` (`convert | repair | reconvert`), `provider varchar(32)`, `model varchar(128)`, `input_tokens int`, `output_tokens int`, `latency_ms int NULL`, `estimated_cost_usd numeric(12,6) NULL`, `pricing_input_usd_per_mtok numeric(12,6) NULL`, `pricing_output_usd_per_mtok numeric(12,6) NULL`, `document_char_count int NOT NULL`(등록 시 `documents.char_count` 스냅샷 — 문서·문자 집계의 근거), `char_count int NOT NULL`(그 호출의 **프롬프트 입력 길이** — 변환·보정은 문서 전체, 재변환은 단위 길이; `documents.char_count`와 다르고 변환+보정 행이 같은 길이를 두 번 실으므로 문서 단위로 합산하지 않는다. **2026-09-07 정정: PR #58로 개인정보 마스킹이 제거돼 「마스킹된」 표현을 걷어내고 평문 프롬프트 입력 길이로 재정의했다.**), `called_at timestamptz`(**호출이 끝난 시각** — 저장 시각이 아니다; 집계 경계 §2.5가 이 값을 쓴다). 색인 `(workspace_id, called_at)`. **본문·프롬프트·응답·마스킹 항목은 절대 넣지 않는다** — 원장은 숫자와 이름뿐이라 암호화 대상이 아니고 `EncryptedField` 인구조사에 걸리지 않는다.
...
2. **기록 위치는 application이다** — decorator가 아니다. decorator는 변환 맥락(어느 문서·워크스페이스·목적)을 모른다. `ConvertDocumentUseCase`가 반환하는 `ConversionUsage`에 호출 목록 `calls: List<LlmCallRecord>`(purpose·provider·model·토큰·지연·비용·단가 스냅샷)를 더하고, `ProcessConversionJob`은 완료 저장과 **같은 트랜잭션**에서, `ReconvertUnitService`는 정산 트랜잭션에서 `LlmCallLedger.append(...)` 포트로 기록한다. **완료된 호출은 결과와 무관하게 기록한다** — 거부(REFUSAL)·절단·빈 결과처럼 파이프라인이 실패로 분류한 완료도 토큰을 썼으므로 남기고, 재시도 예정 여부와 무관하다(2026-09-07 리뷰). **2026-09-08 정정(백로그 「실패 호출 원장 추적」) — 이 배제는 뒤집혔다.** 완료 자체가 없는 provider 예외도 이제 기록한다: `outcome`(V18, `completed | provider_error`) 열이 갈래를 구분하고, provider 예외 행은 토큰 0·비용 `null`·`failure_class`(예외 클래스의 단순 이름, 메시지는 담지 않는다)로 남는다 — 벤더가 실패 입력에 과금해도 이제 원장 **안에서** 그 차이를 대조할 수 있다(§6 리스크 1 정정). 집계(U2·U3)는 `outcome = completed`인 행만 문서·문자·토큰·비용에 합산하고 실패 건수는 `failed_calls`로 따로 낸다. 크레딧은 실제 소비 확정 거래를 별도로 합산한다.
...
5. **집계 단위·시간대.** 워크스페이스 × `[from, to]` 날짜 구간(포함), 경계는 `easydoc.usage.zone`(기본 `Asia/Seoul`)의 자정. `documents`·`characters`·호출/토큰/비용은 `llm_calls`에서 센다(2026-09-07 U2 리뷰 정정 — `documents` 표는 보존 만료·사용자 삭제로 비므로 청구 근거가 될 수 없다): `documents`(그 기간에 완료된 LLM 호출이 한 번이라도 있던 **서로 다른 `document_id`** 수), `characters`(그 문서들의 `document_char_count` 합, 문서별 1회), `llm_calls`, `input_tokens`, `output_tokens`, `estimated_cost_usd`(문자열 소수, 알려진 행 합), `cost_unknown_calls`, `failed_calls`, `by_purpose[...]`. `credits`는 같은 기간 `credit_transactions`의 `consume/conversion` 차감량 합이라 최초 변환과 성공한 재변환을 모두 센다. 거래가 전혀 없는 V15 이전 데이터만 문서별 `ceil(document_char_count/1000)` 합으로 대체한다.
...
7. **운영 리포트 프로필 `usage-report`.** `rotate-keys`와 같은 방식으로 기동해 CSV를 쓰고 종료한다: `--from --to --out` (기본 지난달, `./usage-report.csv`). 행 = 워크스페이스 1건: `workspace_id, workspace_name, owner_email, documents, characters, credits, llm_calls, failed_calls, input_tokens, output_tokens, estimated_cost_usd, cost_unknown_calls`(`failed_calls`는 2026-09-08 정정, `llm_calls` 바로 뒤). 소유자 이메일은 청구서 발송에 필요한 운영 출력이므로 싣되, 로그에는 남기지 않는다. 종료 코드 0/1.
...
- **U1 원장 (M, core·application·infrastructure·worker) → 구현(2026-09-07).** V14, `LlmCallRecord`·`LlmCallPurpose`(core), `LlmCallLedger` 포트 + Jdbc 어댑터, `ConvertDocumentUseCase`가 호출 목록을 반환(합산 필드는 그대로), `ProcessConversionJob`·`ReconvertUnitService`가 기록, 모델별 단가 + 스냅샷, 기존 로그 관측 불변. 테스트: 변환 1회·보정 포함 2회·재변환 1회 각각 행 수·purpose·비용; 단가 미상 `null`; 실 Postgres에서 문서 삭제 후 행 잔존(`SET NULL`); `SensitiveToStringReachTest` 인구조사; `OwnershipPredicateGuardTest`는 대상이 아님(암호 열 없음)을 KDoc에 적는다.
...
- 문서를 보존 만료로 지우거나 사용자가 삭제한 뒤에도, 그리고 워크스페이스를 지운 뒤 사용자 단위 리포트에서도, 그 달의 집계(문서·문자·크레딧·호출·토큰·비용·실패 호출)가 같다.
...
1. ~~벤더가 실패 호출의 입력 토큰에 과금하면 원장이 실제보다 적게 센다~~ — **2026-09-08 해소(백로그 「실패 호출 원장 추적」).** provider 예외도 이제 `llm_calls`에 `outcome = provider_error` 행으로 남는다(§2 결정 2 정정). 실제 토큰 수는 여전히 모르지만(완성 응답이 없어 알 수 없다), 최소한 **몇 건이 실패했는지**는 원장에서 바로 세어 벤더 청구서와 대조할 실마리로 쓸 수 있다 — 이전에는 구조화 로그를 따로 훑어야 했다.
