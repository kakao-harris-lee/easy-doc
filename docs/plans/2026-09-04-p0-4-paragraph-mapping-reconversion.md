...
아래 두 항목 모두 **확정(2026-09-05, 사용자)**됐다. 나머지는 모두 이 문서에서 **확정**이며 선택지로 남기지 않는다.

1. **재변환 호출 예산 기본값 — 확정(2026-09-05, 사용자).** `easydoc.reconversion.call-budget` 기본값 **20회**(문서 1건당 LLM 호출 수). 비용 정책이라 숫자는 사용자가 정한다.
2. **master-plan §3.2 문구 개정 승인 — 확정(2026-09-05, 사용자).** §4의 개정문을 그대로 반영한다.

**S1~S3는 이 게이트와 무관하게 착수한다. S4·S5는 두 항목이 확인되기 전에 시작하지 않는다** — 유료 호출의 상한을 정하는 일이라, 확인 없이 구현하면 되돌릴 대상이 코드가 아니라 나간 비용이다. **두 항목이 모두 확정됐으므로 S4·S5는 이제 착수 가능하다.**
...
**2026-09-13 과금 후속:** 재변환도 대상 원문 단위의 `ceil(char_count / 1000)`크레딧을 예약한다. 후보 생성 성공 시 소비하고, provider 실패·동시성 거절·호출 예산 소진 시 반환한다. 잔액 부족은 LLM 호출과 호출 예산 예약 전에 402로 끝난다.
...
3. **지도로 짝짓는 규칙 — 줄마다 갈 곳이 정확히 하나라는 불변식은 그대로다.**
   - **빈 줄 투영(2026-09-06 리뷰 F2).** 지도의 쉬운 글 쪽은 `splitUnits`(= `\n` 분할, 빈 줄
     **유지** — 조회 응답 `segment_map`과 S3 에디터가 이 색인을 쓴다)이고, 내보내기의 줄은
     `exportContentLines`(빈 줄 **제거**)다. 지도를 바꾸지 않는다 — 계약 파손이다. 대신 S6-2가
     지도의 쉬운 글 색인을 **빈 단위를 건너뛰어** 내보내기 줄 목록에 투영한다
     (`easyUnits.indices.filter { easyUnits[it].isNotBlank() }`). 아래 개수 검사는 이 투영
     **뒤**의 값으로 한다.
   - 전제 검사: `map.sourceUnitCount == units.size`(내보내기 순회가 본 단위 수)이고
     투영된 쉬운 글 단위 수 `== lines.size`. 어긋나면 **지도를 버리고 차례 짝짓기로 떨어진다**
     (`placement = ORDINAL_FALLBACK`, 아래 결과 항목). §7 리스크 2(추출 줄 수 ≠ 내보내기 단위
     수)가 여기서 드러나며, 그 사실은 응답 `details`로 나간다 — 조용히 넘기지 않는다.
   ...
   - **원본 색인이 비어 있는 줄**(`sourceUnitIndexes = []`, 퇴화 지도)은 끝에 덧붙인다
     (`appendedLines`).
   - **`LOW` confidence 줄**은 위 규칙대로 자리에 넣되, **보간이 차례 짝짓기와 다른 자리에 놓은
     줄만** `lowConfidenceLines`로 센다(2026-09-06 리뷰 F1 정정). 면제 조건은 셋이 모두 참일 때다:
     원본 색인이 하나이고(`sources.size == 1`), 그 색인이 투영된 쉬운 글 색인과 같고, 그 단위에
     **갈아 끼워졌다**(끼워 넣기가 아니다). 이유: 앵커가 하나도 없는 문서는 지도가 전부 `LOW`인데
     개수가 같으면 지도의 결론이 차례 짝짓기와 **바이트 단위로 같다** — 오늘 `available`이던
     문서를 같은 파일을 만들면서 `partial`로 낮추는 것은 정보가 아니라 상수다. 보간이 실제로
     자리를 옮긴 줄만 짐작이며, 그 수만큼이 `partial`의 근거다. 남는 위험(앵커가 없어 합침·나눔을
     증명 못 한 채 개수만 맞는 문서)은 오늘과 동일하고, 검수 화면이 단위별로 `low`를 이미 보여 준다.
...
4. **판정(`reflectedPreservation`)의 새 어휘.** `ReflectionOutcome`에 `mergedUnits`·`splitLines`·
   `lowConfidenceLines`와 **`placement: ReflectionPlacement`** (`ORDINAL` = 지도 없이 차례
   짝짓기, 오늘의 기본값 · `MAPPED` = 지도로 놓음 · `ORDINAL_FALLBACK` = 지도를 받았으나
   전제 검사에 실패해 차례로 떨어짐)를 더한다. boolean 플래그(`ordinalFallback`) 대신 세 값
   enum인 이유(2026-09-06 리뷰 F1): S6-1 단독으로도 오늘의 판정이 바이트 그대로 보존돼야
   하는데, 두 값으로는 「지도 없음」과 「지도 거절」을 구분할 수 없어 둘 중 하나의 문구가 틀린다.
   상태 규칙:
   - `available` — 머리말·꼬리말(오늘과 같음)·옮김·비움·덧붙임·저확신이 모두 0이고
     `placement != ORDINAL_FALLBACK`. **합침과 나눔은 `HIGH`면 `available`을 깨지 않는다** —
     대응을 확신한 반영이다. 단 무슨 일이 있었는지는 `details`로 말한다. 따라서 **2.16.0부터
     `available`에도 `details`가 붙을 수 있다**(오늘은 비어 있고, 계약 3726행 근처의 「`available`
     에서는 빈 배열」 문구가 S6-3에서 함께 바뀐다 — 그래서 S6-3 없이 S6-2가 main에 가지 않는다).
     §10.4의 「`available` + details 1건」 픽스처는 머리말·꼬리말이 없는 원본에서만 성립한다.
   - `partial` — 그 밖의 모든 경우. 새 문구(개수만, 문서 문자열 없음):
     「원본 문단 N개는 앞 문단과 합쳐져 빈 문단으로 남습니다」, 「문단 N개는 원본 문단이 나뉘어
     그 뒤에 새 문단으로 들어갑니다」, 「문단 N개는 원본 자리를 확신할 수 없어 앞뒤 비율로 자리를
     옮겨 넣었습니다」(저확신 — 위 면제 규칙으로 실제로 옮긴 줄만 센다), 「원본 구조와 문단 수를
     맞출 수 없어 차례대로 반영합니다」(폴백).
     `SHIFTED_DETAIL`(「밀릴 수 있습니다」)은 `placement != MAPPED`이고 비움·덧붙임이 있을 때
     (오늘의 규칙 그대로) 그리고 `ORDINAL_FALLBACK`이면 언제나 붙는다 — **`MAPPED`에서는 절대**
     붙지 않는다, 지도로 놓은 문단은 밀리지 않기 때문이다. 폴백 문구(「차례대로 반영합니다」)는
     `ORDINAL_FALLBACK`에서만 붙는다.
...
## 11. S7 스팩 — `segment_map.compliant_source_units` (2026-09-06 착수 결정)

§5 「이미 쉬운 글인 입력의 저변경 보장」이 미뤄 둔 자리다. S6(PR #45)·연결 해제(PR #47, 계약
2.17.0)가 병합돼 계약 파일 충돌이 사라진 뒤 사용자가 착수를 결정했다(2026-09-06). 계약은
**2.18.0**, 마이그레이션 없음.

### 11.1 무엇을 더하는가

`SegmentMap`에 **`compliant_source_units: integer[]`(required)** 하나. 저장된 추출 원문을
`\n`으로 쪼갠 원본 단위 중 **`checkStyle`(core/easyread/StyleRules.kt)이 위반 0건으로 통과하고
공백만이 아닌** 단위의 0 기반 색인을 **오름차순·중복 없이** 담는다. 공백만인 단위는 통과로 세지
않는다 — 재변환할 것도 경고할 것도 없는 줄이라 목록에 들어가면 화면이 빈 줄에 배지를 단다.

- **원천은 마스킹 전 원문이다.** 검사 대상은 사용자가 화면에서 읽는 문단이지 LLM이 본 마스킹
  본문이 아니다. `maskText`는 줄 수를 바꾸지 않으므로 색인은 `units[].source_unit_indexes`와
  같은 좌표다. `MaskedSegmentMapDerivation.deriveOrNull`이 이미 복호화한 원문을 갖고 있으므로
  거기서 함께 계산한다 — 조회와 내보내기가 같은 인스턴스를 지나므로 「지도의 원천은 하나」
  (§10.2 결정 1)도 그대로 참이다. 내보내기는 이 필드를 읽지 않는다.
- **core에 둔다.** `core/segment`에 `compliantSourceUnits(sourceUnits: List<String>): List<Int>`
  순수 함수를 두고 `SegmentMap`에 `compliantSourceUnits: List<Int> = emptyList()` 필드를 더한다.
  `alignSegments`는 정렬만 알고 이 값을 채우지 않는다(기본값) — 채우는 곳은 파생 함수 하나다.
  `SegmentMap.toString`은 지금처럼 본문을 찍지 않는다(색인만 늘어난다).
- **저장하지 않는다.** `segment_map`의 다른 필드와 같이 매 조회마다 유도한다.

### 11.2 화면

원본 패널(`SourceTextPanel`) 단위 목록 모드에서, 색인이 목록에 있는 행에 **「이미 쉬운 글
규칙을 통과한 문단」** 표시를 단다. 재변환 버튼은 **막지 않는다** — 사용자의 판단이지 서버의
금지가 아니다. 대신 버튼의 접근 가능한 설명(`aria-describedby`)과 `title`로 「이 문단은 이미
쉬운 글 규칙을 통과합니다. 다시 쓰면 나빠질 수 있습니다.」를 잇는다. 표시는 배지 하나로
충분하고 색만으로 뜻을 전하지 않는다(텍스트가 있다). `segment_map: null`인 단일 textarea
갈래는 바뀌지 않는다.

클라이언트는 이 목록을 다시 계산하지 않는다 — 원문은 읽기 전용이라 낡지 않는다.

### 11.3 수용 기준

- **C1 core:** `["짧은 문장입니다.", "", "   ", <60자 넘는 문장>]` → `[0]`. 빈·공백 단위와
  위반 단위는 빠진다. 결과는 오름차순·중복 없음.
- **C2 파생:** `MaskedSegmentMapDerivation`이 낸 `SegmentMap.compliantSourceUnits`가 C1과 같은
  규칙을 따르고, `deriveOrNull(null, …)`·`deriveOrNull(…, null)`은 여전히 `null`이다.
  `alignSegments` 직접 호출 결과는 `emptyList()`다.
- **C3 계약(api):** `ConversionReadContractTest`의 `segment_map` 키 집합 단언이 새 required
  키를 포함해 통과한다. `SensitiveToStringReachTest`에 걸리지 않는다(정수만 나른다).
- **C4 화면:** 목록에 있는 행에만 배지가 있고, 그 행의 재변환 버튼은 여전히 활성이며
  `aria-describedby`가 경고 문구 요소를 가리킨다. 목록이 빈 배열이면 배지가 하나도 없다.
- **C5 CPU 기록(§5 약속):** 20,000자 원문(단위 약 200개)에 대한 `compliantSourceUnits` 1회
  소요를 로컬에서 재어 이 절 아래에 숫자로 남긴다. 조회마다 도는 계산이므로 100ms를 크게
  넘으면 캐시 여부를 다시 판단한다 — 이번 조각은 측정만 한다.

  **로컬 실측 2026-09-06: 원문 18,842자·200단위, 워밍업 20회 뒤 11회 표본의 중앙값 ≈24ms**
  (Apple Silicon, `kotlinc`로 `core` jar 를 링크해 별도 `main` 함수로 측정 — 이 측정 스크립트는
  커밋하지 않았다). 100ms 문턱에 크게 못 미쳐 이번 슬라이스에서는 캐시가 필요 없다.

### 11.4 범위 밖

- 원본 단위별 위반 사유·개수를 응답에 싣는 것(정수 색인 목록만).
- 자동 변환을 단위별로 쪼개 통과 문단을 건너뛰는 것(§5 그대로).
- 재변환 후보(`reconvertUnit` 응답)에 채택 판정·위반 수를 싣는 것.
