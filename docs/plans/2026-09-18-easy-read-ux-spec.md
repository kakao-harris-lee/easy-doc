...
- 확정된 요금 정책에 따른 ‘필요 이용량 X크레딧 / 남은 이용량 Y크레딧’을 표시한다. D03 미확정 또는 서버 기능 OFF이면 생성 버튼 자체를 공개하지 않는다.
...
| R7 | 선택 가능한 검수된 그림, 본문과 함께 미리보기 | 텍스트만으로도 이해 가능. 웹 표시와 파일 포함 여부를 구분 |
...
- 확인/생성 모달은 이름·설명·초기 포커스·닫은 뒤 포커스 복귀를 갖춘다. Escape로 닫아도 서버 작업 취소를 뜻하지 않는다.
