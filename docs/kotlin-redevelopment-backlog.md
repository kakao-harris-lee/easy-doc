...
- **판정 — 확정(사용자 확정, 2026-09-01): 상한값 재조정으로 끝낸다. 분할 변환은 착수하지 않는다.** 근거: ⑴ 분할 변환의 동기였던 「`MAX_CONVERTIBLE_CHARS`만 올리면 장문이 전부 절단 실패로 떨어진다」는 구조가 1·2차 두 회차 모두 재현되지 않았다 — 2만 자급 문서(022)도 단일 호출 출력이 제품 상한 16,000 안에 들어왔다(11,468토큰). ⑵ **장문일수록 팽창비가 낮다**(022=0.52 < 050=1.21 < 023=1.28 < 047=1.35) — 상한을 올려도 출력이 비례해 커지지 않는다. ⑶ 장문의 사실 보존이 022 변환문 실물로 확인됐다(위). 아래 1~5(분할 단위·마스킹 일관성·부분 실패·크레딧 환산·계약 변경 범위)는 **이 판정으로 착수하지 않게 됐다** — 내용은 지우지 않고 남기되, 열리지 않은 갈래로 읽는다. 남는 실제 작업은 **6번(새 상한값)** 하나뿐이다. **새 상한값은 이 기록에서 정하지 않는다** — 데이터가 지지하는 범위만 적으면, 관측된 최대 입력은 21,926자였고 그때 단일 호출 출력이 11,468토큰(제품 상한 16,000의 72%)이었다. **그 구간의 표본은 1건뿐이다.** 상한을 바꾸면 이미 적어 둔 대로 `MAX_CONVERTIBLE_CHARS`·계약 `x-input-limits.max_review_chars`·프런트 안내가 같은 변경 단위다.

아래는 **분할 변환으로 가기로 판정된 경우에만** 답해야 하는 것들이다. **→ 위 판정(2026-09-01, 상한값 재조정)으로 이 갈래는 착수하지 않는다 — 내용은 기록으로 남긴다.**
