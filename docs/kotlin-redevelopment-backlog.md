...
| 보존 기간 만료를 **조회 질의가 보지 않는다** (원문 조회만 예외) | `GET /documents/{document_id}/source`만 `retention_expires_at > now()`를 건다 | 파기는 `JdbcExpiredDocumentPurge`가 실제로 행을 지울 때 일어나므로 만료 시각과 다음 배치 사이에 창이 열린다. 그 창에서 **`GET /documents`(목록)·`GET /conversions/{conversion_id}`(조회)·`PUT /conversions/{conversion_id}`(검수 저장)·`GET /conversions/{conversion_id}/export`(내보내기)는 만료된 문서의 내용을 그대로 내준다** — `JdbcDocumentRepository.listSql`·`JdbcConversionRepository`의 어느 질의도 그 열을 보지 않는다. **원문 조회만 먼저 닫은 이유는 노출 크기다**: 그 응답은 마스킹 전 문서 **전문**이라(계약 `x-private-response-headers.applies_to`) 같은 창에서 나가는 양이 다른 경로와 다르다 — 목록은 제목뿐이고, 변환 조회·내보내기는 마스킹을 거친 결과와 이미 골라낸 `masked_items`다. 여러 오퍼레이션의 계약 의미를 한 번에 바꾸는 것은 별개 변경이라 이번 범위에서 뺐다. 판단이 필요한 것: ⑴ 나머지 넷에도 같은 술어를 걸 것인가(그러면 만료 직후 검수 화면이 통째로 404가 된다 — 사용자에게는 「배치가 아직 안 돌았을 뿐」과 구분되지 않는다) ⑵ 배치 주기를 줄여 창 자체를 좁힐 것인가 ⑶ 만료를 조회 시점에 파기로 승격시킬 것인가(조회가 쓰기를 하게 되므로 지금의 「조회는 잠그지 않는다」가 깨진다). **master-plan §3.2의 「30일 뒤 자동 삭제」가 약속이므로 셋 중 하나는 골라야 한다** |
