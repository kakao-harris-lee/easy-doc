...
| 크레딧 계정·차감(워크스페이스 잔액, 등록 시 예약, 완료 시 소비, 402 거절) | **구현(2026-09-07, C1+C2, 계약 2.22.0)** — 백엔드·API·운영 진입점·프런트·e2e(비집행+집행) | 계획 `docs/plans/2026-09-07-credit-accounts.md`. V15(`workspace_credit_accounts`·`credit_transactions`·`conversions.credits_reserved`), `GET /workspaces/{workspace_id}/credits`, `createDocument` 402, `credit-grant` 운영 프로필(계좌이체 확인 후 수동 부여, 러너북 「크레딧 충전」, 소유자 조회+부여는 한 트랜잭션), `/usage` 크레딧 카드·거래 표, 업로드 화면 필요/가용·402 안내. **e2e E21은 비집행 변형으로 존재한다**(`frontend/e2e/credits.spec.ts`, `compose.e2e.yml`의 `EASYDOC_CREDITS_SIGNUP_GRANT=1000`) — 가입 부여·예약이 화면·202 헤더에 보이는지만 잰다. **e2e E22는 집행 켜짐 변형으로 존재한다**(`frontend/e2e/credits-enforced.spec.ts`, `compose.e2e-enforced.yml`의 `EASYDOC_CREDITS_ENFORCED=true`·`EASYDOC_CREDITS_SIGNUP_GRANT=1`, `playwright.enforced.config.ts`로 별도 스택·별도 CI 단계에서 이 스펙 하나만 돈다) — 가입 부여 1크레딧 소진 후 두 번째 등록의 402(`X-Credits-Required`·`X-Credit-Balance`)와 화면·`/usage` 반영을 잰다. **플랜 정책 확정(2026-09-08, 사용자):** 만료·이월 없음, 월초 수동 `credit-grant`, 가입 부여 50, 부여 직후 집행, 관리자 초대는 CLI 유지 — 계획 §7. 코드 변경 없음(`.env.example`에 `EASYDOC_CREDITS_SIGNUP_GRANT`·`EASYDOC_CREDITS_ENFORCED` 문서화). |
