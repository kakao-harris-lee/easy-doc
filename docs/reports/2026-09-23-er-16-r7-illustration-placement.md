...
- **CI 회귀 1건과 수정.** 첫 PR CI에서 e2e E17(320px에서 모든 조작 요소가 44px 이상)이 검수 화면에서 실패했다. 이 PR이 더한 `<select>`(`h-10`, 40px)와 「그림 배치 저장」 버튼(기본 크기 40px)이 원인이었고, 그림 스펙은 CI에서 skip되지만 백엔드 토글이 켜져 패널은 렌더되므로 E17이 잡았다. 로컬에서 배치 스펙만 돌리고 E17을 함께 돌리지 않아 놓친 것이다. 둘 다 `min-h-11`(44px)로 고치고 회귀 테스트(클래스 단언)를 더했다. 수정 후 로컬 스택에서 `--grep "E17|그림 배치"` **2 통과 / 0 실패**.
