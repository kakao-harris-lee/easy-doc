...
후속 수정의 프런트 check·845개 테스트·build와 E17/행동 안내 총 3개 E2E가 모두 통과했다. 그림 제안 토글을 true로 명시했고 E17에서 문제 버튼 노출을 확인했으므로 기능이 꺼진 경로의 통과를 근거로 삼지 않았다. 원격 CI는 최종 후속 커밋에서 재실행한다.
