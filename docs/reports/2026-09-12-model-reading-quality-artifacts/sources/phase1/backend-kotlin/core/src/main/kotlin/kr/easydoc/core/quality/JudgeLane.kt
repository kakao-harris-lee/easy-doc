...
/**
 * 변환문이 의미를 보존하며 고학년 수준에서 이해 가능한지 기존 평가 호출에서 함께 묻는다.
 * 호출 여부는 [JudgeLane] 이 정한다 — 이 클래스를 비밀값 없이 만들지 마라.
 */
class GoldenJudge(private val provider: LlmProvider, private val options: LlmOptions = LlmOptions()) {
    fun score(
        document: GoldenDocument,
        converted: String,
    ): JudgeScore {
        val completion = provider.complete(LlmPrompt.forJudge(document.sourceText, converted, document.requiredFacts), options)
        val text = completion.text
        val passed = !completion.truncated && isYes(text)
        return JudgeScore(document.id, passed = passed, reason = if (passed) null else reasonLineOf(text))
    }

    private fun isYes(text: String): Boolean = text.trim().equals("yes", ignoreCase = true)
