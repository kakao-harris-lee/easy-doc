...
/** Responses API의 추론 강도. 미설정이면 기존 비추론 모델 요청도 그대로 유지한다. */
enum class OpenAiEffort(val wire: String) {
    NONE("none"),
    MINIMAL("minimal"),
    LOW("low"),
    MEDIUM("medium"),
...
class OpenAiProvider(private val settings: OpenAiSettings) : LlmProvider {
    ...
    override fun toString(): String = "OpenAiProvider(model=${settings.model}, effort=${settings.effort})"
    ...
    private fun requestBody(
        ...
        put("store", false)
        settings.effort?.let { putObject("reasoning").put("effort", it.wire) }
        // GPT-5.6 이후 기본 캐시 쓰기는 추가 과금된다. 단발 변환은 명시적 경계 없이
        // explicit 모드를 써서 캐시 읽기·쓰기를 하지 않는다(기존 모델의 요청은 유지).
        if (settings.model.startsWith("gpt-5.6") || settings.model.startsWith("gpt-6")) {
