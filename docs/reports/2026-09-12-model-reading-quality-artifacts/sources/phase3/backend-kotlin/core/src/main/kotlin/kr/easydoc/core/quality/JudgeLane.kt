...
class GoldenJudge(
    ...
    private val options: LlmOptions = LlmOptions(),
) {
    fun score(
        document: GoldenDocument,
        converted: String,
    ): JudgeScore {
        val prompt = LlmPrompt.forJudge(document.sourceText, converted, document.requiredFacts)
        val completion = provider.complete(prompt, options)
