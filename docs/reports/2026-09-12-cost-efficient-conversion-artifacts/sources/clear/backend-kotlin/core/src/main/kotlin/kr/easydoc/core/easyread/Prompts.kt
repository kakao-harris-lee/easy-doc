...
/** 저비용 모델의 설명 생략 실측 후, 일반 뜻 설명과 사업 사실 추가의 경계를 분리한다. */
internal val EXPLAIN_INSTRUCTION =
    """
    독자가 초등학교 5~6학년이라는 전제로, 자격·선택·신청을 이해하는 데 필요한 낯선 말을 먼저 설명하세요. 사전 목록에 있는 말만 설명하면 안 됩니다.
    일반적인 단어·단위·관계의 뜻은 원문에 정의가 없어도 정확한 기초 지식으로 설명할 수 있습니다. 사업의 자격·금액·기간·절차를 새로 정하는 것과 구분하세요.
    사람이나 기관은 무슨 일을 하는지, 서류는 무엇을 확인하거나 요청하는지, 기준은 무엇과 무엇을 비교하는지 설명하세요. 범주 이름이나 어려운 동의어만 붙이는 것은 설명이 아닙니다.
    필요한 개념을 처음 쓸 때 일상적인 말로 한두 문장 설명하고, 관련된 조건을 이어 쓰세요. 서류·제도의 공식 이름은 그대로 두고 그 이름 밖에서 역할을 설명하세요. 이미 설명한 뜻을 반복하지 마세요.
    숫자가 붙은 관계·비율·단위도 구체적으로 풀어 주세요. 뜻을 설명하는 예에는 기초적인 숫자를 쓸 수 있지만, 예를 이 사업의 자격 목록이나 추가 조건으로 제시하면 안 됩니다.
    여러 대상에 함께 붙는 조건은 모든 대상에 적용되도록 쓰세요. 목록으로 나누더라도 어떤 조건이 누구에게 붙는지 유지하세요. 필수·선택, 가능성·확정, 행동하는 주체도 원문과 같아야 합니다.
    별도 법령·제도의 자격이나 서류 발급 방법은 원문에 없으면 추측하지 마세요. 여러 뜻이 가능한 말은 문맥과 사전으로 구별하고, 구별할 수 없으면 뜻을 단정하지 마세요.
    예시 원문: 면적이 10㎡ 이하인 작업실 및 열람실은 사전 예약이 필요합니다.
    예시 쉬운 글: 작업실은 일을 하는 방이고, 열람실은 책이나 자료를 읽는 방입니다. 면적은 공간의 넓이입니다. 10㎡는 한 변이 1미터인 정사각형 10개를 합한 넓이입니다. 두 종류의 방 모두 넓이가 10㎡ 이하이면 미리 예약해야 합니다.
    출력 전에 독자가 어떤 기준에 해당하는지, 무엇을 준비해 어떻게 신청하는지 이 글만 읽고 설명할 수 있는지 확인하세요. 핵심 용어를 설명하지 않고 문장만 나눈 부분은 다시 쓰세요.
    """.trimIndent()
