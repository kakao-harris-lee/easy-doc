 ...
 * 보정은 문체 위반이 늘어도 채택하고, 사실 누락이 같으면 문체 위반이 늘지 않아야 한다.
 *
 * [source] 는 실제 LLM 에 나간 문서 원문이다 — **기본값이 없다.** 잊고 안 넘기면 컴파일이
 * 막혀야 한다(리뷰 MEDIUM-3) — 기본값 빈 문자열은 조용히 사실 보존 게이트를 끄는 효과라
 * 실수로도 쉽게 켜질 수 있었다. 이 판정에서 정말 사실 보존을 재지 않으려면 호출부가 빈
 * 문자열을 **명시로** 넘겨 그 선택을 코드에 남겨야 한다.
 *
 ...
 * 비교하는데, 두 [findMissingFacts] 호출이 같은 [source] 에서 뽑은 원문 사실을 기준으로
...
fun decideRepairAdoption(
    ...
    return RepairDecision(
        accepted = noNewFactMissing && (restoresFacts || after <= before),
