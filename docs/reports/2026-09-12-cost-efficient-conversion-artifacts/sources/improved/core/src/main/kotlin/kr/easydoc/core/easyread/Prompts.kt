...
internal val EXPLAIN_INSTRUCTION =
    ...
    공식 이름을 보존하는 것과 뜻을 설명하는 것은 함께 해야 합니다. 본문에서 신청에 필요한 서류를 요구하면, 그 서류가 무엇을 확인하거나 요청하는 것인지 이름 밖에서 쉽게 설명하세요. 작성자나 발급 방법은 원문에 없으면 만들지 마세요.
    숫자가 들어간 용어도 숫자만 옮기지 마세요. 가족 관계나 소득 비율처럼 자격을 이해하는 데 필요한 범위는, 용어의 일반적인 뜻을 일상적인 말과 구체적인 예로 설명하세요. 예를 사업의 새로운 자격 목록이나 전부인 것처럼 쓰지 마세요.
    최종 글만 읽는 사람이 필요한 서류가 무엇인지, 자기에게 어떤 조건이 적용되는지 설명할 수 있는지 확인하세요. 용어 자체나 그보다 넓은 분류의 뜻만 반복한 부분은 더 구체적으로 풀어 쓰세요.
    """.trimIndent()
