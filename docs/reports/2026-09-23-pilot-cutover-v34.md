...
19:28~21:47 KST 사이에는 행동 안내 작업이 대기·실행 중인 순간의 탈퇴가 FK 오류로 실패할 수 있었다. 해당 구간 행동 안내 작업은 0건이었다. 24시간 관측 창은 21:47 KST부터 다시 센다(23:50 재배포로 다시 갱신 — 아래 절). 테스트만 바꾼 PR #151은 이 뒤에 머지했다(`ef5fb70b`, 운영 동작 무관).

## 행동 안내 재시도 상한 재배포 (23:50 KST, 사용자 승인)

PR #154(행동 안내 작업 리스 재획득 상한, 기본 5회 — `EASYDOC_ACTION_GUIDE_MAX_LEASE_ATTEMPTS`, 1~100)를 반영했다. 사용자 선택 「둘 다 머지 + 파일럿 재배포」. 알려진 재현 경로가 없는 예방 수정이다.

| 항목 | 값 |
|---|---|
| 실행 커밋 | main `0a6fa445`(PR #154, #153). 마이그레이션 변경 없음 → V34 그대로 |
| 사전 확인 | 변환 작업 12건 모두 `done`, 행동 안내 작업 0건 |
| 실행 | `./docker_startup.sh restart`, 백엔드 이미지 23:50 KST 재빌드, 컨테이너 5개 정상 |
| 확인 | health 200, worker env 토글 유지, 기동 후 3분 ERROR/Exception 0건 |

24시간 관측 창은 23:50 KST부터 다시 센다(종료 판독 2026-09-24 23:50 KST 이후).
