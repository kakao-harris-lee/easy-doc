...
**2026-09-12 결정:** 유료 결제는 신용·체크카드만 허용한다. 계좌이체, 세금계산서 필요 여부 질문, 카드 취소 후 이체 재수납은 제공하지 않는다. 카드 결제 증빙은 카드매출전표로 처리한다. `/usage`의 세금계산서 요청 폼·목록·조회 호출은 제거했다. 기존 요청 데이터와 관리자 처리/API는 남아 있으며, 신규 요청 API 자체를 폐쇄한 것은 아니다. 상용 결제는 아직 열지 않았고, local/test에서는 토스 테스트 카드로 Start를 결제해 한 달 50크레딧을 적용한다. 테스트 승인과 영수증은 실제 청구·세무 증빙이 아니다.

아래 CSV는 운영·원가 확인용이며 파일럿 고객에게 청구하거나 입금을 요청하지 않는다.

`llm_calls`(V14) 원장을 사용자·워크스페이스별로 집계해 운영 CSV로 낸다.
`usage-report`는 `migrate`·`rotate-keys`와 같은 CLI one-off이며 결제를 청구하지 않는다.
...
1. **월초에 리포트를 뽑는다.** `docker compose run --rm`은 실행이 끝나면 그 컨테이너를
   ...
   **`--out`을 반드시 마운트 경로(`/out/...`) 아래로 지정한다** — 생략하면 기본값
   ...
   수·문자 수·크레딧·비용이 집계에서 빠지지 않는다. **`owner_email`이
   ...
   누군가에게 청구서를 보내지 않는다.**
2. **운영 지표를 확인한다.** 문서·문자·크레딧과 LLM 호출·토큰·원가 추정치는 운영 점검용이다. `estimated_cost_usd`는 고객 결제 금액이 아니다. `cost_unknown_calls`는 단가 미상, `failed_calls`는 완료되지 않은 호출 수다. 사용 이력이 없는 작업 공간은 행이 없을 수 있다. 사용자 화면에는 이번 달 문서·문자·크레딧 요약만 보여준다.
3. **기존 세금계산서 요청 기록은 관리자 화면에서 확인한다.** 카드 전용 정책으로 사용자 요청 폼은 제거되었다. 과거 기록이 남아 있으면 실제 거래·발급 여부를 확인해 처리하고, UI 제거만으로 삭제하거나 발급 완료로 바꾸지 않는다.

리포트 CSV에는 소유자 이메일·워크스페이스 이름이 실리므로 운영 확인에 필요한 동안만 보관하고,
...
`docs/plans/2026-09-07-credit-accounts.md`). 수동 조정은 운영상 정정용이며 입금이나 별도 분량 구매를 받는 경로가 아니다. 주기 제공량·초기화는 현재 정책을 따른다.

**과거 월초 수동 충전·만료 없음 절차는 폐기했다.** 현재 포함 이용량은 주기 종료 시 초기화되고 이월되지 않는다. `credit-grant`는 자동 결제나 구독 상태를 생성하지 않는다.

**절차:**

1. **조정 사유 확인.** 장애 보상·운영상 정정 등 근거를 확인한다. 결제나 입금 확인을 대신하지 않는다.
2. **`credit-grant`로 부여한다.** `usage-report` 호출과 같은 모양이다 — 바인드 마운트는
   필요 없다(파일을 쓰지 않는다). `--workspace`는 워크스페이스 uuid,
   `--credits`는 정수(운영상 수동 부여는 양수, 환급 취소 같은 조정은 음수),
...
4. **집행 상태를 유지한다.** 운영 기본값은 `EASYDOC_CREDITS_ENFORCED=true`다. 미결제·만료·
   소진 계정은 가용 크레딧이 없으므로 문서 등록이 402로 거절되고, Start 테스트 결제를
   완료하면 그 시점부터 한 달 50크레딧이 설정된다. `users.is_admin=true`인 계정도 같은
   규칙을 적용한다. 장애 보상·정정으로 수동 조정한 경우에만 위 1~3단계로 결과를 확인한다.
...
다섯이다: 「사용자 의견」(최신순 의견·만족도·사용 의향·소요 시간), 「워크스페이스」(검색·크레딧 조회·수동 조정), 「세금계산서」(상태별 목록·발급/거절
...
- 기존 기록의 `invoice-handle` 프로필 — 「세금계산서」 탭의 처리 폼이 같은
...
관리자 계정으로 계정 메뉴 「관리」 → 「사용자 의견」에서 확인한다(계약 2.32.0, GET /admin/feedback).
보존 기간이 끝나 자유 의견이 파기되면 척도만 남는다. 사용자에게는 제출 완료 표시를 남기고 입력 폼을 숨긴다.
