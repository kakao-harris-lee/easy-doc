...
- Backend: Kotlin, Spring Boot, Gradle 멀티모듈 (`backend-kotlin/` — `core` → `application` → `infrastructure` → `api`/`worker`)
...
- 로컬 인프라(PostgreSQL)는 `docker-compose.yml`로 띄운다.
...
docker compose config                      # compose 정의 자체의 유효성
```
...
- `kotlin-migrate` 컨테이너가 Flyway 마이그레이션을 한 번 적용하고 정상 종료하며, `kotlin-api`와 `kotlin-worker`는 그 성공을 기다렸다가 뜬다. `frontend`는 `kotlin-api`의 healthcheck 통과를 기다린다.
...
언어 독립적인 선별 데이터(골든 문서, easy-read 예시, 마스킹·스타일 규칙 fixture)는 `data/**`에 있다. 이동 경위와 hash는 [`docs/plans/_workspace-python-removal/02_data-migration-manifest.md`](docs/plans/_workspace-python-removal/02_data-migration-manifest.md)에 기록되어 있다.
