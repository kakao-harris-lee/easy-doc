...
from app.ingest.extractors import _docx_blocks, extract_text
...
# 3) m:t — OMML 수식
d.add_paragraph()._p.append(
    parse_xml(f'<m:oMath xmlns:m="{M}"><m:r><m:t>x+1=2</m:t></m:r></m:oMath>')
)
...
CMapName currentdict /CMap defineresource pop
    ...
    pg = add(
        ...
        b"/Resources << /Font << /F1 %d 0 R >> >> /Contents %d 0 R >>" % (pages_n, fn, cn)
    )
...
(SP / "spike-oracle.json").write_text(
