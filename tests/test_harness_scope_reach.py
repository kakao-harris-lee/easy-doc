...
- **충족을 강등하면 하한이 못 잡는다.** `충족 = 예` 3행을 `아니오` + `안 돎` 으로
  내리면 규칙 3의 대상에서 빠지는데, 하한 여유가 실측 기준 행 5 · `충족 = 예` 3 ·
  표기 7 이라 `test_판정이_실제로_행을_보고_있다` 도 통과한다. 하한은 **소실**을 잡지
  **강등**을 잡지 않는다.
...
- **행 삭제와 위조 행 추가.** 하한(`_ROW_FLOOR` 등)은 여유를 두고 잡았고 상한은 없다.
  기록 위조는 이 검사의 threat model 이 아니라 **리뷰와 diff** 의 몫이다. 하한을 정확
  개수로 바꾸지 마라 — Phase 표가 늘 때마다 깨져 마찰만 키운다.
...
#: 판정이 **0건 검사로 통과**하는 것을 막는 하한. 현재 실측은 행 45 · `충족 = 예` 18 ·
#: 표기 52 이고, 하한은 그보다 낮게 잡아 증가는 막지 않고 소실만 잡는다. 줄이려면
#: 이 상수를 고쳐야 하고 그 diff 가 리뷰에 올라간다.
_ROW_FLOOR: Final = 40
_MET_YES_FLOOR: Final = 15
_TOKEN_FLOOR: Final = 45
...
def test_판정이_실제로_행을_보고_있다(target_tables: list[Table]) -> None:
    ...
    통과하므로, 하한을 세워 그 상태를 실패로 만든다. 하한은 지금 값보다 낮게 잡아
    행이 늘어나는 것은 막지 않고 **사라지는 것만** 잡는다.
    """
    ...
    assert rows >= _ROW_FLOOR, (
        f"대상 행이 {rows}개다 (하한 {_ROW_FLOOR}) — 표가 사라졌거나 파싱이 깨졌다."
    )
    assert met_yes >= _MET_YES_FLOOR, (
        f"`충족 = 예` 행이 {met_yes}개다 (하한 {_MET_YES_FLOOR}) — "
        "규칙 2가 검사할 대상이 사라지면 그 규칙은 항상 통과한다."
    )
    assert tokens >= _TOKEN_FLOOR, (
        f"실행 경로 표기가 {tokens}개다 (하한 {_TOKEN_FLOOR}) — "
