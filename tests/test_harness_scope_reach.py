...
- **제목·표 이름 앞 40자 밖의 편집은 안 잡는다.** 키는 행 제목과 표 caption 을 각각
  ...
  충돌과 **대상 표 사이의** caption 충돌을 둘 다 위반으로 잡는다. 현재 대상 표 6개의
...
#: 대상 표 개수 — Phase 0·1·2·3·4 종료 조건 표 + 「아직 돌리지 않은 검증 게이트」 표.
#: 2026-08-19 게이트 20(`03_auth`): Phase 3 종료 조건 표 신설로 4 → **5**.
#: 2026-08-19 게이트 25(`04_crypto`): Phase 4 종료 조건 표 신설로 5 → **6**.
EXPECTED_TARGET_TABLES: Final = 6
...
#: 2026-08-19 게이트 24(`03_phase3-close`) 후속: 표기 65 → **66**.
#: **행 수·정체성 집합은 무변동**이다 — Phase 3 표 7행 그대로이고 `충족` 도 행 5 만 `예` 다.
#: 늘어난 표기는 하나뿐이고 **「없던 실행이 생겼다」가 아니라 「유보하던 사유가 소멸했다」**라
#: 성격이 위 ⑷ 와 다르다. 그래서 따로 적는다.
#:   행 4(소유권 404) `ci:kotlin` → `· ci:quality` 추가 (**+1**). 위 ⑷ 는 이 표기를
#:   **함께 적지 않은** 이유를 *"그 게이트가 이 배치에서 네 형태의 탐지를 잃어 조치 대기 중"*
#:   으로 적었다. 그 사유가 소멸했다 — 복원 `01d78a1` 이 게이트 24 에서 **3관점 합의로 통과**
#:   했고(Claude F1~F4 실측 · codex 실행 · privacy-gate CI exit 0), CI 도달도 관측됐다
#:   (run `32225305372` · headSha `2a4523d` · 잡 `quality` **success**).
#:   확장 `6be9612`(밑줄 결합 상수 2형태)는 **리뷰 미수령**이라 이 표기의 근거로 **세지 않는다**
#:   — `01d78a1` 하나만으로 표기가 선다(범위는 근거를 넘지 않는다).
#:   ⑷ 의 옛 문면은 **고치지 않는다**. 그것이 무엇에 대해 내려진 유보였는지가 남아 있어야
#:   이 갱신이 번복이 아니라 **사유 소멸**임을 다음 사람이 알아본다.
#: 2026-08-19 게이트 25(`04_crypto`): 행 52 → **62**, 표기 66 → **78**. Phase 4 종료 조건 표를
#: 신설해 계획 §5 Phase 4(`:411-420`)의 여섯 항목 + 종료 조건 **네 조각**을 **항목당 한 행**으로
#: 옮겼다(10행 · 표기 12 = `안 돎`×7 · `ci:kotlin`×2 ·
#: `ci:kotlin · 1회성:…04_security_privacy-gate.md` · `ci:kotlin · ci:quality`).
#: **10행 전건이 `충족 = 아니오`** 라 `EXPECTED_MET_YES_KEYS` 에는
#: 들어가지 않고, 대신 **10행 전부가 `EXPECTED_UNRESOLVED_KEYS`** 에 들어갔다.
#: 같은 회차에 `EXPECTED_MET_YES_KEYS` 도 **한 줄 늘었다** — Phase 3 행 4(소유권 404)의 승격이다
#: (아래 그 상수의 주석 참조). **두 상수가 같은 diff 에서 함께 움직이는 것이 이 신고의 형태**다:
#: 판정 범위가 넓어졌고(표 신설), 주장 하나가 새로 섰다(행 4).
EXPECTED_ROWS: Final = 62
EXPECTED_REACH_TOKENS: Final = 78
...
#: 에도 같은 값을 쓴다 — 현재 대상 표 6개의 caption 은 전부 40자 미만이라 클립이 걸리지
...
EXPECTED_UNRESOLVED_KEYS: Final[frozenset[tuple[str, str]]] = frozenset(
    {
        ...
        # Phase 4 — 문서 API·암호화·내보내기 (2026-08-19 신설 · 게이트 25 `04_crypto`)
        #   10행 **전부** 미해결 항목을 들고 있다. `충족` 도 전건 `아니오` 다 — 이 단위(`crypto`)가
        #   만든 것은 종료 조건을 **만족시킬 수 있는 도구**이고, 조건 자체는 저장 경로가 그 도구를
        #   실제로 쓰는지로 판정된다(cross §8 — 세 레인 일치). AEAD 정확성 축 하나만 닫혔고
        #   그 행조차 조치 배치가 진행 중이라 미해결이 일곱이다.
        ("Phase 4 — 문서 API·암호화·내보내기", "JSON/multipart 업로드와 제한 처리"),
        ("Phase 4 — 문서 API·암호화·내보내기", "DOCX/PDF/HWPX 추출"),
        ("Phase 4 — 문서 API·암호화·내보내기", "암호화 저장·복호화 조회"),
        ("Phase 4 — 문서 API·암호화·내보내기", "문서 목록·삭제, 변환 조회·검수 저장"),
        ("Phase 4 — 문서 API·암호화·내보내기", "DOCX/TXT/HWPX 내보내기"),
        (
            "Phase 4 — 문서 API·암호화·내보내기",
            "저장 암호화 AEAD 정확성 검증과 문서 차분 비교(§4.5)",
        ),
        (
            "Phase 4 — 문서 API·암호화·내보내기",
            "종료 조건: 실 PostgreSQL 에서 업로드 → 조회 → 검수 → 3",
        ),
        (
            "Phase 4 — 문서 API·암호화·내보내기",
            "종료 조건: 평문이 DB·로그에 없다 (I-7 round-trip·변조 ",
        ),
        (
            "Phase 4 — 문서 API·암호화·내보내기",
            "종료 조건: 문서 처리 요구사항 인벤토리 항목의 미충족 0",
        ),
        (
            "Phase 4 — 문서 API·암호화·내보내기",
            "종료 조건: 문서 차분 비교(§4.5)의 `미확인` 불일치 0",
        ),
    }
)
...
EXPECTED_MET_YES_KEYS: Final[frozenset[tuple[str, str]]] = frozenset(
    {
        ...
        # 2026-08-19 게이트 25(`04_crypto`) 리더 판정 ⑧: 「소유권 404」 행을 **예로 올렸다**.
        # 게이트 24 가 이 행에 건 조건은 **하나**였다 — 「`6be9612` 리뷰 수령」. 게이트 25 가
        # 3관점으로 그 범위를 수령했고(codex 호출 A·B 가 `--base 76f6863` branch diff 42파일 ·
        # Claude 1차가 `6be9612` 를 직접 다룸) **차단① 0 · privacy-gate 차단 0** 이다.
        # 열려 있는 차단②(L-1)의 대상은 이 행의 **내용이 아니라 통로**라 조건이 겨눈 자리가 아니다.
        # **단서 둘이 행에 병기돼 있다** — L-1(그 커밋이 심판 산출물을 같은 커밋으로 편집했다) ·
        # H7(403 잔여의 **종류**가 열려 있고 그 잔여가 선언되지 않았다). 닫힌 것은 수령이지
        # 지적이 아니므로, 이 행은 `EXPECTED_UNRESOLVED_KEYS` 에도 **그대로 남아 있다**
        # (두 집합에 동시에 드는 것이 정상이다 — 주장이 섰다는 것과 잔여가 없다는 것은 다르다).
        (
            "Phase 3 — 데이터·인증·작업 공간 API",
...
def test_규약_대상_표_여섯_개를_찾는다(target_tables: list[Table]) -> None:
...
_COVERAGE_AXIS_NAMES: Final = (
    ...
    # 아래 셋은 이 파일의 [_ledger_round_cells] 가 **빌려 쓴다**(X-11 강제자). 파서를
    #   복제하지 않는 대신, 빌린 이름이 사라지면 그 검사가 조용히 죽는 대신 여기가 먼저
    #   신고한다.
    "_section",
    "_ledger_rows",
    "_LEDGER_HEADING",
...
# ---------------------------------------------------------------------------
# 이연 장부 `리뷰할 회차` 칸의 **어간 문법 강제자** (X-11, 2026-08-23)
#
# 규약은 `0272d3b` 이 SKILL.md 에 세웠고 **강제자는 유보했다** — 사유는 "오늘 원장의 그 칸이
# 산문이라 강제하면 즉시 빨개지고, 원장 데이터는 리더 소관이라 규약을 세우는 레인에서 고칠
# 수 없다" 였다. `aff6204` 가 그 칸 20행 전부를 백틱 어간으로 바꾸면서 **유보 사유가
# 소멸했다.** 사유 없는 유보는 다음 회차에 「원래 그런 것」으로 굳으므로 여기서 건다.
# ---------------------------------------------------------------------------

#: 칸 전체가 **백틱 토큰 하나**여야 한다. 산문·복수 표기·굵게 표시를 여기서 거른다.
_ROUND_CELL_STEM: Final = re.compile(r"`([^`]+)`")


def _ledger_round_cells() -> list[str]:
    """장부 유효 행의 `리뷰할 회차` 칸을 원문 그대로 준다.

    파서를 복제하지 않고 커버리지 강제자의 것을 빌린다 — 사본을 두면 두 판정의 「행이란
    무엇인가」가 갈린다. 빌린 이름들은 [_COVERAGE_AXIS_NAMES] 에 등재해, 그쪽에서
    사라지면 이 검사가 조용히 죽는 대신 그 목록이 먼저 신고하게 해 둔다.
    """
    module = _review_coverage_module()
    ledger = module._section(module._LEDGER_HEADING)
    assert ledger is not None, (
        "원장에서 「리뷰 이연 장부」 절을 찾지 못했다 — 절 머리가 바뀌었거나 절이 사라졌다.\n"
        "  빈 입력에서 통과하면 이 문법 판정이 0건 검사가 된다(규칙 4 ⑶)."
    )
    return [str(row.round_).strip() for row in module._ledger_rows(ledger)]


def test_이연_장부의_리뷰할_회차_칸이_어간_문법을_지킨다() -> None:
    """**X-11 의 강제자** — 그 칸은 백틱 어간 하나이고 어간 문법을 지킨다.

    왜 문법을 요구하는가: 그래야 그 칸이 커버리지 표의 `회차` 칸과 **기계로 맞대어진다.**
    산문(`04_documents-c6 2회차`)이면 「이 빚을 어느 회차가 갚는가」가 사람의 독해에만
    남고, 장부의 출구를 재는 X-3b 는 그 독해를 할 수 없다.

    ## 문법 이전 어간 핀을 이 자리로 **넓히지 않는다** (규칙 4 ⑵)

    [PRE_GRAMMAR_STEMS] 는 산출물 파일명에 대한 **면제 목록**이고 장치 분류상 은폐형
    성분을 갖는다. 은폐형은 같은 종류의 빈자리가 재발해도 넓히지 않는다 — 넓히면 면제가
    소비자 수만큼 늘어난다. 넓힐 **근거도 없다**: 오늘 장부 24행의 값은 두 종뿐이고
    (`xx_harness-r2` · `04_documents-c6r2`) 둘 다 핀 없이 합법이다.

    그 결과 `16_gate15-fixes` 같은 문법 이전 어간을 이 칸에 적으면 **지목된다.** 그것이
    의도다 — 지난 회차의 산출물 이름은 그대로 두되, 앞으로 적는 빚은 문법을 지킨다.

    ## 못 재는 것

    **그 회차가 실재하는지는 보지 않는다.** 존재하지 않는 미래 회차를 적어도 통과한다 —
    커버리지 표의 `회차` 칸과의 실물 대조는 이 검사의 범위 밖이다.
    """
    scopes = scope_values()
    assert scopes, "[test_scope_정본_표가_비지_않는다] 를 먼저 보라 — 분모가 비었다."

    cells = _ledger_round_cells()
    assert cells, (
        "이연 장부에 유효한 표 행이 0 건이다 — 이 검사가 아무것도 판정하지 않았다.\n"
        "  빈 분모에서 통과하면 안 된다(규칙 4 ⑶)."
    )

    violations: list[str] = []
    for cell in cells:
        match = _ROUND_CELL_STEM.fullmatch(cell)
        if match is None:
            violations.append(f"{cell!r} — 백틱 어간 하나가 아니다(산문·복수 표기·굵게 표시)")
        elif not stem_is_legal(match.group(1), scopes):
            violations.append(f"{cell!r} — 어간이 `{{phase}}_{{scope}}[-{{회차}}]` 문법을 어긴다")

    assert not violations, (
        f"`리뷰할 회차` 칸이 문법을 어긴 행 {len(violations)} 건 (전체 {len(cells)} 행):\n"
        + "\n".join(f"    - {line}" for line in sorted(set(violations)))
        + "\n\n문법: 백틱 어간 하나 — `{phase}_{scope}[-{회차}]`.\n"
        "  `{scope}` 는 SKILL.md 정본 표의 값이어야 한다. 새 대상이면 **표에 값을 먼저**\n"
        "  추가하라. 문법 이전 어간 핀(PRE_GRAMMAR_STEMS)은 산출물 파일명 전용이고\n"
        "  이 칸으로 넓히지 않는다 — 은폐형 목록은 소비자를 늘리지 않는다(규칙 4 ⑵)."
    )
