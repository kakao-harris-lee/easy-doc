...
-- 「잠정 충족」으로 적는다.
--
-- **기준 ① 에는 같은 처리를 하지 않는다 — 단조성 때문이다.** 기준 ① 은 건수 조건
-- (`usable >= 8`)이고 `usable` 은 표본이 늘어날수록 늘기만 한다. 즉 표본 3건에서 8건을
-- 이미 넘겼다면 표본이 10건으로 늘어도 그 충족은 **뒤집히지 않는다.** 기준 ② 는 평균이라
-- 다르다 — 3건 평균 5.0 은 남은 7건에 따라 3.5 아래로 얼마든지 내려간다. 두 기준을
-- 다르게 다루는 것은 실수가 아니라 이 사유이며, 사유 없이 통일하지 않는다.
...
WITH s AS (
    SELECT
        ...
        -- 이 파일에서는 여기 한 곳에만 적는다 — 아래 판정과 비고가 전부 이 값을 참조한다.
        10 AS target_n,
        count(*) AS n,
    ...
    SELECT
        ...
        CASE
            ...
            WHEN s.n < s.target_n THEN '표본 ' || s.n || '건 / ' || s.target_n
                || '건 — 파일럿 미완. 충족은 뒤집히지 않지만(건수 조건의 단조성) 미충족은 아직 판정이 아니다.'
            ELSE '표본 ' || s.n || '건.'
        END AS note
    FROM s
    UNION ALL
