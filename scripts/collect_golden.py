...
def report(draft: GoldenDraft, path: Path | None) -> None:
    ...
    # privacy-allow: LOG-BODY @8c5b3928 — 문서 id 와 글자 수만 보간한다(본문·제목 제외).
    ...
    if draft.stats.masked_counts:
        ...
        # privacy-allow: LOG-BODY @b91cb227 — 범주별 건수 합계만 보간한다(가려진 원문 제외).
    ...
    if draft.stats.auto_category is not None:
        # privacy-allow: LOG-BODY @f2d52727 — 분류값(열거형)만 보간한다.
        print(f"자동 분류: {draft.stats.auto_category} (규칙 기반 추정 — 사람 확인 필요)")
    if draft.stats.suggested_facts:
        # privacy-allow: LOG-BODY @bc68d035 — 추출 후보 **개수**만 보간한다(팩트 문면 제외).
        print(
