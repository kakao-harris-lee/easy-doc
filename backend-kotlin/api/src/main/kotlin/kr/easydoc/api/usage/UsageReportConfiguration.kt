 ...
 * 컴파일 시점에 볼 수 없다). 이 클래스는 그 빈을 받아 CLI 실행부([UsageReportRunner])만
 * 배선한다 — `KeyRotationConfiguration`이 무거운 배치 객체를 조립하는 것과 달리, 여기서
 * 새로 조립할 것은 실행부 하나뿐이다.
 */
...
class UsageReportConfiguration {
    @Bean
    fun usageReportRunner(service: UsageReportService): UsageReportRunner = UsageReportRunner(service)
}
