 ...
 *
 * 같은 종류의 결함이 **다섯 번** 났다(`retainTruncated` 미단언 · 조각 동결 · 늦은 등록의
 * 소급 적중 미검증 · 축 재고 미고정 · **도달 미단언**). 매번 「하나 더 지키면 닫힌다」였다가
 * 다시 열렸다. 그래서 **성질을 열거하고 장치 이름을 붙인다** — 「없다」도 적는다. 빈 칸이
 * 다음 회차의 자리다.
 *
 * | # | 성질 | 지키는 장치 |
 * |---|---|---|
 ...
 * | 11 | 카나리 값이 실제로 제품 경로를 지난다 | `EXPECTED_REACH` 정확 열거 핀 + `ReachLog`(도달 케이스) |
 * | 12 | ⒝ 의 로거 고정이 실제로 방출을 막는다 | **상시 장치 없다** — 수동 음성 대조(고정 3줄 삭제)로만 확인 |
 * | 13 | 카나리 값이 로그에 자연 발생하지 않는다 | **없다** — 합성값 선택의 조심함뿐 |
 *
 * ### 남은 빈 칸 — **12 와 13**
 *
 * 성질 11 은 닫혔다(게이트 28 P-2 다섯 번째 조치). 도달 케이스가 요청 **일곱 갈래**(문서 다섯
 * + 계정 생성·로그인)의 기대 상태 코드를 정확 열거 핀으로 고정하고 실제와 **순서까지 정확
 * 일치**를 단언한다. 고치기 전에는 인증 헤더를 빼 전건 401 로 만들어도 **초록**이었다(실측 —
 * 실패 0건). 왜 그 형태인지·기대값을 계약에서 읽지 않은 사유·이 핀이 가리지 못하는 것
 * (422 안에서 스키마 층과 서비스 층은 갈리지 않는다)은 `DocumentBodyLogLeakReachTest` KDoc 의
 * 「요청이 **의도한 층**까지 갔음을 단언한다」에 있다.
 *
 * **12 와 13 은 여전히 빈 칸이다. 지우지 않는다.**
 *
