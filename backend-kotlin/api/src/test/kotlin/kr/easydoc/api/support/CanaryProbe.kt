 ...
 *
 * ## 이 카나리가 지켜야 할 성질 **전부**와, 각각을 지키는 장치
 *
 * 같은 종류의 결함이 **일곱 번** 났다(`retainTruncated` 미단언 · 조각 동결 · 늦은 등록의
 * 소급 적중 미검증 · 축 재고 미고정 · 도달 미단언 · 도달 핀이 422 안의 층을 안 갈랐다 ·
 * **성공 팔의 저장 도달을 응답으로 대신했다**). 매번 「하나 더 지키면 닫힌다」였다가 다시
 * 열렸다. 그래서 **성질을 열거하고 장치 이름을 붙인다** — 「없다」도 적는다. 빈 칸이 다음
 * 회차의 자리다.
 *
 * **일곱 번째는 앞의 여섯과 종류가 다르다** — 장치가 성질을 안 겨눈 것이 아니라 **추격을
 * 멈추자는 판단의 근거**가 성질을 안 겨눴다. 근거는 *"성공 상태는 이른 거절로 위조할 수
 * 없으므로 성공 팔이 저장 경로를 진다"* 였는데, 뒷문장은 참이고 앞문장으로 이어지지 않는다.
 * `202` 는 **응답 사실**이고 저장 사실이 아니다.
 *
 * **표에 「닫힘」을 적을 때는 그것이 참인지 먼저 물어야 한다.** `3a3333e` 는 성질 11 을 닫힘으로
 * 적었는데 **거짓이었다** — 같은 커밋의 KDoc 이 「상태 코드만으로는 422 안의 층이 갈리지
 * 않는다」를 스스로 적어 두었고, 그 한계가 일곱 중 **셋**(전부 422)을 열어 두고 있었다.
 * 표가 거짓이면 그 표가 다음 빈 칸을 가린다.
 *
 * | # | 성질 | 지키는 장치 |
 * |---|---|---|
 * | 1 | 캡처가 살아 있다 | `sawPositiveControl()` |
 * | 2 | 강제 TRACE 가 실제로 먹었다 | `traceEvents() != 0` |
 * | 3 | 보관이 전량이다(잘리지 않았다) | `retainTruncated()` 단언 |
 * | 4 | 소급 루프가 보관분을 훑었다 | `sawRetroControl()` |
 * | 5 | 늦게 등록된 카나리가 보관분에 적중을 낸다 | 늦게 등록되는 통제 카나리 + `controlHitAxes()` |
 * | 6 | 늦은 등록이 **빠짐없이** 소급 대조를 지났다 | `pendingRetroMatches()` |
 * | 7 | **재는 축의 재고가 선언과 정확히 일치하고 비어 있지 않다** | `EXPECTED_AXES` 정확 열거 핀 + `registeredAxes()` |
 * | 8 | 실패 메시지가 카나리 **값**을 담지 않는다 | `residualCanaryFragments()`(도달·단위 양쪽) |
 * | 9 | 조각은 **완전한** 카나리 집합으로만 만들어진다 | 읽기 빗장(`sealed`) + 단위 케이스 |
 * | 10 | 통제 적중과 유출 적중이 섞이지 않는다 | 별개 레지스트리 + 단위 케이스 |
 * | 11 | 카나리 값이 실제로 제품 경로를 지난다 | 도달 핀(상태 + `detail` 모양) + 저장 핀 — **부분**, 잔여는 아래 |
 * | 12 | ⒝ 의 로거 고정이 실제로 방출을 막는다 | **상시 장치 없다** — 수동 음성 대조(고정 3줄 삭제)로만 확인 |
 * | 13 | 카나리 값이 로그에 자연 발생하지 않는다 | **없다** — 합성값 선택의 조심함뿐 |
 *
 * ### 남은 빈 칸 — **11 의 잔여 · 12 · 13**
 *
 * **11 에서 닫힌 부분** — 축이 셋이다. 셋 다 「고치기 전 초록」을 실측으로 확인한 뒤 붙였다.
 *
 * - **상태 코드**(일곱 갈래, 순서까지 정확 열거) — 인증(401)·미디어 타입(415)·경로(404·405)에서
 *   끊긴 요청을 잡는다. 이 축이 없던 판에서는 인증 헤더를 빼 전건 401 로 만들어도 **초록**이었다
 *   (실패 0건).
 * - **`detail` 모양** — 422 세 갈래가 **스키마·프레임워크 바인딩**(배열)이 아니라
 *   **서비스·도메인**(문자열)에서 거절됐음을 잡는다. 상태 코드만 있던 판에서는 앞단 검증을 넣어
 *   세 갈래를 도메인 전에 끊어도 **초록**이었다(실패 0건).
 * - **저장 상태**(`documents` 를 `source_format` 별로 · `conversions`·`conversion_jobs` 행 수 ·
 *   암호문이 자리표시자가 아님 · 제목 카나리가 실린 행 수) — 접수된 두 팔이 **Postgres 행이
 *   됐음**을 응답이 아니라 DB 쪽에서 잡는다. 앞의 두 축만 있던 판에서는 서비스에서 삽입 세 건만
 *   건너뛰게 해 **행이 하나도 안 생겨도** 두 팔이 `202/NONE` 이라 **초록**이었다(실패 0건).
 *
 * **11 의 잔여 — 장치 없다. 두 갈래이고, 둘째가 이번 회차에 드러난 것이다.**
 *
 * - **도메인 안의 어느 지점인가** — 모양은 스키마 층과 도메인 층을 가르지만 도메인 내부는
 *   가르지 않는다. 컨트롤러가 던지는 `InvalidInputException`(`MISSING_FILE_PART_MESSAGE` ·
 *   `INVALID_WORKSPACE_ID_MESSAGE`)도 문자열이므로, ⑸ 손상 파일이 파서 대신 **컨트롤러**에서
 *   끊기면 바이트는 추출기에 닿지 않는데 지문은 `422/STRING` 그대로다. 리더가 마감 `export`
 *   단위 착수 전으로 붙여 두었다.
 * - **저장된 본문이 그 카나리인가** — 암호문은 **길이 관계**로만 본다(`char_count` 초과).
 *   평문을 꺼내 대조하는 것은 이 장치가 막으려는 노출과 같으므로 하지 않는다. 그래서 「본문
 *   자리에 **다른** 평문이 실렸다」는 구분되지 않는다. 제목 축은 평문 열이라 값까지 재지만
 *   본문 축은 재지 않는다.
 *
 * **12 와 13 도 여전히 빈 칸이다. 지우지 않는다.**
 *
 * - **12** — ⒝ 의 로거 고정이 실제로 방출을 막는지는 **상시 장치가 없다.** 고정 3줄을 지우는
 *   **수동** 음성 대조(N-A)로만 확인하므로, 그 대조를 돌리지 않은 회차에는 아무것도 재지 않는다.
 * - **13** — 카나리 값이 로그에 자연 발생하지 않음은 **합성값 선택의 조심함뿐**이다. 값이 우연히
 *   흔한 문자열이 되면 「적중 0」이 아니라 「적중 폭주」로 드러나 조용한 실패는 아니지만, 그
 *   성질을 **재는** 장치는 없다.
 *
 * ## 이 장치의 **결론이 참이기 위한 최소 집합**
 *
 * 위 13속성 표는 「무엇을 지켜야 하는가」의 목록이고, **「무엇이 증명되면 멈춰도 되는가」는
 * 아니다.** 일곱 번째 결함이 정확히 그 빈자리에서 났다 — 「핵심은 ⑵⑶ 가 진다」라는 **근거**가
 * 성질을 겨누지 못했는데, 표에는 그 근거를 검사할 칸이 없었다. 그래서 옆에 적는다.
 *
 * 이 장치의 결론은 하나다:
 *
 * > **제품이 문서 본문·제목·비밀번호·액세스 토큰을 로그에 남기지 않는다.**
 *
 * 「적중 0건」이 그 결론이 되려면 네 묶음이 **모두** 참이어야 한다. 하나라도 거짓이면 0건은
 * 결론이 아니라 동어반복이다.
 *
 * | 묶음 | 뜻 | 지금 지는 것 |
 * |---|---|---|
 * | **A. 자극이 발생했다** | 카나리가 로그를 낼 수 있는 경로를 실제로 지났다 | A1·A2 도달 핀 · A3 저장 핀 · **A4 없음** |
 * | **B. 관측이 발생했다** | 방출된 것을 probe 가 전량 봤다 | 성질 1·2·3·4·5·6·7 |
 * | **C. 판정이 유효하다** | 대조가 참이고 판정 자체가 새지 않는다 | 성질 8·9·10 · **C1 = 성질 13 빈 칸** |
 * | **D. 일반화가 성립한다** | 이 실행의 0건이 **제품 기본 구성**의 0건을 함의한다 | D1 구성상 참 · **D2 = 성질 12 빈 칸** |
 *
 * - **A1** 요청이 인증·미디어 타입·경로에서 끊기지 않았다(상태 코드 축) · **A2** 422 갈래가
 *   스키마 층이 아니라 도메인에서 거절됐다(`detail` 모양 축) · **A3** 접수된 팔이 Postgres
 *   행이 됐다(저장 축) · **A4 「본문 자리에 들어간 평문이 그 카나리다」 — 장치 없다**
 *   (암호문을 길이 관계로만 보므로. 성질 11 잔여)
 * - **C1** needle 이 로그에 자연발생하지 않는다 — 그것이 참이 아니면 「적중 0」을 「안 샌다」로
 *   읽을 수 없다(성질 13)
 * - **D1** 강제 TRACE 는 기본 구성의 상위집합이다 — 어떤 로거의 레벨도 내려가지 않고 방출만
 *   늘어난다(구성상 참) · **D2** ⒝ 의 로거 고정이 실제로 방출을 막는다(성질 12)
 *
 * ### 이 표가 곧바로 말해 주는 것
 *
 * **남은 빈 칸 둘은 장식이 아니라 최소 집합의 구성원이다.** 성질 13(C1)이 없으면 「0건」을
 * 「안 샌다」로 읽을 수 없고, 성질 12(D2)가 없으면 이 실행의 0건을 **제품**의 0건으로 옮길 수
 * 없다. 즉 **D2 가 거짓이면 이 케이스의 결론은 「강제 TRACE 에서 우리가 고정한 세 로거를 뺀
 * 나머지가 안 샌다」로 좁아진다** — 그것도 값은 있지만 원장 조건 18 이 요구한 문장은 아니다.
 * 12·13 은 여전히 **이 커밋 범위가 아니다**(리더 판정). 다만 「부수적」이 아니라는 사실은 적어
 * 둔다 — 그렇게 읽히면 다음 회차가 또 그것을 미룬다.
 *
 * ### 이 표의 한계 (정직하게)
 *
 * 이 목록 자체가 **선언**이므로 `CLAUDE.md` 규칙 4 의 **범위 선언형**이고, 그것을 검사하는
 * 장치는 **없다.** 묶음을 하나 빠뜨렸는지 기계가 말해 주지 않는다 — 일곱 번째 결함이 난 기제와
 * 같다. 그래서 이것은 「멈춰도 되는가」의 **최종 판정**이 아니라 **판정 전에 답해야 할 질문
 * 목록**이다. 멈추자는 판단을 적을 때 A~D 각 묶음에 대해 「무엇이 그것을 지는가」를 적을 수
 * 있어야 하고, 「그 자리는 다른 것이 진다」로 넘길 때는 **그 다른 것이 같은 명제를 지는지**를
 * 확인해야 한다(일곱 번째가 바로 그 확인을 건너뛴 것이다).
 *
...
internal class CanaryProbe(private val retroControl: String) : AppenderBase<ILoggingEvent>() {
    ...
    /**
     * **실제로 등록된 축의 재고.** 유출·통제 **양쪽**을 담고, `값은 담지 않는다` — 축 이름은
     * 비밀이 아니지만 needle 은 비밀이다.
     *
     * 이 값이 필요한 이유: 앞의 장치들은 **기제**(늦은 등록이 작동하는가·소급이 집합을 지나는가)
     * 를 지키고 **재고**(무엇을 재고 있는가)는 아무도 지키지 않았다. 축 등록을 통째로 지우면
     * 통제 카나리는 별개 값이라 그대로 적중하고, `pendingRetroMatches()` 는 **등록되지 않은
     * 축에 항목을 만들지 않으므로** 비울 것이 없어 초록이었다. 즉 등록부는 `CLAUDE.md` 규칙 4 의
     * **범위 선언형**인데 핀이 없었고, 규칙 4 ⑶ 이 그 종류에 대해 못박은 「빈 선언에서 통과하면
     * 안 된다」를 위반했다. 호출자가 이 집합을 **정확 열거 핀**과 대조한다.
     */
