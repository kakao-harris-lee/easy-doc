 ...
 *
 * 같은 종류의 결함이 **네 번** 났다(`retainTruncated` 미단언 · 조각 동결 · 늦은 등록의 소급
 * 적중 미검증 · 축 재고 미고정). 매번 「하나 더 지키면 닫힌다」였다가 다시 열렸다. 그래서
 * **성질을 열거하고 장치 이름을 붙인다** — 「없다」도 적는다. 빈 칸이 다음 회차의 자리다.
 *
 * | # | 성질 | 지키는 장치 |
 * |---|---|---|
 ...
 * | 11 | **카나리 값이 실제로 제품 경로를 지난다** | **없다** — 아래 「남은 빈 칸」 |
 * | 12 | ⒝ 의 로거 고정이 실제로 방출을 막는다 | **상시 장치 없다** — 수동 음성 대조(고정 3줄 삭제)로만 확인 |
 * | 13 | 카나리 값이 로그에 자연 발생하지 않는다 | **없다** — 합성값 선택의 조심함뿐 |
 *
 * ### 남은 빈 칸 — 성질 11 이 가장 위험하다
 *
 * 이 케이스는 요청 다섯 갈래를 태우지만 **응답을 한 번도 보지 않는다**. 그래서 `POST /documents`
 * 가 계약 변경·회귀로 전건 4xx 를 내기 시작하면 본문·제목이 저장 경로에 **닿지 않고**, 로그에
 * 카나리가 없는 것이 당연해져 **케이스 전체가 공허하게 초록**이 된다. 성질 1·2 는 캡처와 레벨을
 * 증명하지만 **트래픽이 실제로 처리됐는지는 증명하지 않는다.** 이것은 지금까지 네 번과 **같은
 * 종류**(통제가 있는데 성질을 안 겨눈다)이고, 규모는 더 크다 — 다른 열두 성질이 다 참이어도
 * 결론이 무의미해진다. 처방 후보는 응답 상태 코드를 **정확 열거 핀**으로 고정하는 것이다
 * (성질 7 과 같은 형태). **이 커밋 범위 밖이라 열거만 하고 두었다.**
 *
