 ...
 *
 * 같은 종류의 결함이 **여섯 번** 났다(`retainTruncated` 미단언 · 조각 동결 · 늦은 등록의
 * 소급 적중 미검증 · 축 재고 미고정 · 도달 미단언 · **도달 핀이 422 안의 층을 안 갈랐다**).
 * 매번 「하나 더 지키면 닫힌다」였다가 다시 열렸다. 그래서 **성질을 열거하고 장치 이름을
 * 붙인다** — 「없다」도 적는다. 빈 칸이 다음 회차의 자리다.
 *
 * **표에 「닫힘」을 적을 때는 그것이 참인지 먼저 물어야 한다.** 직전 판(`3a3333e`)은 성질 11 을
 * 닫힘으로 적었는데 **거짓이었다** — 같은 커밋의 KDoc 이 「상태 코드만으로는 422 안의 층이
 * 갈리지 않는다」를 스스로 적어 두었고, 그 한계가 일곱 중 **셋**(전부 422)을 열어 두고 있었다.
 ...
 * | 11 | 카나리 값이 실제로 제품 경로를 지난다 | `EXPECTED_REACH` 정확 열거 핀(상태 코드 **+ `detail` 모양**) + `ReachLog` — **부분**, 잔여는 아래 |
 * | 12 | ⒝ 의 로거 고정이 실제로 방출을 막는다 | **상시 장치 없다** — 수동 음성 대조(고정 3줄 삭제)로만 확인 |
 * | 13 | 카나리 값이 로그에 자연 발생하지 않는다 | **없다** — 합성값 선택의 조심함뿐 |
 *
 * ### 남은 빈 칸 — **11 의 잔여 · 12 · 13**
 *
 * **11 에서 닫힌 부분** — 도달 케이스가 요청 **일곱 갈래**(문서 다섯 + 계정 생성·로그인)의
 * 지문을 정확 열거 핀으로 고정하고 실제와 **순서까지 정확 일치**를 단언한다. 지문은 두 축이다:
 *
 * - **상태 코드** — 인증(401)·미디어 타입(415)·경로(404·405)에서 끊긴 요청을 잡는다. 이 축이
 *   없던 판에서는 인증 헤더를 빼 전건 401 로 만들어도 **초록**이었다(실측, 실패 0건).
 * - **`detail` 모양** — 422 세 갈래가 **스키마·프레임워크 바인딩**(배열)이 아니라
 *   **서비스·도메인**(문자열)에서 거절됐음을 잡는다. 상태 코드만 있던 판에서는 앞단 검증을
 *   넣어 세 갈래를 도메인 전에 끊어도 **초록**이었다(실측, 실패 0건).
 *
 * **11 의 잔여 — 장치 없다.** 모양은 스키마 층과 도메인 층을 가르지만 **도메인 안에서 어느
 * 지점인지**는 가르지 않는다. 컨트롤러가 던지는 `InvalidInputException`(`MISSING_FILE_PART_MESSAGE`
 * · `INVALID_WORKSPACE_ID_MESSAGE`)도 문자열이므로, ⑸ 손상 파일이 파서 대신 **컨트롤러**에서
 * 끊기면 바이트는 추출기에 닿지 않는데 지문은 `422/STRING` 그대로다. 다음 회차의 자리다.
 *
