...
class WorkspaceEndpointReachTest {
     ...
     *
     * ## 문턱이 auth(1.5)보다 넓은 이유
     *
     * 그쪽은 Argon2 1회(~100ms)가 바닥에 깔려 있어 고정 비용의 흔들림이 비에 거의 안 들어온다.
     * 여기는 질의 한 번(밀리초 단위)이라 같은 절대 지터가 **훨씬 큰 비**로 나타난다. 문턱을
     * 좁히면 잡는 것 없이 깜박이기만 한다. 잡으려는 것은 「행을 하나 더 읽는가」이고, 그것은
     * 이 규모에서도 배 단위로 드러난다.
     *
    ...
    companion object {
        ...
        /** 명세는 KDoc 에 있다 — auth(1.5)보다 넓은 이유가 바닥 비용의 차이다. */
        private const val MAX_TIMING_RATIO = 2.0
