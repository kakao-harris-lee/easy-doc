 ...
 *
 * ## 왜 파일 하나인가 — 세 자리가 같은 성질을 재고 있었다
 *
 * `privacy-gate` 회차 2 의 처방 **X1-1** 이 지목한 것이다. 세 자리(`DocumentListReachTest`
 * DL-9 ② · `DocumentDeleteReachTest` DD-3 · `WorkspaceEndpointReachTest` WR-4)가 같은 성질을
 * 재는데 **재는 것이 서로 달랐다**:
 *
 * | 자리 | 상태 | 본문 | 헤더 이름 집합 |
 * |---|---|---|---|
 * | DL-9 ② (고치기 전) | 잰다 | **UTF-8 디코딩 문자열** | **0건** |
 * | DD-3 (고치기 전) | 잰다 | **UTF-8 디코딩 문자열** | 잰다 |
 * | WR-4 (고치기 전) | 잰다 | **UTF-8 디코딩 문자열** | 잰다 |
 *
 * 헤더 축은 한 자리가 아예 비어 있었고, 본문 축은 세 자리 모두 **바이트 단언이 아니었다.**
 ...
 *
 * 판정을 세 벌 두지 않는 이유는 그 표 자체다 — 세 벌이면 한쪽만 고쳐지는 날 **같은 성질을
 * 서로 다른 강도로 재면서 셋 다 초록**이 된다.
 *
 * ## 제외 헤더도 여기 한 벌만 둔다
 *
...
object OwnershipConcealment {
    ...
    fun assertIndistinguishable(
        ...
        assertThat(headerNames(others))
            .withFailMessage(
                "%s: 헤더 이름 집합이 다르다 — 없는 것에만 %s, 남의 것에만 %s",
                label,
                headerNames(absent) - headerNames(others),
                headerNames(others) - headerNames(absent),
            ).isEqualTo(headerNames(absent))
    }
}
