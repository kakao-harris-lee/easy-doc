...
CREATE TABLE llm_calls (
    ...
    -- 단위 크레딧은 아래 `document_char_count` 에서 파생한다(계획 §2 결정 5 「크레딧」,
    -- 2026-09-08 리뷰로 `documents` 표 대신 이 열을 정본으로 삼도록 정정). 재변환은
    -- 문서 전체가 아니라 그 단위(`maskedUnitOf`)만의 길이다.
    char_count integer NOT NULL,
    -- 이 호출이 속한 **문서**의 `documents.char_count` 스냅샷(2026-09-08 리뷰 신설) —
    -- 위 `char_count`(호출별 마스킹 입력 길이)와는 다른 축이다. U2 집계가 문서 수·문자
