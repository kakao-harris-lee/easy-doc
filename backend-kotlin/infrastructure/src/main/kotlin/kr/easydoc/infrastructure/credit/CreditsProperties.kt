 ...
 *
 * [enforced] 코드 기본값은 기존의 격리 테스트·직접 조립 호환을 위해 `false`로 둔다.
 * 지원하는 Compose 운영 설정(`.env.example`)은 `true`이며, 미결제·만료·소진 계정은
 * 문서 등록이 402로 거절된다. 관리자도 같은 계정·구독 경계를 사용하며 예외가 아니다.
 * 꺼진 별도 회귀 환경에서도 예약·소비·거래는 **똑같이 기록된다** — 잔액이 음수가 될 수 있다.
 *
