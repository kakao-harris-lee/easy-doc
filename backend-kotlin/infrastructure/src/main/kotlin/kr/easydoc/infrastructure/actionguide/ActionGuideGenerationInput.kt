...
/** 원문과 현재 저장 본문을 같은 content_revision에서 읽는다. 평문은 로그에 남기지 않는다. */
class ActionGuideGenerationInput(
    val sourceText: String,
    val savedBody: String,
) {
    override fun toString(): String =
        "ActionGuideGenerationInput(source=${sourceText.length}자, body=${savedBody.length}자)"
}

fun interface ActionGuideInputSource {
    fun load(job: StoredActionGuideJob): ActionGuideGenerationInput?
}

/** provider 시작 뒤에도 소유권·보존 기간·본문 버전을 다시 확인하는 조회 전용 어댑터. */
class JdbcActionGuideInputSource(
    private val jdbc: JdbcClient,
    private val cipher: ContentCipher,
) : ActionGuideInputSource {
    override fun load(job: StoredActionGuideJob): ActionGuideGenerationInput? =
        jdbc
            .sql(INPUT_SQL)
