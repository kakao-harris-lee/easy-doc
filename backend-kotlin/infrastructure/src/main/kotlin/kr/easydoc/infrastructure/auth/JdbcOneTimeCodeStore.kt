...
import kr.easydoc.application.auth.OneTimeCodeStore
...
/**
 * 일회성 코드 테이블(`email_verification_codes`·`password_reset_codes`) 공통 접근 —
 * [table] 이름만 다르고 나머지 SQL·해싱·쿨다운·시도 상한 로직은 완전히 같다.
 *
 * `JdbcVerificationCodeStore`(스키마 `V7__email_verification.sql`)의 본문을 여기로 옮긴
 * 것이다(리뷰를 마친 SQL·상수 시간 비교·경쟁 방지 가드는 그대로다) — 비밀번호 재설정
 * (`V11__password_reset_codes.sql`)이 같은 메커니즘을 필요로 할 때 SQL을 다시 베끼는
 * 대신 이 기반 클래스를 상속해 [table] 만 바꾼다(`JdbcPasswordResetCodeStore`).
 *
 * [table] 은 이 모듈이 정한 두 상수 문자열만 받으므로 문자열 보간을 SQL에 직접 섞어도
 * 인젝션 표면이 생기지 않는다 — 값이 사용자 입력에서 오지 않는다.
 *
 * 함수 수 상한을 억제한다 — 원본 클래스 KDoc과 같은 근거(상수 시간 비교, 경쟁 방지
 * 가드가 작은 사유별 함수로 쪼갠 결과다).
 */
@Suppress("TooManyFunctions")
abstract class JdbcOneTimeCodeStore
    protected constructor(
        ...
        private fun toActiveCodeRow(rs: ResultSet): ActiveCodeRow =
            ActiveCodeRow(
        ...
        private fun consume(
            ...
            jdbc
                .sql("UPDATE $table SET consumed_at = :now WHERE id = :id AND consumed_at IS NULL")
                .param("now", now)
                .param("id", id)
                .update()
        }
        ...
        private fun rejectIfWithinCooldown(
            ...
            val lastIssuedAt =
                jdbc
                    ...
                    .query { rs, _ -> rs.getObject("created_at", OffsetDateTime::class.java).toInstant() }
                    .optional()
                    .orElse(null) ?: return
        ...
        private companion object {
            ...
            /** 계약 `POST /auth/email-verification/request` 429 예시. 두 어댑터가 공유한다. */
