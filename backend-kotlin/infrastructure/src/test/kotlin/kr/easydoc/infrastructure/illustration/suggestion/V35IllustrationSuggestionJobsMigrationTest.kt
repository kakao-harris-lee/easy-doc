...
class V35IllustrationSuggestionJobsMigrationTest {
    private val database: DatabaseHandle by lazy {
        PostgresTestSupport.createEmptyDatabase("v35_illustration_suggestion_jobs").also { migrate(it) }
    }
    ...
    @Test
    @DisplayName("V34 에서 이어온 행동 안내 예약도 V35 로 올린 뒤 문서 삭제에서 정확히 한 번 해제된다")
    fun `V34 에서 올라온 예약이 삭제에서 한 번만 해제된다`() {
        // V35 는 V29 의 정산 trigger 를 내리고 합친 trigger 를 세운다 — 빈 DB 가 아니라 **이미
        // 예약이 살아 있는** DB 에서 올려도 그 예약이 삭제 때 한 번만 해제되는지가 관건이다.
        val upgraded = PostgresTestSupport.createEmptyDatabase("v35_upgrade_from_v34")
    ...
    private fun seedActiveActionGuideJobAtV34(handle: DatabaseHandle): SeededV34 {
        ...
        handle.execute(
            ...
            VALUES ('${seeded.workspaceId}', '${seeded.ownerId}', 'V35 대상 공간');
