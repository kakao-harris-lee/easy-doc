...
internal class LaneConversionTrace(
    ...
    override fun complete(
        ...
        return try {
            delegate.complete(prompt, options).also { calls += TraceCall(prompt, it, null) }
        } catch (exc: LlmProviderException) {
            calls += TraceCall(prompt, null, exc.javaClass.simpleName)
            throw exc
        }
    }
    ...
    private class TraceCall(
        ...
        fun details(
            ...
            return mapOf(
                ...
                "systemPrompt" to prompt.system,
                "userPrompt" to prompt.user,
