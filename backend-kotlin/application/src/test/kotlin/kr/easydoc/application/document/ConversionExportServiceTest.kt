...
class ConversionExportServiceTest {
    ...
    fun `유도값도 선택지도 없으면 409 다`() {
        ...
        assertThat(trulyUnexportable)
            .describedAs("오늘은 모든 원본에 기본 내보내기 형식이 있다")
            .isEmpty()
    }

    @Test
    @DisplayName("PDF 원본은 형식을 생략하면 TXT 로 내보내고 저장된 PDF를 열지 않는다")
    fun `PDF 는 기본 TXT 신문서를 만든다`() {
        val world = World()
        val conversionId = UUID.randomUUID()
        world.seedDone(conversionId, Seed(easyText = "쉬운 글", sourceFormat = SourceFormat.PDF))
        world.seedOriginal(conversionId)

        val file = world.export(conversionId, null)

        assertThat(world.reflector.reflected)
            .describedAs("TXT 내보내기에 PDF 원본 구조를 반영하려 했다")
            .isEmpty()
        assertThat(world.exporter.calls.map { it.format }).containsExactly(ExportFormat.TXT)
    ...
    fun `형식이 완료보다 먼저다`() {
        ...
        val pdfMismatched = UUID.randomUUID()
        world.seedDone(mismatched, Seed(easyText = "본문", sourceFormat = SourceFormat.DOCX, status = pending))
        world.seedDone(pdfMismatched, Seed(easyText = "본문", sourceFormat = SourceFormat.PDF, status = pending))

        assertThatThrownBy { world.export(mismatched, ExportFormat.TXT) }
            .isInstanceOf(ConflictException::class.java)
            .hasMessage(EXPORT_FORMAT_MISMATCH_MESSAGE)
        assertThatThrownBy { world.export(pdfMismatched, ExportFormat.DOCX) }
            .isInstanceOf(ConflictException::class.java)
            .hasMessage(EXPORT_FORMAT_MISMATCH_MESSAGE)
    }
