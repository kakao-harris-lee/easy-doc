...
class RepairDecisionTest {
    ...
    @Test
    @DisplayName("위반이 늘지 않으면 채택한다 — 과잉 거부 가드")
    fun `지키면 채택한다`() {
        val decision =
    ...
    fun `건수를 보고한다`() {
        val decision = decideRepairAdoption(original = plain, candidate = plain, source = "")
    ...
    @Test
    @DisplayName("source 에 사실이 없으면(빈 문자열 명시) 사실 보존 게이트는 항상 통과한다")
