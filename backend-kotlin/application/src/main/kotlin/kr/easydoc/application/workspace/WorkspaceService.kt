...
class WorkspaceService(
     ...
     *
     * 계약은 둘 다 409 로 정했을 뿐 동시에 해당할 때 어느 쪽을 말할지는 정하지 않았다
     * (2026-08-19 확인). **마지막 하나**를 먼저 보는 이유는 그것이 **무조건적인 거절**이기
     * 때문이다 — 문서를 다 비워도 결과가 바뀌지 않는다. 반대로 골랐다면 사용자는 안내받은
     * 대로 문서를 지운 뒤 다시 거절당하고, 그 사이에 되돌릴 수 없는 파기를 이미 실행한
     * 뒤다. 계약이 이 자리에서 지키려던 것(*"무엇이 사라지는지 사용자가 먼저 보게 한다"*)이
     * 바로 그 파기이므로, 순서를 이렇게 두는 편이 조항의 뜻에 맞는다.
     */
    fun delete(
        ownerId: UUID,
        workspaceId: UUID,
