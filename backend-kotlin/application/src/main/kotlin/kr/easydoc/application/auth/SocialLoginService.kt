 ...
 *
 * `LongParameterList` 를 억제한다 — [emailVerification] 은 `AuthService.signup`(비밀번호
 * 가입)과 같은 이유로 필요하다: 네이버처럼 이메일이 미검증인 채로도 새 계정을 만들
 * 수 있는 제공자는(2026-09-05 결정, `x-social-login.providers.x-note`) 가입 뒤 같은
 * 이메일 인증 코드 절차를 타야 한다. `SocialLoginRepositories` 처럼 하나로 묶기엔 이
 * 저장소들과 성격이 달라(가입 부수 효과지 저장소가 아니다) 새 그룹을 만들지 않는다.
 */
...
class SocialLoginService
    ...
    constructor(
         ...
         * 갈래 셋:
         *  1. 이미 연결된 신원 → 로그인.
         *  2. 신원은 새롭지만 같은 이메일의 계정이 이미 있다 → 409(자동 연결하지 않는다 —
         *     명시적 연결은 [linkStart]/[linkCallback], 로그인한 뒤 계정 설정에서 잇는 별도
         *     흐름이다).
        ...
        private fun consumeLoginState(
            ...
            val consumed =
                states.consume(providerId, state, redirectUri)
        ...
        /**
         * 이메일이 있는지 본다. **네이버는 미검증이어도 통과한다** — 그 제공자엔 애초에
         * `email_verified` 개념이 없다(2026-09-05 결정, `x-social-login.providers.x-note`).
         * 다른 제공자는 여전히 검증까지 요구한다(OIDC `email_verified: true`). 통과하면
         * 정규화된 이메일을 돌려준다.
         *
         * **형식·길이도 `AuthService.signup`과 같은 규칙([requireValidEmail])으로 본다.**
         * 제공자가 준 값을 그대로 믿지 않는다 — 상한(255자)을 넘으면 `users.email
         * varchar(255)` 삽입이 500으로 새고, 형식이 깨지면 나중에 이메일 인증 코드 발송이
         * 영영 실패하는 계정이 생긴다. 위반은 스키마 형식 문구가 아니라 **같은
         * `email_required` 422 갈래**(제공자별 문구)로 옮긴다 — 호출자 관점에서는 "이
         * 제공자에게서 쓸 수 있는 이메일을 받지 못했다"는 같은 사실이기 때문이다.
         */
        private fun requireEmailPresent(
            identity: SocialIdentity,
            providerId: SocialLoginProviderId,
        ): String {
            val email = identity.email
            val mustBeVerified = providerId != SocialLoginProviderId.NAVER
            if (email.isNullOrBlank() || (mustBeVerified && !identity.emailVerified)) {
                throw InvalidInputException(emailRequiredMessage(providerId))
            }
        ...
        companion object {
            ...
            /**
             * 계약 `422` 예시 — 네이버는 이메일 자체가 없을 때만 이 갈래를 탄다(미검증은 더 이상
             * 막지 않는다, [requireEmailPresent]). 다른 제공자와 문구를 가르는 이유를 그대로
             * 안내한다 — 사용자 요청, 2026-09-05.
             */
            const val NAVER_EMAIL_REQUIRED_MESSAGE =
                "네이버 계정에 이메일이 없어 가입할 수 없습니다. 이메일로 가입하거나 다른 방법을 이용해 주세요"

            /** 제공자별 `email_required` 문구 — 네이버만 다르다([NAVER_EMAIL_REQUIRED_MESSAGE]). */
            fun emailRequiredMessage(providerId: SocialLoginProviderId): String =
                when (providerId) {
                    SocialLoginProviderId.NAVER -> NAVER_EMAIL_REQUIRED_MESSAGE
                    else -> EMAIL_REQUIRED_MESSAGE
                }

            /** 계약 `409` 예시 — 같은 이메일의 계정이 이미 있다(검증 여부와 무관, 자동 연결 안 함). */
