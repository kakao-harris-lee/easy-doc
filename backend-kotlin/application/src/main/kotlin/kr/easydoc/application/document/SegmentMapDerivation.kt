 ...
 *
 * [DocumentExporter] 와 같은 모양의 `fun interface` 다 — 실물은 하나([DefaultSegmentMapDerivation])
...
fun interface SegmentMapDerivation {
     ...
     *
     * [body] 는 두 호출부 모두 `edited_text ?: easy_text` 다(오늘 `ConversionQueryService:112`가
     * 그렇다).
     */
    fun deriveOrNull(
...
/**
 * [SegmentMapDerivation] 의 유일한 실물 — 원문과 본문에서 매번 다시 계산한다.
 *
 * **저장하지 않는다** — 매 호출마다 (원문, 본문)에서 [alignSegments] 로 다시 계산한다.
 *
 ...
 * 넘기고, 이 클래스는 **그 뒤의 계산**(복호화 → 정렬)만 공유한다.
 */
class DefaultSegmentMapDerivation(private val cipher: ContentCipher) : SegmentMapDerivation {
