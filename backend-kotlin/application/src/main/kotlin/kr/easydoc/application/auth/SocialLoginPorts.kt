...
/**
 * 지원하는 소셜 로그인 제공자. **오늘은 `google`·`kakao`·`naver` 셋이다**(계약 2.15.0) —
 * 계약 `paths./auth/oauth/{provider}/start.parameters[provider].schema.enum` 이
 * `[google, kakao, naver]` 를 연다. backlog §1.4 의 예고 목록(`reserved`)은 이제 비어
 * 있다 — 예고했던 값이 전부 열렸다.
 *
 * **문자열 → enum 해석은 이 타입이 하지 않는다.** API 경계의
 * `kr.easydoc.api.auth.SocialLoginProviderIdConverter`(Spring `Converter`)가 그 자리다 —
 * enum 밖 값은 스키마 층 422 **배열**로 나가야 한다(`ValueSlotInvariantReachTest`,
 * `ExportFormatConverter` 와 같은 자리). 이 서비스(`SocialLoginService`)는 이미 해석된
 * [SocialLoginProviderId] 만 받는다 — 원시 문자열을 다시 파싱하지 않는다.
 */
