...
interface OneTimeCodeStore {
    ...
    /**
     * [code]와 일치하는 활성 코드만 무효화한다. 외부 발송 실패 뒤 전달되지 않은 코드를
     * 회수할 때 쓴다 — **이 요청이 발급한 코드**만 지운다. `userId` 만으로 무효화하면
     * (이전 구현) 이 요청이 실패로 판정되는 사이 같은 사용자가 이미 재발급받아 전달까지
     * 끝낸 최신 코드를 함께 지워버린다(발급 → 발송 지연 → 재발급 → 발송 성공 → 원래
     * 요청의 실패 처리 순서로 겹치면 발생). 일치하는 활성 코드가 없으면(다른 요청이
     * 이미 재발급했다면) 아무것도 하지 않는다 — 기본 구현을 두지 않는다: 이 포트의
     * 모든 구현체가 이 스코프를 명시적으로 지키게 한다([UserRepository]의 3개 신규
     * 메서드가 조용한 no-op 대신 `error(...)` 기본값을 두는 것과 같은 근거).
     */
    fun revoke(
        userId: UUID,
        code: String,
