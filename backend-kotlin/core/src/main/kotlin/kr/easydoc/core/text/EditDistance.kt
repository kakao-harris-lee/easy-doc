 ...
 * ## O(n·m) 이 성립하는 근거
 *
 * 이 함수가 받는 두 문자열은 변환 대상 본문과 그 검수 수정본이고, 양쪽 다
 * [kr.easydoc.core.document.MAX_CONVERTIBLE_CHARS](4,000자)로 **이미 막혀 있다**. 그래서
 * 최악이 4,000 × 4,000 = 1,600만 칸으로 상한이 있고, 입력 길이가 무한한 자리가 아니다.
 * 그 상한이 사라지면(예: 더 긴 문서를 받게 되면) 이 구현의 비용 판단을 다시 해야 한다.
 *
 * 표는 **두 행만** 들고 굴린다. 전체 표(n×m)를 잡으면 상한에서 1,600만 개의 `Int` 가 되고,
