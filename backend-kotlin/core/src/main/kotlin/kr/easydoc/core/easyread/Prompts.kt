...
/**
 * [LENGTH_INSTRUCTION]의 "정보는 하나도 버리지 말고 문장 수를 늘려 나눠 담으세요"는
 * **한 문장을 나눌 때**의 지시라, 여러 줄이 같은 모양으로 반복되는 표·나열 자체를 모델이
 * 통째로 요약해 버리는 실패를 막지 못한다. 6차 유료 측정에서 문서 `022`가 「운행 기간별
 * 되거두는 비율」 표(6~48개월 × 70~30%)를 통째로 버리고 「보조금을 취소하고 돈과 이자를
 * 돌려받습니다」 한 문장만 남긴 것이 그 사례다(`docs/plans/2026-09-09-content-loss.md` S4 ⑴) —
 * 시민이 얼마를 토해내야 하는지가 사라졌다. 그래서 표·나열을 문장 나누기와 별도 절로 세워
 * "항목을 하나도 빠뜨리지 말라"를 명시한다.
 *
 * 예시는 골든셋 본문이 아니라 일반 행정 문투로 새로 지었다(과적합 방지, [SPLIT_EXAMPLES] 와
 * 같은 방침).
 */
internal const val TABLE_INSTRUCTION =
    "원문에 표가 있거나 항목마다 값이 다른 나열이 있으면 그 항목을 하나도 빠뜨리지 말고 모두 옮기세요.\n" +
        "기간·구간·대상에 따라 값이 달라지는 표가 특히 그렇습니다. " +
        "'기간에 따라 다릅니다'처럼 뭉뚱그린 한 문장으로 줄이면 안 됩니다. " +
        "그렇게 줄이면 읽는 사람이 자기에게 해당하는 값을 알 수 없습니다.\n" +
        "표는 줄을 바꿔 한 줄에 한 항목씩 적고, 각 줄에 그 항목의 조건과 값을 함께 적으세요.\n" +
        "예시\n" +
        "원문 표: 가입 기간 1년 미만 5% / 1년 이상 3년 미만 3% / 3년 이상 1%\n" +
        "쉬운 글:\n" +
        "가입한 지 1년이 안 됐으면 5%입니다.\n" +
        "가입한 지 1년 이상 3년 미만이면 3%입니다.\n" +
        "가입한 지 3년이 넘었으면 1%입니다.\n" +
        "항목이 많아 줄이 늘어나도 괜찮습니다. 줄 수를 줄이려고 항목을 묶거나 빼지 마세요."
...
internal val SELF_CHECK_INSTRUCTION =
    "출력하기 전에 아래를 스스로 확인하고, 어긋나는 곳은 고친 뒤에 최종 결과만 출력하세요.\n" +
        ...
        "8. 원문에 있던 표나 나열의 항목이 결과에 하나도 빠짐없이 남아 있는가? " +
        "항목 수를 세어 원문과 맞는지 확인하고, 여러 항목을 한 문장으로 묶어 줄인 곳이 있으면 되돌린다."
