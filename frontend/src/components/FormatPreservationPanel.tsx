 ...
 *    없고, 고른 형식으로 검수한 내용만 담은 **새 문서**를 만든다 — 원본 레이아웃은
 *    반영되지 않는다(`DESIGN.md` §6.5 2026-09-02, `FormatPreservationStatus.not_applicable`
 *    ⑷). 「준비 중」이라고 쓰지 않는다 — 이것은 이미 되는 일이다.
 * ⑵ **고를 형식도 없다**(오늘은 없는 갈래) — 내려받기 버튼 자체가 없다는 사실을 말한다.
 *    **「준비 중」이라고도 쓰지 않는다.** PDF를 같은 PDF로 다시 만드는 것은 미구현이
 *    아니라 하지 않기로 정해진 범위다 — 곧 될 것처럼 적으면 오지 않을 기능을 기다리게
