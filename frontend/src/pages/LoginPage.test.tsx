...
describe('로그인 화면', () => {
  it('휴대폰 인증 뒤 체험 5크레딧을 받는 전체 순서를 보여준다', () => {
    ...
    const login = screen.getByRole('region', { name: '로그인' })
    const intro = screen.getByRole('complementary')

    expect(
      within(login).queryByRole('region', { name: '무료 체험 시작 순서' }),
    ).not.toBeInTheDocument()
    expect(within(intro).getByRole('region', { name: '무료 체험 시작 순서' })).toBe(guide)
...
describe('카카오 계정 연결 이어가기 (?link=kakao)', () => {
  it('로그인에 성공하면 연결 시작 요청을 보내고 인가 URL로 이동한다 — 홈으로는 가지 않는다', async () => {
    const user = userEvent.setup()
    vi.mocked(login).mockResolvedValue({
      access_token: 'token-abc',
