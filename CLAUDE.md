...
- **현재 Python 구현의 출력을 정답으로 삼아 Kotlin 결과를 거기에 맞추기** — Python은 참고 구현이고 기준은 요구사항·정책 충족이다(master-plan 6.2). 동작이 갈리면 어느 쪽이 요구사항에 맞는지 판단해 기록한다. 예외 둘: 정책 불변식(마스킹 선행 등)과 기존 데이터 호환(Fernet 암호문·JWT·Argon2)은 동일성 자체가 요구사항이다
...
Kotlin 이전 작업의 완료 판정은 **"현재 Python과 같은 값이 나오는가"가 아니라 "요구사항·정책(master-plan 3장·4장)을 충족하는가"**다. Python과 결과가 다르면 그 자체를 결함으로 보지 말고, 어느 쪽이 요구사항에 맞는지 판단해 근거와 함께 남긴다 — Python 쪽이 틀린 경우도 전제한다. 단 마스킹 선행·no-store·소유권 은닉 같은 정책 불변식과 암호문·토큰 호환은 예외 없이 동일해야 한다.
