...
- **목표·마일스톤·로드맵·스팩은 세션 모델(Opus 5 [1m] 또는 Fable 5.1 [1m]) 하나가 단독으로 쓴다(2026-09-04).** planner·analyst·critic·deep-reasoner·brainstorming 같은 다단계 기획 파이프라인이나 Codex 검토를 붙이지 않는다. 세션 모델이 코드·계약·backlog를 직접 읽고 `docs/**`의 기획 문서를 직접 타이핑한다. 이유: 기획을 여러 에이전트로 쪼개면 컨텍스트가 흩어져 스팩 하나에 몇 시간이 걸렸다. 1M 컨텍스트 모델 하나가 보는 편이 빠르고 싸다.
