...
- **Python 코드를 명세로 옮기기 전에 지우기** — 재개발에는 Python이라는 차분 그물이 없어 **누락이 곧 영구 손실**이다. 프롬프트 전문·스타일 규칙 상수·치환 비문 검출 튜닝값은 코드에만 있다. 추출 목록은 `docs/migration/_workspace/03_rebuild-extraction-list.md`
