...
| 2026-08-14 | 규칙 5 복원 절차의 전제 갱신 — `cp -iv` 별칭 제거(사용자), 규칙은 유지 | skills/kotlin-migration(규칙 5) | 별칭 제거로 프롬프트 정지 기제는 소멸(실측 `type cp` → `/bin/cp`). 규칙을 유지하는 근거: 같은 날 세 번째 사고가 별칭과 무관한 형태를 보임 — `cp` 백업의 조용한 실패로 낡은 사본 복원, 케이스 하한 151→108 삭감(`f73b8bd`). `cp` 복원의 근본 결함은 프롬프트가 아니라 **검증 없음**이며 git 경유 + sha256 대조가 별칭 유무와 무관하게 그것을 증명한다 |
