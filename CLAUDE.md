...
- **구현 모델은 Sonnet 5다.** 코드 작성, 테스트, 디버깅, 리팩터링, 설정 변경, 문서 갱신에 Opus를 사용하지 않는다.
- 기본 모델은 `.claude/settings.json`의 `sonnet`이며, 구현 중 상위 모델로 자동 승격하지 않는다.
- **Opus 5는 사용자가 명시적으로 요청한 계획·아키텍처 판단에만 허용한다.** Opus 세션은 읽기와 계획 작성만 하고 파일 수정·테스트·하위 에이전트 호출 전에 종료한다.
- 계획은 Codex를 우선 사용해도 충분하다. 계획이 끝난 뒤 구현은 새 Sonnet 세션에서 수행한다.
- 비용이 불명확한 모델 fallback, 백그라운드 에이전트, 병렬 에이전트, 재귀 위임을 사용하지 않는다.
...
- 프로젝트의 `Agent`·`Skill` 도구는 설정에서 차단한다. 오케스트레이션·하네스·스킬 체인을 만들거나 호출하지 않는다.
- 주 에이전트가 요청 범위를 직접 읽고 직접 구현한다. 구현을 다른 모델이나 에이전트에 위임하지 않는다.
- 한 번에 요청된 한 단위만 처리한다. 범위가 크면 자동 분해·병렬 실행하지 말고 비용이 작은 순서로 나눌 경계를 사용자에게 제시한다.
- 기존 Gradle/npm/Compose 검증을 사용한다. 에이전트용 검증 하네스, fingerprint, 별도 리뷰 루프를 만들지 않는다.
- 동일 사실을 확인하려고 저장소 전체를 반복 탐색하지 않는다. 요청 경로 → 관련 테스트 → 계약 순으로 한 번씩 좁혀 읽는다.
