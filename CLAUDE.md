...
- **Opus 5 이상(Fable 5 포함)은 계획·아키텍처 판단과 오케스트레이션에 허용한다.** 상위 모델 세션이 코드를 직접 고치는 대신, 범위를 나눠 Sonnet 실행 에이전트에 위임하고 결과를 종합한다. 「생각은 상위 모델, 타이핑은 Sonnet」이 코드의 기본형이다.
- **목표·마일스톤·로드맵·스팩은 세션 모델(Opus 5 [1m] 또는 Fable 5.1 [1m]) 하나가 단독으로 쓴다(2026-09-04).** planner·analyst·critic·deep-reasoner·brainstorming 같은 다단계 기획 파이프라인을 붙이지 않는다. Codex 계획 검토는 저작의 일부가 아니며 사용자가 필요할 때 명시 요청한다. 세션 모델이 코드·계약·backlog를 직접 읽고 `docs/**`의 기획 문서를 직접 타이핑한다. 이유: 기획을 여러 에이전트로 쪼개면 컨텍스트가 흩어져 스팩 하나에 몇 시간이 걸렸다. 1M 컨텍스트 모델 하나가 보는 편이 빠르고 싸다.
- **Codex 심판의 코드 범위는 되돌리기 어려운 경로로 한정한다(2026-09-11 — 2026-09-04의 전면 제외를 대체).** 결제·인증/인가·암호화·비밀번호/토큰·DB 마이그레이션·데이터 파기처럼 사고 시 복구가 없는 변경은 사용자가 범위·비용을 승인하면 Codex 심판에 올릴 수 있다. 그 밖의 코드 diff·PR은 Codex로 보내지 않고 Claude 측 리뷰어(sonnet)가 저작 에이전트와 다른 패스로 본다. 전면 제외의 이유였던 외부 유료 호출 빈도는 이 범위 제한으로 억제한다.
...
- 오케스트레이션 계층 파일(`CLAUDE.md`, `AGENTS.md`, `.claude/**`)과 기획 문서(목표·마일스톤·로드맵·스팩, `docs/plans/**`·backlog·master-plan)는 주 에이전트 직접 수정을 허용한다. 위 "문서 갱신 타이핑 금지"는 제품 문서(README·API 문서)와 `contracts/**`, 코드에 적용된다. 제품 코드(`backend-kotlin/`, `frontend/`)는 예외 없이 Sonnet 실행 에이전트가 타이핑한다.
- 저작과 검토는 다른 패스로 둔다. 구현한 에이전트가 같은 컨텍스트에서 자기 결과를 승인하지 않는다.
- 다른 모델 계열의 독립 검증·리뷰는 `.claude/agents/codex-reviewer.md` 레인 몫이지만, **코드는 되돌리기 어려운 경로(결제·인증/인가·암호화·마이그레이션·데이터 파기)만 심사 대상이다(2026-09-11 — 2026-09-04의 전면 제외를 대체).** 그 경로의 코드와, 기획 문서·계약 파일 같은 코드 외 산출물을 사용자가 명시적으로 지정한 경우에만 타며, 유료 외부 LLM 호출이라 위 모델·비용 정책의 승인 규칙을 그대로 따른다. 디스패치 조건은 하나뿐이다: **디스패치 전에 오케스트레이터가 심사 범위(대상 diff·파일)와 유료 Codex 호출임을 제시하고 사용자의 확인을 받는 것.** 그 확인이 그 1회 호출의 비용·범위 승인이다. 리뷰 요청도, Codex를 지명한 요청도, 커밋·구현 완료도 이 관문을 대체하지 않는다 — 요청 문구는 레인 선택일 뿐이다. 승인 없는 리뷰 요청은 Claude 측 리뷰어로 처리하고, 승인 범위를 넘는 반복 호출은 금지다.
- 기존 Gradle/npm/Compose 검증을 사용한다. 에이전트용 검증 하네스, fingerprint, 별도 리뷰 루프를 새로 만들지 않는다. 단 codex 심판 레인의 「심사 대상 리비전 포착」(reviewed_at_head·scope digest)은 레인 절차의 일부로 예외다 — 승인과 코드 상태를 결속하는 최소 장치이며 새 하네스가 아니다.
