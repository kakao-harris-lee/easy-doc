...
- **Codex 심판에서 코드 부분은 제외한다(2026-09-04).** 코드 diff·구현 결과·PR은 Codex로 보내지 않는다. 코드 리뷰는 Claude 측 리뷰어(sonnet)가 저작 에이전트와 다른 패스로 본다. 외부 유료 호출이 너무 잦아진 것이 이유다.
