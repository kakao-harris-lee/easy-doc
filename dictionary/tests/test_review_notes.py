...
# schema.sql. 회귀 재현(TestEnsureVEntryFullView)에 실물 구버전 스키마가
# 필요해서 손으로 다시 옮겨 적는 대신 커밋된 fixture로 고정해 읽는다. CI가
# shallow checkout(git history 없음)으로 돌기 때문에 `git show`로 매번 읽을
# 수 없다.
