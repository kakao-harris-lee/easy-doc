...
### 법제처 법령용어 목록 — 원천이 아니다 (저장소 밖 보관)

`법제처_국가법령정보센터_법령용어_20240912.csv`(11.6만 행)는 **적재하지 않는다.** 대치어(순화어) 컬럼이 없어 `easy_term`을 채울 수 없기 때문이다. 이 원천 단독의 적재 가능 엔트리는 0건이다.

**대체 용도도 없다.** 갭 리스트의 "잘린 조각"(`국민기초생활 보장법`이 `국민기초생활`/`보장법`으로 쪼개지는 문제, [tools/README.md](./tools/README.md))을 판정할 법령용어 화이트리스트로 쓰자는 안이 있었으나 **실측으로 기각했다.** 양방향으로 깨진다 — 걸러내야 할 `국민기초생활`이 그 자체로 등재돼 있어 조각을 통과시키고(`시행령`·`시행규칙`·`신청방법`도 등재), 반대로 골든 코퍼스의 법령명 후보 25건 중 원형 일치는 6건, 공백을 지우고 맞춰도 9건뿐이라 진짜 법령명을 거부한다. **이 목록은 법령 「제목」 목록이 아니라 법령에 「쓰이는 어휘」 목록**이라서 그렇다.

그래서 **파일은 저장소 밖 `~/moleg-legal-terms/`로 옮겼다** — 다듬은말(`~/korean-refined-words/`)과 같은 처분이다. 검증된 용도가 없는 2.7MB를 저장소에 두면 다음 사람이 "쓰라고 둔 것"으로 읽기 때문이다. 다만 역할은 다르다 — 다듬은말은 실측으로 확인된 후보 풀이고, 이건 **검증된 용도가 없는 보관물**이다. 판단 근거와 "탈락한 원천에 대체 용도를 붙일 때도 그 용도를 실측해야 한다"는 교훈은 [DESIGN.md §5.7](./DESIGN.md#57-원천-평가-기준-순화-목록은-쉬운-말-목록이-아니다)에 있다.
...
정본을 다시 만들 때 내보내기 전에 [검수 SQL](data/reviews/2026-09-11-reading-level.sql)을 적용한다. 현재 정본과 배포 산출물에는 적용되어 있다. `링크(rink)`의 운동장 후보 두 건은 웹 링크와 뜻을 구분할 수 없어 폐기 상태로 보존하고 배포 색인에서는 제외한다. `거주`는 `살다`라는 뜻과 활용 주의로 수정했다. 수정한 예문은 사람이 검수한 것으로 표시하지 않는다. 원천 행정용어 CSV는 변경하지 않았다.

```sh
sqlite3 dist/easy_dict.sqlite3 < data/reviews/2026-09-11-reading-level.sql
```

Kotlin 제품의 컨텍스트는 이제 정의·주의사항을 함께 제공하고 `is_golden=true`인 예문만 사용한다. 기존 Python 참조 출력과의 문자열 동등성을 목표로 하지 않는다. 프롬프트와 고정 문서 픽스처는 Kotlin 테스트로 관리하며, 이전 사전 효과 측정은 새 프롬프트 품질을 입증하지 않는다.

## 2026-09-12 생성 컨텍스트 재검증
...
easy-doc(Kotlin)의 R3 생성 컨텍스트와 R6 설명 패널은 **사람이 읽고 승인한 뜻풀이만** 쓴다. 배포 색인(`easy_dict.index.json`)의 엔트리에 `"v": "reviewed"`가 붙어 있는지로 그것을 가른다 — 키가 없으면 미검수(`DefinitionReviewStatus.UNVERIFIED`)이고, `"reviewed"` 아닌 값이 오면 제품이 기동을 거부한다.
...
1. **검수 큐를 뽑는다.** 배포되는 엔트리 중 뜻풀이가 있는 것 전부(현재 423건)를 CSV로 만든다. 뜻풀이가 그대로 사용자·LLM에게 설명으로 나가는 `replace_strategy=keep`·`risk_level=high`(현재 43건)가 앞에 오고, 나머지는 표제어 가나다순이다. Excel이 한글을 깨뜨리지 않게 UTF-8 BOM을 붙인다.
   ...
   이번 회차 큐는 `data/reviews/2026-09-23-definition-review-queue.csv`로 커밋되어 있다(423행). 컬럼은 `term, easy_term, definition, replace_strategy, risk_level, sources, reviewed_at, reviewed_by`이고, 뒤 두 칸이 검수자가 채우는 자리다.
...
3. **검수 SQL을 쓴다.** `data/reviews/2026-09-23-reviewed-definitions.template.sql`(전부 주석이라 그대로 실행하면 no-op)을 복사해 `data/reviews/<검수일>-reviewed-definitions.sql`을 만들고, 2단계에서 채운 표제어만 `WHERE term IN (...)`(같은 표제어에 엔트리가 여럿이면 `id`)에 넣는다.

   ```bash
   sqlite3 dist/easy_dict.sqlite3 < data/reviews/<검수일>-reviewed-definitions.sql
   ```

4. **내보낸다.** 정본을 다시 만들 때는 검수 SQL을 2026-09-11 → 2026-09-12 → 이 파일 순서로 적용한 뒤 `export_all`을 돌린다. 기존 `dist/easy_dict.sqlite3`처럼 이 컬럼이 없던 시절 만들어진 DB는 `easydict.definition_review.ensure_definition_review_columns()` + `easydict.review_notes.ensure_v_entry_full_view()`가 `ALTER TABLE`로 메운다(`review_note` 때와 같은 처방, 재실행 안전).

5. **백엔드에 옮긴다.** `dist/easy_dict.index.json`을 `backend-kotlin/infrastructure/src/main/resources/dictionary/`로 복사한다. Gradle의 `checkDictionaryIndex`가 두 파일의 바이트 동일성을 확인하므로, 이 복사를 빠뜨리면 백엔드 빌드가 실패한다.
