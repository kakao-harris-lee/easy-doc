...
#: (DEFAULT_MAX_TOKENS 16,000 — Anthropic에서는 사고 토큰과 나눠 쓴다) 안에 들어와야 하고,
#: 넘으면 LLM 호출 비용을 다 치른 뒤 절단(LLMTruncatedError)으로 실패한다. 돈을 쓰기 전에
#: 업로드 시점에 거절하는 편이 정직하다.
#:
...
class DocumentService:
    ...
    async def export_conversion(
        ...
        **마스킹을 되돌리는 곳은 이 메서드뿐이다.** 내보내기는 담당자가 그대로 배포할
        문서라 `[[주민등록번호1]]`이 남아 있으면 쓸 수 없다. 조회 응답·목록·로그를 비롯한
