...
class FakeProvider(LLMProvider):
    ...
    responses에 Exception을 섞으면 그 차례에 해당 예외를 던진다 — 실패 경로 테스트용.
    자원을 쥐지 않으므로 aclose()는 기본 no-op을 그대로 쓴다.
    """
    ...
    async def complete(
        ...
        *,
        ...
        self.calls.append(
            FakeCall(system=system, user=user, max_tokens=max_tokens, temperature=temperature)
        )
