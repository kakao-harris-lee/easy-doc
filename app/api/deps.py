...
from app.services.documents import ConversionStore, DocumentService, DocumentStore, WorkspaceLookup
from app.services.workspaces import WorkspaceService, WorkspaceStore
...
def get_workspace_repository(
    ...
    """작업 공간 저장소 (쓰기 계약). 테스트는 이 의존성을 fake로 갈아끼운다."""
    return WorkspaceRepository(session)


def get_workspace_lookup(session: Annotated[AsyncSession, Depends(get_session)]) -> WorkspaceLookup:
    """작업 공간 저장소 (읽기 계약).

    문서 서비스에는 읽기 계약만 넘긴다 — 같은 구현이지만 선언한 타입이 다르면, 문서
    경로에서 작업 공간을 만들거나 지우는 호출은 타입 검사에서 걸린다. 세션은 요청 단위로
    공유되므로 저장소 객체가 둘이어도 같은 트랜잭션을 본다.
    """
    return WorkspaceRepository(session)
...
def get_auth_service(
    ...
    workspaces: Annotated[WorkspaceCreator, Depends(get_workspace_repository)],
    ...
    작업 공간 저장소를 함께 받는 이유는 가입이 기본 작업 공간까지 만들기 때문이다.
    사용자 저장소와 **같은 요청 세션**을 쓰므로 커밋 하나로 둘이 함께 확정된다.
    """
...
def get_workspace_service(
    workspaces: Annotated[WorkspaceStore, Depends(get_workspace_repository)],
) -> WorkspaceService:
    """작업 공간 서비스 조립."""
...
def get_document_service(
    ...
    workspaces: Annotated[WorkspaceLookup, Depends(get_workspace_lookup)],
