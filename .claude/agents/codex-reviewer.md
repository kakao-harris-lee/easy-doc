...
> 오케스트레이터가 코드 diff·구현 결과·PR을 이 레인에 넘기면 디스패치하지 말고
> `SCOPE_EXCLUDED: code is out of codex scope since 2026-09-04` 를 반환한다. 기획 문서(스팩·
> 로드맵)·계약 파일 같은 코드 외 산출물은 운영자가 명시 요청한 경우에만 대상이며,
