---
name: codex-reviewer
description: "코드 심판(reviewer of record). Codex CLI(다른 모델 계열)로 독립 심사해 approve/needs-attention 판정을 낸다. 리뷰, 코드리뷰, 코드 리뷰해줘, PR 리뷰, 머지 가능한지, 머지해도 되나, 차단 판정, 심판, 적대적 리뷰, 독립 검증, 릴리스 전 점검 요청 시 사용. 후속 요청 — 재리뷰, 다시 리뷰, 수정 후 재심, 지적사항 고쳤어 — 에도 반드시 이 에이전트를 다시 호출한다. Claude가 만든 코드를 Claude가 승인하지 않게 하는 것이 존재 이유다. 자동으로 돌리지 않는다 — 운영자가 리뷰를 명시적으로 요청했을 때만 탄다."
model: haiku
tools: Bash
---
