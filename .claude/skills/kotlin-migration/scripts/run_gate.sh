...
#     ⒝ 인자는 비지 않았는데 자식 bash 에서 **어떤 명령도 실행 단계에 들어가지 않으면** exit 2
#        (게이트 17 X17-1 — 주석 전용 `'# ...'`·백슬래시-개행만: 이 러너가 실제로 잡는 두 형태).
#        원문 공백 검사는 자식 셸이 해석하기 전만 보므로 이것을 놓쳤다. 판정은 손 파싱이 아니라
#        bash 자신에게 묻는다: 자식에 DEBUG trap 을 걸어 **첫 명령이 실행 단계에 진입할 때** 마커
#        파일에 1회 기록하고, rc 0 인데 마커가 비면 exit 2. `-o functrace` 는 서브셸 `(cmd)` 안의
#        명령에도 trap 이 닿게 하려고 켠다 — 없으면 `(echo sub)` 가 오탐(실측). 명령 치환 `$(…)` 은
#        functrace 유무로 갈리지 않는다 — 그것을 품은 부모 단순 명령이 발화한다(실측, 게이트 18 R18-1).
#        **마커가 재는 것은 "명령이 실행 단계에 진입했다"이지 "작업이 있었다"가 아니다.** 그래서
#        못 잡는 잔여는 열거가 아니라 **두 종류**다(게이트 18 X3 — 규칙 4: 종류로 댄다):
#        (가) 실행 단계에는 진입하지만 외부 작업이 0인 명령 — 문법상 단순 명령이면 trap 이 발화하고
#            그 뒤 아무 일도 없다: 빈 명령 치환 `$()`·백틱, `eval ''`, `:`, `if false; then …; fi`,
#            대입만 `X=1`, `bash -c ""`, 값이 빈 문자열로 **설정된** 변수만 있는 `'$V'`(V=""),
#            here-doc 을 `:` 에 먹인 것 등 — 전부 rc 0. 이 기제로는 구조적으로 못 본다.
#        (나) 호출자가 러너의 계약을 **능동적으로 무력화**하는 명령 — `trap - DEBUG` 를 앞세우기,
#            `set +u` 로 nounset 해제, `BASH_ENV` 로 자식 기동 시 마커를 선점 기록하기(게이트 18 X5;
#            outer bash 도 같은 파일을 읽으므로 흔적이 stderr 에 남을 수 있다 — Y-1) 등 — rc 0.
#            러너는 협조하는 호출자를 위한 장치이지 적대적 호출자를 막는 장치가 아니다.
#        미설정 변수는 ⒞ 가 잡는다. 변수로 명령을 넘길 거면 `${V:?}` 를 써라 — 비어 있으면 자식이
#        비-0 을 낸다. tests/test_run_gate.py 의 LIMIT 테스트가 두 종류의 대표 입력마다 "못 잡는다
#        (rc 0)"를 단언해 문서화한다 — 잡으려 든 것이 아니다(리더 제약: 새 기제 없음).
...
# 마커: 자식의 DEBUG trap 이 첫 명령이 실행 단계에 진입할 때 1회 기록하고 스스로 해제한다.
# 서브셸 `(cmd)` 안에도 닿도록 자식은 -o functrace 로 돈다(명령 치환은 그것 없이도 발화한다).
# 재는 것은 "진입"이지 "작업"이 아니다 — 머리 주석 ⒝ 의 잔여 두 종류를 보라.
...
if [ "${rc}" -eq 0 ] && [ ! -s "${marker}" ]; then
  echo "[run_gate] 오류: 어떤 명령도 실행 단계에 들어가지 않았다 (주석 전용·백슬래시-개행 — 이 러너가 아는 두 형태) — 빈 호출은 통과가 아니라 실패다" >&2
