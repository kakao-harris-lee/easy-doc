...
5. **닫은 뒤 음성 대조를 붙인다.** 장치를 떼면 정확히 무엇이 깨지는지 확인한다. 안 깨지면 그 장치는 검증된 것이 아니다. 이 저장소는 음성 대조로 "절반이 빈 채 초록"(오류 본문)과 "생존 14건"(품질 게이트)을 찾았다.
   ...
   **게이트 명령은 러너 경유 또는 파이프 금지.** 파이프라인의 종료 코드는 마지막 명령의 것이라 `pytest | tail` 은 pytest 가 빨간데도 0 으로 끝난다 — 실제로 `e90cfe4` 가 빨간 가드인 채 커밋·푸시됐고(60초 뒤 `614afed` 정정), 같은 회차 리뷰어 자신도 두 번 빠졌다(macOS 에 `timeout` 부재로 실패했는데 `EXIT=0`, zsh 에서 `PIPESTATUS` 는 빈 값 — zsh 는 소문자 `pipestatus`·1-기반). CLAUDE.md 규칙 4 근거 4번(대리 지표가 성공 코드를 만든 원장 기록 사고)과 같은 구조의 두 번째 발생이다(게이트 15 X3). 그러므로 종료 코드를 판정 근거로 삼는 명령(게이트 실행·음성 대조 포함)은 **파이프 없이 직접 돌리고**, 출력 절단 등으로 파이프가 필요하면 **`.claude/skills/kotlin-migration/scripts/run_gate.sh '<명령 문자열 하나>'` 를 경유한다** — 러너는 그 문자열을 `bash -o pipefail` 아래에서 돌려 파이프 안 어느 단계의 실패든 비-0 으로 전파하고, 실행한 명령과 종료 코드를 stdout 에 남긴다(탐지형 — 면제 목록 없음. 인자는 **인용형 문자열 정확히 하나** — argv 나열형은 인용 경계를 잃어 거부된다. 빈·공백 인자는 exit 2). **러너 호출 자체를 파이프에 태우면 무효다** — `run_gate.sh 'pytest' | tail` 은 파이프를 호출 측 셸이 러너보다 먼저 만들어 outer 종료 코드가 다시 `tail` 의 것이 된다(게이트 16 B — 러너가 구조적으로 못 보는 자리). 파이프는 반드시 **따옴표 안**에 넣어라. 인자에 비밀값을 넣지 마라 — 전문이 stdout 에 기록된다. 러너의 계약은 `tests/test_run_gate.py` 가 고정하고(`ci:quality` 경로 명시), 자기 도달·한계 세 상태의 정본은 스크립트 머리 주석이다 — 여기 옮겨 적지 않는다.
