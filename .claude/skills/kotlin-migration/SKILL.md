...
5. **닫은 뒤 음성 대조를 붙인다.** 장치를 떼면 정확히 무엇이 깨지는지 확인한다. 안 깨지면 그 장치는 검증된 것이 아니다. 이 저장소는 음성 대조로 "절반이 빈 채 초록"(오류 본문)과 "생존 14건"(품질 게이트)을 찾았다.
   ...
   **[2026-08-14 전제 갱신 — 규칙은 유지]** 사용자가 별칭을 제거해 `cp` 는 이제 `/bin/cp` 그대로다(실측: `zsh -ic 'type cp'` → `/bin/cp`). 프롬프트 정지 기제는 사라졌지만 **규칙은 그대로 둔다** — 같은 날 세 번째 사고가 별칭과 무관한 형태를 이미 보였다: 별칭 시절 `cp` 백업이 `not overwritten` 으로 조용히 실패해 **낡은 사본**이 남았고, 그것으로 복원하자 케이스 하한이 151 → 108 로 깎였다(`f73b8bd` 커밋 메시지·`02_parity-verifier_case-identity.md` §9). `cp` 복원의 근본 결함은 프롬프트가 아니라 **검증 없음**이다 — 무엇을 덮었는지, 사본이 최신인지 아무것도 증명하지 않는다. git 경유 복원과 sha256 대조는 별칭 유무와 무관하게 그것을 증명한다.
