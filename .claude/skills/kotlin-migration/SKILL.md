...
5. **닫은 뒤 음성 대조를 붙인다.** 장치를 떼면 정확히 무엇이 깨지는지 확인한다. 안 깨지면 그 장치는 검증된 것이 아니다. 이 저장소는 음성 대조로 "절반이 빈 채 초록"(오류 본문)과 "생존 14건"(품질 게이트)을 찾았다.
   **복원은 `cp` 로 하지 마라.** 이 환경의 `cp` 는 `cp -iv` 로 별칭돼 있어 **`-f` 를 줘도 덮어쓰기 프롬프트가 뜬다.** 무인 복원을 기대한 스크립트는 거기서 멈추고, **변조된 파일이 디스크에 남은 채 아무도 모른다.** 2026-08-13 하루에 두 번 났다 — 하나는 15시간 54분 대기했고(그 백업이 전날 것이라 완료됐다면 그날 작업이 통째로 되돌아갔다), 다른 하나는 47분 대기했는데 **하필 그 파일이 가드 본체**였고 같은 시각 기준선 기록 실행이 그것을 쓰고 있었다.
