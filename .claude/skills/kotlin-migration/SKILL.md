...
이 하네스에서 같은 실패가 **일곱 번** 났다. 형태는 전부 같다 — **선언한 범위와 실제 도달 범위가 다른데 아무도 재지 않았다.** 여섯은 도달이 선언보다 **좁았고**, 하나는 규칙이 근거보다 **넓었다.** 좁은 쪽만 기억하면 이 절이 "넓게 잡아라"로 읽혀 정확히 반대로 고장난다.
...
4. **범위는 근거를 넘지 않는다.** 열거식이 실제로 두 번 누락된 실측이 있으면 전역이 정당하고, 한 번 겪은 일에는 그 한 자리만 막는다. **넓은 규칙이 안전해 보이는 것은 착시다** — 넓은 규칙은 신호를 끈다. 파일 하나가 문제인데 전역 무시 패턴을 넣으면 다음 사고는 아예 보이지 않는다.
...
| 7 | **(반대 방향)** `.gitignore`로 사고 재발을 막는다 | 파일 하나가 문제였는데 전역 패턴 `*.jso`를 넣어, 다음에 같은 사고가 나도 `git status`에 보이지 않게 만들었다 — **보이는 이상 징후를 안 보이는 것으로 바꿨다** | 규칙의 범위를 근거와 대조해 발견 |

1~6은 도달이 좁았던 경우고 **7은 규칙이 넓었던 경우**다. 7이 없으면 이 절은 "의심스러우면 넓게 잡아라"로 읽히는데, 그 독해가 만드는 것이 바로 7번 사고다.

### 어디에 적용하는가

- **Phase 종료 판정** — `00_progress.md`의 종료 조건 행에 `충족 = 예`를 쓰기 전에, 그 행의 근거가 **실제로 돈 경로**에서 나왔는지 확인한다. 표 아래 "아직 돌리지 않은 게이트 목록"에는 게이트 이름만 적지 말고 **각 게이트가 지금 어디서 도는가**(로컬 명령 / CI 잡 이름 / 안 돎)를 함께 적는다. "안 돎"이 적힌 게이트를 근거로 종료 조건을 닫지 않는다.
