...
#:
#: ## 왜 한정/맨 두 갈래로 쪼갰나 (게이트 29, 리더 재판정)
#:
#: ⓑ 의 수신자 꼬리표(`_403_RECEIVER_TAIL`)가 **어느 이름에 붙는가**를 가르기 위해서다.
#: 꼬리표는 이름 뒤에 체인이 이어지면 "이 이름은 수신자다"로 보고 떨어뜨리는데, 그
#: 판단이 옳은 자리는 **점 없는 맨 식별자**(`FORBIDDEN_HANDLES.any { … }`)이고
#: 틀린 자리는 **한정된 이름**(`HttpStatus.FORBIDDEN.value()`)이다 — 후자는 앞의
#: 한정자(`HttpStatus.`)가 이미 "이 이름은 HTTP 상태다"를 말하므로 뒤에 `.value()` 가
#: 이어져도 나가는 값은 여전히 403 이다.
#:
#: 두 조각을 나눠 두면 ⓑ 가 갈래마다 다른 꼬리표 정책을 쓸 수 있고, `_403_NAME` 은
#: **두 조각에서 합성**되므로 ⓐ 가 보는 범위는 한 글자도 바뀌지 않는다. 손으로 세 벌을
#: 적으면 사본이 갈리고, 갈린 사본은 늘 조용한 쪽이었다(게이트 23 ⓐ).
_403_NAME_QUALIFIED = r"(?:[A-Za-z_]\w*\.)+\w*(?:FORBIDDEN|Forbidden)\w*"
_403_NAME_BARE = r"\w*(?:FORBIDDEN|Forbidden)\w*"
_403_NAME = rf"(?:{_403_NAME_QUALIFIED}|{_403_NAME_BARE})"
...
#:   ⓑ 에서 **한정된 이름**(`HttpStatus.FORBIDDEN`)은 뒤에 체인이 이어져도
#:   (`.value()`) 본다 — 앞의 한정자가 이미 자리를 보증한다.
#: - **안 본다**: 두 자리 **밖**의 이름(선언·`in` 연산·문자열·후행 주석 — Q1~Q4) ·
#:   ⓑ 자리 안이더라도 **점 없는 맨 식별자 뒤에 멤버 접근·첨자가 이어지는 경우**
#:   (`-> FORBIDDEN_HANDLES.any { … }` — `_403_RECEIVER_TAIL`).
#:
#: 둘째 항의 **적용 대상이 이번 회차(게이트 29)에 좁혀졌다.** 직전 판(게이트 28)은 같은
#: 꼬리표를 ⓑ 의 **모든** 이름에 걸었고, 그래서 `return HttpStatus.FORBIDDEN.value()`
#: 라는 **진짜 403** 을 함께 잃었다. 리더 재판정으로 꼬리표를 맨 식별자에만 건다.
#:
#: ## 왜 오탐 쪽으로 기울였나 (이 문단을 지우면 다음 사람이 소음을 이유로 되돌린다)
#:
#: 두 오차의 **성질이 대칭이 아니다.**
#:
#: | 오차 | 형태 | 무슨 일이 벌어지나 |
#: |---|---|---|
#: | **오탐** | `Rules.FORBIDDEN_HANDLES.any { … }` 를 403 으로 봄 |
#:   BLOCK 이 떠서 **사람이 본다**. 시끄럽지만 안전하다 |
#: | **미탐** | `return HttpStatus.FORBIDDEN.value()` 를 놓침 |
#:   **아무 신호가 없다**. 조용하고 위험하다 |
#:
#: `CLAUDE.md` 규칙 4 ⑵ 가 은폐형을 넓히지 못하게 하는 이유가 정확히 이것이다 —
#: 은폐 장치를 넓히는 것은 다음 사고를 안 보이게 만드는 일이다. 게이트 28 판은 참 적중
#: 하나를 **조용히** 잃는 쪽이었고, 지금 판은 오탐 하나를 **시끄럽게** 남기는 쪽이다.
#:
#: **오늘 실측으로는 이 선택에 손해가 없다.** 게이트 28 이 닫은 네 형태(저장소 실제 줄 ·
#: `FORBIDDEN_HANDLES.any{}` · `FORBIDDEN_IN_FILENAME.contains(ch)` ·
#: `FORBIDDEN_ANNOTATIONS[k]`)는 **넷 다 맨 식별자**라 그대로 닫힌 채 남고, `.value()`
#: 갈래만 되돌아온다. 새로 받아들이는 오탐은 **한정된 수신자**(`Rules.FORBIDDEN_HANDLES`)
#: 하나인데 저장소에 그런 자리는 0건이고, 생기더라도 BLOCK 으로 시끄럽게 드러난다.
#: 그 오탐은 회귀 표에 **`blocks=True` 인 케이스로 명시**돼 있다 — 다음 사람이 그것을
#: 결함으로 오해해 조용히 좁히지 못하게 하려는 것이다.
#:
...
#: **수신자 꼬리표** — ⓑ 의 **맨 식별자** 갈래에서 "이름이 곧 나가는 값"이 **아닌**
#: 자리를 떨어뜨린다.
#:
#: 매치된 이름 바로 뒤에 멤버 접근(`.식별자`)이나 첨자(`[`)가 이어지면 그 이름은
#: **수신자**이고, 나가는 값은 체인의 결과다. `-> FORBIDDEN_HANDLES.any { … }` 가 그것이다.
#:
#: **붙는 자리는 맨 식별자(`_403_NAME_BARE`) 하나다** (게이트 29 리더 재판정).
#:
#: - ⓐ 에 안 붙이는 이유 — 바깥 호출 이름(`sendError`·`status`)이 이미 자리를 보증하므로
#:   인자가 체인이어도 근거가 남는다(N24).
#: - ⓑ 의 **한정된 이름**(`_403_NAME_QUALIFIED`)에 안 붙이는 이유 — 앞의 한정자
#:   (`HttpStatus.`)가 그 이름이 HTTP 상태임을 말한다. `return HttpStatus.FORBIDDEN.value()`
#:   는 체인을 거쳐도 나가는 값이 403 이므로 **진짜 적중**이다(D3). 게이트 28 판은 여기에도
#:   꼬리표를 걸어 그 적중을 조용히 잃었다 — 미탐은 신호가 없고 오탐은 BLOCK 으로 보인다.
#: - 맨 식별자에 붙이는 이유 — 그 갈래에는 이름 자신 말고 자리를 보증하는 것이 없다.
#:   `FORBIDDEN_HANDLES`·`FORBIDDEN_IN_FILENAME`·`FORBIDDEN_ANNOTATIONS` 가 전부 이 형태다.
#:
...
#: 사본은 늘 조용한 쪽이었다(게이트 23 ⓐ 가 겪은 형태). **붙는 자리**도 같은 방식으로
#: 잰다 — `test_수신자_꼬리표는_맨_식별자에만_붙는다` 가 조각 문자열로 대조한다.
...
_403_STATUS_SITE = (
    ...
    #    내보내는 것이 아니라 읽는 것이다.
    #
    #    이 갈래는 **이름의 모양에 따라 둘로 갈린다**(게이트 29 리더 재판정):
    #
    #      · 한정된 이름(`HttpStatus.FORBIDDEN`) — 꼬리표 **없음**. 앞의 한정자가 자리를
    #        보증하므로 뒤에 `.value()` 가 이어져도 나가는 값은 403 이다(D3).
    #      · 맨 식별자(`FORBIDDEN_HANDLES`) — 꼬리표 **있음**. 자리를 보증하는 것이 이름
    #        자신뿐이라, 뒤에 체인이 이어지면 나가는 값은 그 이름이 아니라 체인의 결과다
    #        (`-> FORBIDDEN_HANDLES.any { … }` — R1~R4).
    #
    #    한정 갈래를 **먼저** 둔다 — 맨 갈래가 `\w*` 로 시작해 한정자 조각을 삼킬 수 있는
    #    형태는 아니지만(한정자 뒤 이름이 403 토큰을 품어야 한다), 읽는 순서를 의미 순서와
    #    맞춰 둔다.
    #
    #    꼬리표는 **맨 갈래의 문자열 안에서** 끝난다. 밖에서 `+` 로 이어 붙이면 뒤에 갈래가
    #    하나 더 생기는 순간 꼬리표가 **그 새 갈래로 조용히 옮겨간다** — 아래
    #    `test_수신자_꼬리표를_떼면_오탐이_되살아난다` 와
    #    `test_수신자_꼬리표는_맨_식별자에만_붙는다` 가 그 이동도 잡지만, 붙는 자리를
    #    문면으로 못 박아 두는 편이 낫다.
    rf"|(?:\b(?:return|throw|raise)\b|->|(?<![=!<>+\-*/%])=(?!=))"
    rf"\s*(?:{_403_NAME_QUALIFIED}\b|{_403_NAME_BARE}\b{_403_RECEIVER_TAIL})"
)
...
RULES: tuple[Rule, ...] = (
    ...
    Rule(
        ...
        "값 산출 자리 안이더라도 **점 없는 맨 식별자** 뒤에 멤버 접근·첨자가 이어지면"
        "(`FORBIDDEN_X.any { … }`) 그 이름은 수신자이지 나가는 값이 아니라 후보가 아니다. "
        "한정된 이름(`HttpStatus.FORBIDDEN.value()`)은 체인이 이어져도 후보다 — 미탐은 "
        "조용하고 오탐은 이 BLOCK 으로 드러나므로 오탐 쪽으로 기울였다(게이트 29).",
        None,
        (),
